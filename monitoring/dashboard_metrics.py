...
if __name__ == "__main__":
    ...
    try:
        ...
        if hasattr(collector, 'api_base_url'):
            ...
            all_validation_failures.append("DashboardMetricsCollector missing api_base_url attribute")
    except Exception as e:
        all_validation_failures.append(f"DashboardMetricsCollector instantiation failed: {e}")
    ...
    try:
        ...
        if isinstance(metrics, dict) and all(key in metrics for key in expected_keys):
            ...
            all_validation_failures.append(f"Metrics structure invalid, got keys: {list(metrics.keys()) if isinstance(metrics, dict) else 'not dict'}")
    except Exception as e:
    ...
    try:
        ...
        if "api" in metrics["services"] or "docker" in metrics["services"]:
            ...
            all_validation_failures.append("No metrics were collected by individual methods")
    except Exception as e:
        all_validation_failures.append(f"Individual metrics collection methods failed: {e}")
