    ...
    def cleanup_old_data(self, days: int = 30):
        ...
        try:
            ...
            with sqlite3.connect(self.db_path) as conn:
                ...
                cursor.execute("DELETE FROM deployment_events WHERE timestamp < ?", (deployment_cutoff,))
                events_deleted = cursor.rowcount
                
                conn.commit()
                
                logger.info(f"Cleaned up old data: {metrics_deleted} metrics, {status_deleted} status, {events_deleted} events")
...
if __name__ == "__main__":
    ...
    try:
        ...
        if isinstance(history, list):
            ...
            all_validation_failures.append(f"Event logging/retrieval failed: expected list, got {type(history)}")
    except Exception as e:
    ...
    try:
        ...
        test_metrics = {
            ...
            "services": {
                "api": {"status": "healthy", "response_time": 100}
            },
            "performance": {
                "cpu_percent": 50.0
            }
