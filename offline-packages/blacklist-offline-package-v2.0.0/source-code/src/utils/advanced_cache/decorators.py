...
def cache_decorator(
    ...
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(*args, **kwargs):
            ...
            if not cache:
                logger.warning(
                    "No cache instance available, executing function directly"
                )
...
def cache_lock(lock_key: str, timeout: int = 30, retry_interval: float = 0.1):
    ...
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(*args, **kwargs):
            ...
            if not cache:
                logger.warning(
                    "No cache instance available for locking, executing function directly"
                )
                return func(*args, **kwargs)
...
def cache_result_only(ttl: int = 300, key_prefix: str = None):
    ...
    return cache_decorator(
        ttl=ttl, key_prefix=key_prefix, include_args=False, include_kwargs=False
    )
