...
class CICDUtils:
    ...
    def get_file_content(self, project_id: str, file_path: str) -> str:
        ...
        try:
            response = self.session.get(
                "{self.base_url}/api/v1/projects/{project_id}/files/{file_path}"
            )
    ...
    def update_file(self, project_id: str, file_path: str, content: str) -> bool:
        ...
        try:
            data = {
                ...
                "file_path": file_path,
                "content": content,
                "commit_message": "Auto-fix: {file_path}",
            }
    ...
    def retry_pipeline(self, project_id: str, pipeline_id: str) -> bool:
        ...
        try:
            response = self.session.post(
                "{self.base_url}/api/v1/pipelines/{pipeline_id}/retry",
    ...
    def get_project_info(self, project_id: str) -> Dict[str, Any]:
        """프로젝트 정보 조회"""
        try:
            response = self.session.get("{self.base_url}/api/v1/projects/{project_id}")
            if response.status_code == 200:
    ...
    def list_pipelines(self, project_id: str, status: str = "failed") -> list:
        ...
        try:
            ...
            response = self.session.get(
                "{self.base_url}/api/v1/pipelines", params=params
            )
            if response.status_code == 200:
    ...
    def send_notification(
        ...
        try:
            ...
            response = self.session.post(
                "{self.base_url}/api/v1/notifications", json=data
            )
