...
def analyze_metrics(results: Dict) -> Tuple[bool, List[str]]:
    ...
    if 'http_req_duration' in results['metrics']:
        ...
        if duration['p(95)'] > 500:
            failures.append(f"p95 response time {duration['p(95)']}ms exceeds 500ms threshold")
        
        # Check p99 < 1000ms
        if duration['p(99)'] > 1000:
            failures.append(f"p99 response time {duration['p(99)']}ms exceeds 1000ms threshold")
    ...
    if 'http_req_failed' in results['metrics']:
        ...
        if fail_rate > 0.05:
            failures.append(f"HTTP failure rate {fail_rate*100:.2f}% exceeds 5% threshold")
