            ...
            for pattern_name, pattern_regex in self.patterns.items():
                ...
                for init_content in init_methods:
                    if re.search(pattern_regex, init_content):
                        ...
                        matched_lines = [line.strip() for line in lines if re.search(pattern_regex, line)]
...
def analyze_codebase(src_path: Path) -> Dict[str, any]:
    ...
    for file_path in python_files:
        # 테스트 파일과 특정 파일들 제외
        if any(exclude in str(file_path) for exclude in ['test_', '__pycache__', '.pyc']):
            continue
...
def generate_report(analysis_results: Dict[str, any]) -> str:
    ...
    report_lines.append(f"  - 분석 비율: {analysis_results['files_with_patterns']/analysis_results['total_files']*100:.1f}%")
    report_lines.append("")
    
    # 카테고리별 빈도
    report_lines.append("📋 카테고리별 패턴 빈도:")
    for category, count in sorted(analysis_results['category_frequency'].items(), key=lambda x: x[1], reverse=True):
        report_lines.append(f"  - {category:12}: {count:3}개")
    report_lines.append("")
    
    # 패턴별 상세 빈도
    report_lines.append("🔍 패턴별 상세 빈도:")
    for pattern, count in sorted(analysis_results['pattern_frequency'].items(), key=lambda x: x[1], reverse=True):
    ...
    for file_path, pattern_count, total_occurrences in high_duplicate_files[:10]:
        report_lines.append(f"  - {file_path:50} ({pattern_count}개 패턴, {total_occurrences}회 출현)")
    report_lines.append("")
    ...
    if service_pattern_count > 15:
        report_lines.append(f"  ✅ BaseService 클래스 도입 권장 (현재 {service_pattern_count}개 중복)")
    
    if timestamp_pattern_count > 5:
        report_lines.append(f"  ✅ TimestampMixin 도입 권장 (현재 {timestamp_pattern_count}개 중복)")
...
if __name__ == "__main__":
    ...
    try:
        ...
        if len(analyzer.patterns) < 5:
            all_validation_failures.append("Pattern test: Insufficient patterns defined")
    except Exception as e:
