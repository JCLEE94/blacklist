...
class ArgoAuthManager:
    ...
    def check_server_connectivity(self):
        ...
        try:
            # ArgoCD 서버 연결 테스트
            result = subprocess.run(
                ['argocd', 'cluster', 'list', '--server', self.argocd_server, '--grpc-web'],
    ...
    def check_authentication_status(self):
        ...
        try:
            result = subprocess.run(
                ['argocd', 'account', 'get-user-info', '--server', self.argocd_server, '--grpc-web'],
    ...
    def list_applications(self):
        ...
        try:
            result = subprocess.run(
                ['argocd', 'app', 'list', '--server', self.argocd_server, '--grpc-web', '-o', 'json'],
                capture_output=True, text=True, timeout=30
            )
    ...
    def verify_auth_and_generate_report(self):
        ...
        if not cli_ok:
            ...
            report['recommendations'].append("ArgoCD CLI를 설치하세요: curl -sSL -o argocd-linux-amd64 https://github.com/argoproj/argo-cd/releases/latest/download/argocd-linux-amd64")
        ...
        if cli_ok:
            ...
            if not conn_ok:
                ...
                if "인증이 필요" in conn_msg:
                    report['recommendations'].append(f"ArgoCD 로그인: argocd login {self.argocd_server} --grpc-web")
        ...
        if report['authentication']:
            ...
            if app_ok:
                ...
                if not apps_or_error:
                    report['recommendations'].append(f"'{project_name}' 관련 ArgoCD 애플리케이션이 없습니다. 새로 생성하는 것을 고려하세요.")
            else:
    ...
    def display_report(self, report):
        ...
        print(f"  {conn_emoji} 서버 연결: {'성공' if report['server_connectivity'] else '실패'}")
        ...
        if report['project_applications']:
            ...
            for app in report['project_applications']:
                ...
                print(f"  {sync_emoji} {health_emoji} {app['name']} (Sync: {app['sync_status']}, Health: {app['health_status']})")
