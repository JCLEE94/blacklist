...
cursor.execute(
    "SELECT country, COUNT(*) as count FROM blacklist_ip WHERE country IS NOT NULL AND country != '' GROUP BY country ORDER BY count DESC LIMIT 10"
)
...
cursor.execute(
    "SELECT detection_date, COUNT(*) as count FROM blacklist_ip GROUP BY detection_date ORDER BY detection_date DESC LIMIT 10"
)
...
cursor.execute(
    "SELECT threat_type, COUNT(*) as count FROM blacklist_ip WHERE threat_type IS NOT NULL GROUP BY threat_type"
)
