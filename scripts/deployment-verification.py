...
class DeploymentVerifier:
    ...
    def __init__(
        self, target_url: str = "https://blacklist.jclee.me", timeout: int = 300
    ):
    ...
    def verify_version_deployment(self) -> Tuple[bool, Dict]:
        ...
        version_endpoints = ["/health", "/api/health", "/", "/api", "/api/v2/health"]
            ...
            if success and "data" in result and isinstance(result["data"], dict):
                ...
                if found_version:
                    ...
                    if version_match["matches"]:
                        ...
                        print(
                            f"⚠️ Version mismatch at {endpoint}: found {found_version}, expected {self.expected_version}"
                        )

        deployment_info["deployment_status"] = (
            "success" if version_found else "version_mismatch"
        )
    ...
    def run_comprehensive_health_check(self) -> Tuple[bool, Dict]:
        ...
        health_results = {"overall_health": True, "checks": {}, "summary": {}}
        ...
        for endpoint, expected in critical_endpoints.items():
            ...
            if not actual_success:
                ...
                print(
                    f"❌ Critical endpoint failed: {endpoint} (status: {result.get('status_code')})"
                )
            else:
        ...
        for endpoint, expected in optional_endpoints.items():
            ...
            health_results["checks"][endpoint] = {**result, "critical": False}
        ...
        health_results["summary"] = {
            ...
            "health_score": round(
                (
                    (len(critical_endpoints) - critical_failures)
        ...
        while (datetime.now() - self.start_time).total_seconds() < max_wait:
            ...
            if health_success:
                ...
                if version_success:
                    ...
                    print(
                        f"⚠️ Service is up but version not yet updated (attempt {attempt})"
                    )
            else:
...
def main():
    ...
    parser.add_argument(
        "--wait", action="store_true", help="Wait for deployment to complete"
    )
    parser.add_argument(
        "--health-only",
        action="store_true",
