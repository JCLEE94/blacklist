    ...
    def find_similar_files(self) -> List[Dict]:
        ...
        for file_path in python_files:
            try:
                ...
                file_signatures[str(file_path.relative_to(self.project_root))] = signature
        ...
        for i in range(len(files)):
            for j in range(i + 1, len(files)):
                ...
                common_functions = set(sig1['function_names']) & set(sig2['function_names'])
                if common_functions:
...
def main():
    ...
    try:
        ...
        print(f"\U0001F504 Duplicate Function Groups: {stats['duplicate_function_groups']}")
        print(f"\U0001F4C4 Similar File Pairs: {stats['similar_file_pairs']}")
        print(f"\U0001F5D1\uFE0F Potentially Dead Files: {stats['potentially_dead_files']}")
        ...
        if report['similar_files']:
            ...
            for similarity in report['similar_files'][:5]:
                ...
                if similarity['common_functions']:
                    print(f"    Common functions: {', '.join(similarity['common_functions'][:3])}")
        ...
        common_imports = [(imp, files) for imp, files in report['common_imports'].items() 
                         if len(files) > 15]
        if common_imports:
            print()
            print("\U0001F4E6 MOST COMMON IMPORTS (consolidation opportunities):")
            for import_stmt, files in sorted(common_imports, key=lambda x: len(x[1]), reverse=True)[:5]:
