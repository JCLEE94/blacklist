...
def find_download_method():
    ...
    # 버튼
    buttons = soup.find_all(['button', 'a', 'input'], class_=re.compile(r'download|excel|export', re.I))
    for btn in buttons:
    ...
    print("\n5. 다운로드 함수 내용 확인:")
    for script in scripts:
        if script.string and any(keyword in script.string for keyword in ['advisoryListDownload', 'excelDownload', 'downloadExcel']):
            # 함수 내용 추출
            ...
            for line in lines:
                if any(keyword in line for keyword in ['download', 'excel']) and 'function' in line:
    ...
    for form in forms:
        ...
        if any(keyword in (form_id + form_name).lower() for keyword in ['download', 'excel', 'search']):
        ...
        if tbody:
            ...
            if rows:
                ...
                for j, cell in enumerate(cells):
                    ...
                    if any(keyword in text for keyword in ['IP', 'ip', '주소', 'address']):
