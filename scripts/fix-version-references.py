...
def find_old_version_references(project_root: Path, current_version: str) -> Dict[str, List[Tuple[int, str, str]]]:
    ...
    file_patterns = [
        '**/*.py', '**/*.md', '**/*.yml', '**/*.yaml', '**/*.json',
        '**/*.sh', '**/*.txt', '**/Dockerfile*', '**/*.toml'
    ]
    
    skip_patterns = [
        '.git/', '__pycache__/', '.pytest_cache/', 'node_modules/',
        '.venv/', 'venv/', 'logs/', 'data/', 'instance/',
        '.pyc', '.pyo', '.jpg', '.png', '.gif', '.ico'
    ]
    ...
    for pattern in file_patterns:
        for file_path in project_root.glob(pattern):
            if file_path.is_file() and not any(skip in str(file_path) for skip in skip_patterns):
                try:
                    ...
                    for line_num, line in enumerate(lines, 1):
                        for old_version in old_versions:
                            if old_version in line and current_version not in line:
                                ...
                                file_changes.append((line_num, line.strip(), new_line.strip()))
                                break
...
def apply_version_fixes(project_root: Path, references: Dict[str, List[Tuple[int, str, str]]], current_version: str) -> int:
...
def main():
    ...
    print("   3. Commit changes: git add . && git commit -m 'fix: update version references'")
