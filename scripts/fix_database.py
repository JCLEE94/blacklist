...
def fix_database():
    """Fix database schema"""
    db_path = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'instance', 'blacklist.db')
    ...
    cursor.execute("CREATE INDEX IF NOT EXISTS idx_blacklist_created ON blacklist_ip(created_at)")
