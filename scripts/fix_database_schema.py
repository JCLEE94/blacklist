    ...
    try:
        ...
        if hasattr(monitor, "_check_cache_status"):
            ...
            try:
                ...
                logger.info(
                    f"✅ _check_cache_status 실행 성공: {result.get('status', 'unknown')}"
                )
                return True
    ...
    try:
        with schema.connection_manager.get_connection() as conn:
            ...
            required_tables = [
                (
                    "blacklist_entries",
                    schema.table_definitions.create_blacklist_entries_table,
                ),
                (
                    "collection_logs",
                    schema.table_definitions.create_collection_logs_table,
                ),
