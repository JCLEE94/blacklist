    ...
    try:
        ...
        if not ip_col:
            ...
            for col in df.columns:
                ...
                if any(
                    "." in str(val) and len(str(val).split(".")) == 4 for val in sample
                ):
                    ip_col = col
                    break
        ...
        if result.get("success"):
            logger.info(
                f"✅ Successfully imported {result.get('imported_count', 0)} IPs"
            )
...
def main():
    ...
    if len(sys.argv) < 2:
        ...
        print(
            "  python import_regtech_excel.py ~/Downloads/regtech_blacklist.xlsx --clear"
        )
        sys.exit(1)
