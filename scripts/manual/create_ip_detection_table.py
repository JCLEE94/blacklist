...
def create_ip_detection_table():
    ...
    try:
        ...
        cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='ip_detection'")
        if cursor.fetchone():
        ...
        cursor.execute("CREATE INDEX idx_ip_detection_created_at ON ip_detection(created_at)")
