    ...
    def analyze_dependencies(self, src_path: Path) -> Dict:
        ...
        for file_path in python_files:
            ...
            for import_name in file_info['imports']:
                ...
                if import_name.startswith('.') or any(part in import_name for part in ['core', 'utils', 'api']):
...
def generate_structure_report(src_path: Path) -> str:
    ...
    avg_complexity = sum(info['complexity_score'] for info in analyzer.file_info.values()) / len(analyzer.file_info) if analyzer.file_info else 0
    ...
    if issues['large_files']:
        ...
        for item in sorted(issues['large_files'], key=lambda x: x['lines'], reverse=True)[:5]:
            report_lines.append(f"  - {item['file']:50} ({item['lines']:3}줄, {item['functions']:2}함수, {item['classes']:2}클래스)")
        report_lines.append("")
    
    if issues['complex_files']:
        report_lines.append("🔄 복잡한 파일들 (상위 5개):")
        for item in sorted(issues['complex_files'], key=lambda x: x['complexity'], reverse=True)[:5]:
            report_lines.append(f"  - {item['file']:50} (복잡도 {item['complexity']:2}, {item['functions']:2}함수)")
        report_lines.append("")
