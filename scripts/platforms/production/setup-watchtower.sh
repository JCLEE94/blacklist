...
# 기존 컨테이너 정리 (있을 경우)
echo "🧹 기존 컨테이너 정리 중..."
docker-compose -f deployment/docker-compose.watchtower.yml down 2>/dev/null || true
...
docker-compose -f deployment/docker-compose.watchtower.yml up -d

# 상태 확인
echo ""
echo "📊 컨테이너 상태:"
docker-compose -f deployment/docker-compose.watchtower.yml ps

echo ""
echo "✅ Watchtower 설정 완료!"
echo ""
echo "📌 사용 방법:"
echo "  - 시작: docker-compose -f deployment/docker-compose.watchtower.yml up -d"
echo "  - 중지: docker-compose -f deployment/docker-compose.watchtower.yml down"
echo "  - 로그: docker-compose -f deployment/docker-compose.watchtower.yml logs -f"
