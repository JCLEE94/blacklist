#!/bin/bash

# Application Standalone Container Runner
# Builds and runs Application container independently

set -e

# Colors for output
RED='\033[0;31m'
GREEN='\033[0;32m'
YELLOW='\033[1;33m'
BLUE='\033[0;34m'
NC='\033[0m'

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" &> /dev/null && pwd)"
PROJECT_ROOT="$(dirname "$SCRIPT_DIR")"

CONTAINER_NAME="blacklist-app-standalone"
IMAGE_NAME="blacklist-application:standalone"
NETWORK_NAME="blacklist-standalone"
CONTAINER_IP="172.25.0.30"
POSTGRES_IP="172.25.0.10"
REDIS_IP="172.25.0.20"

# Build Application image
build() {
    echo -e "${YELLOW}Building Application image...${NC}"
    cd "$PROJECT_ROOT"
    docker build -f docker/application/Dockerfile -t "$IMAGE_NAME" .
    if [ $? -eq 0 ]; then
        echo -e "${GREEN}✓ Application image built successfully${NC}"
    else
        echo -e "${RED}✗ Failed to build Application image${NC}"
        exit 1
    fi
}

# Start Application container
start() {
    echo -e "${YELLOW}Starting Application container...${NC}"
    
    # Create network if it doesn't exist
    if ! docker network ls | grep -q "$NETWORK_NAME"; then
        echo -e "${YELLOW}Creating network $NETWORK_NAME...${NC}"
        docker network create --subnet=172.25.0.0/16 "$NETWORK_NAME"
    fi
    
    # Stop and remove existing container
    docker stop "$CONTAINER_NAME" 2>/dev/null || true
    docker rm "$CONTAINER_NAME" 2>/dev/null || true
    
    # Start container
    docker run -d \
        --name "$CONTAINER_NAME" \
        --network="$NETWORK_NAME" \
        --ip="$CONTAINER_IP" \
        -p 2542:2542 \
        -e DATABASE_URL="postgresql://blacklist_user:blacklist_standalone_password_change_me@$POSTGRES_IP:5432/blacklist" \
        -e REDIS_URL="redis://$REDIS_IP:6379/0" \
        -e REGTECH_USERNAME="demo" \
        -e REGTECH_PASSWORD="demo" \
        -e SECUDIUM_USERNAME="demo" \
        -e SECUDIUM_PASSWORD="demo" \
        -e FLASK_ENV=production \
        -e PORT=2542 \
        -e COLLECTION_ENABLED=true \
        -e FORCE_DISABLE_COLLECTION=false \
        -v blacklist-app-data:/app/data \
        -v blacklist-app-logs:/app/logs \
        --restart=unless-stopped \
        "$IMAGE_NAME"
    
    if [ $? -eq 0 ]; then
        echo -e "${GREEN}✓ Application container started${NC}"
        echo -e "Container: $CONTAINER_NAME"
        echo -e "IP: $CONTAINER_IP"
        echo -e "Port: 2542"
        echo -e "URL: http://localhost:2542"
    else
        echo -e "${RED}✗ Failed to start Application container${NC}"
        exit 1
    fi
}

# Stop container
stop() {
    echo -e "${YELLOW}Stopping Application container...${NC}"
    docker stop "$CONTAINER_NAME" 2>/dev/null || true
    echo -e "${GREEN}✓ Application container stopped${NC}"
}

# Remove container
remove() {
    echo -e "${YELLOW}Removing Application container...${NC}"
    stop
    docker rm "$CONTAINER_NAME" 2>/dev/null || true
    echo -e "${GREEN}✓ Application container removed${NC}"
}

# Show status
status() {
    echo -e "${BLUE}=== Application Container Status ===${NC}"
    if docker ps | grep -q "$CONTAINER_NAME"; then
        echo -e "${GREEN}✓ Container: Running${NC}"
        docker ps --format "table {{.Names}}\t{{.Status}}\t{{.Ports}}" | grep "$CONTAINER_NAME"
        
        # Test connection
        if curl -f -s http://localhost:2542/health > /dev/null 2>&1; then
            echo -e "${GREEN}✓ Application: Ready${NC}"
        else
            echo -e "${RED}✗ Application: Not ready${NC}"
        fi
    else
        echo -e "${RED}✗ Container: Not running${NC}"
    fi
}

# Show logs
logs() {
    docker logs -f "$CONTAINER_NAME"
}

# Health check
health() {
    if curl -f -s http://localhost:2542/health > /dev/null 2>&1; then
        echo -e "${GREEN}✓ Application is healthy${NC}"
        curl -s http://localhost:2542/health | jq . 2>/dev/null || curl -s http://localhost:2542/health
        return 0
    else
        echo -e "${RED}✗ Application is not healthy${NC}"
        return 1
    fi
}

case "${1:-help}" in
    build) build ;;
    start) build && start ;;
    stop) stop ;;
    remove) remove ;;
    status) status ;;
    logs) logs ;;
    health) health ;;
    *)
        echo "Application Standalone Container"
        echo "Usage: $0 {build|start|stop|remove|status|logs|health}"
        echo ""
        echo "Commands:"
        echo "  build   - Build Application image"
        echo "  start   - Build and start container"
        echo "  stop    - Stop container"
        echo "  remove  - Stop and remove container"
        echo "  status  - Show container status"
        echo "  logs    - Show container logs"
        echo "  health  - Check application health"
        ;;
esac