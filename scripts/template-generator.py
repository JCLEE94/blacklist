...
class TemplateGenerator:
    ...
    def load_nodeport_config(self):
        ...
        if not self.nodeport_config_file.exists():
            raise FileNotFoundError(f"NodePort 설정 파일이 없습니다: {self.nodeport_config_file}")
    ...
    def generate_kustomization_template(self, nodeport_config):
        ...
        kustomization = {
            ...
            'kind': 'Kustomization',
            'metadata': {
                'name': f"{project_name}-kustomization"
            },
            'namespace': project_name,
            'resources': [
                'deployment.yaml',
    ...
    def save_templates(self, nodeport_config):
        ...
        templates = {
            ...
            'argocd-application.yaml': self.generate_argocd_application_template(nodeport_config),
