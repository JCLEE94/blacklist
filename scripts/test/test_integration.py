...
def test_statistics_integration():
    ...
    try:
        ...
        with test_app.test_client() as client:
            ...
            assert 'success' in analytics_data and analytics_data['success'], "분석 API 응답 실패"
            ...
            assert trends['total_ips'] == total_ips, f"총 IP 수 불일치: stats={total_ips}, trends={trends['total_ips']}"
            assert trends['active_ips'] == active_ips, f"활성 IP 수 불일치: stats={active_ips}, trends={trends['active_ips']}"
            ...
            print(f"    - 상위 국가: {top_countries[0]['country']} ({top_countries[0]['count']}개)")
            ...
            if len(daily_trends) > 0:
                ...
                print(f"    - 최근 트렌드: {daily_trends[0]['date']} ({daily_trends[0]['new_ips']}개)")
...
def test_database_api_consistency():
    ...
    try:
        ...
        cursor.execute('SELECT source, COUNT(*) FROM blacklist_ip WHERE is_active = 1 GROUP BY source')
        ...
        with test_app.test_client() as client:
            ...
            # 3. 일관성 검증
            assert db_active_count == api_active_count, f"활성 IP 수 불일치: DB={db_active_count}, API={api_active_count}"
            assert db_sources.get('REGTECH', 0) == api_regtech_count, f"REGTECH 카운트 불일치: DB={db_sources.get('REGTECH', 0)}, API={api_regtech_count}"
            assert trends['active_ips'] == api_active_count, f"트렌드 활성 IP 불일치: Trends={trends['active_ips']}, API={api_active_count}"
            ...
            if db_countries and trends['top_countries']:
                ...
                assert db_top_country[0] == api_top_country['country'], f"상위 국가 불일치: DB={db_top_country[0]}, API={api_top_country['country']}"
                assert db_top_country[1] == api_top_country['count'], f"상위 국가 카운트 불일치: DB={db_top_country[1]}, API={api_top_country['count']}"
...
def test_collection_data_flow():
    ...
    try:
        ...
        with test_app.test_client() as client:
            ...
            enabled = status_data.get('enabled', status_data.get('collection_enabled', False))
            assert enabled is not None, f"수집 상태 필드를 찾을 수 없습니다: {status_data.keys()}"
            ...
            if 'application/json' in content_type:
                ...
                if isinstance(active_data, dict) and 'ips' in active_data:
                    ...
                    active_count = len(active_data) if isinstance(active_data, list) else 0
            else:
                # 텍스트 형식인 경우
                text_data = response.get_data(as_text=True)
                active_count = len([line for line in text_data.split('\n') if line.strip()])
            ...
            assert 'threat_feed' in fortigate_data or 'entries' in fortigate_data or 'blacklist' in fortigate_data, f"FortiGate 응답 구조가 예상과 다릅니다: {fortigate_data.keys()}"
            
            # 5. 데이터 일관성 검증
            print("  ✓ 전체 플로우 데이터 일관성 검증...")
            
            # 통계와 활성 IP 수가 일치하는지 확인
            assert active_count == before_count, f"통계와 활성 IP 수 불일치: 통계={before_count}, 활성목록={active_count}"
    ...
    try:
        ...
        with test_app.test_client() as client:
            for endpoint in endpoints:
                ...
                # 성능 기준: 평균 500ms 이하
                assert avg_time < 0.5, f"{endpoint} 평균 응답시간이 너무 깁니다: {avg_time*1000:.2f}ms"
