...
class UIFunctionVerifier:
    ...
    def verify_all_functions(self) -> Dict[str, Any]:
        ...
        results = {"total_tests": len(tests), "passed": 0, "failed": 0, "details": []}
        ...
        for test_name, test_func in tests:
            try:
                ...
                if result["success"]:
                    ...
                    results["failed"] += 1
                    logger.error(
                        f"❌ {test_name}: 실패 - {result.get('error', 'Unknown error')}"
                    )
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if not missing:
                    ...
                    return {"success": False, "error": f"HTML 요소 누락: {missing}"}
            else:
    ...
    def verify_css_loading(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if loaded_css:
                return {"success": True, "details": f"로딩된 CSS: {len(loaded_css)}개"}
            else:
                return {
                    "success": False,
    ...
    def verify_js_loading(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if loaded_js:
                return {"success": True, "details": f"로딩된 JS: {len(loaded_js)}개"}
            else:
                return {
                    "success": False,
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if found >= 2:
                    return {
                        "success": True,
                        "details": f"데이터 관리 요소 {found}개 발견",
                    }
                else:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if isinstance(data, dict) and "collection_enabled" in data:
                    return {
                        "success": True,
                        "details": f"수집 상태: {data.get('collection_enabled')}",
                    }
                else:
    ...
    def verify_analytics(self) -> Dict[str, Any]:
        """통계 및 분석 기능 검증"""
        try:
            analytics_urls = ["/api/v2/analytics/trends", "/api/v2/sources/status"]
    ...
    def verify_health_endpoints(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if working_count >= 3:
                ...
                return {
                    "success": False,
                    "error": f"헬스체크 부족: {working_count}개만 작동",
                }
