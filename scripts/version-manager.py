...
class VersionManager:
    ...
    def validate_version_consistency(self) -> bool:
        ...
        if inconsistent_files:
            print(
                f"❌ Version inconsistencies found in {len(inconsistent_files)} locations:"
            )
    ...
    def create_version_commit(
        self, new_version: VersionInfo, bump_type: VersionBumpType
    ) -> bool:
...
def main():
    ...
    parser.add_argument(
        "--validate-only", action="store_true", help="Only validate version consistency"
    )
    parser.add_argument(
        "--show-references",
        action="store_true",
    ...
    parser.add_argument(
        "--no-commit", action="store_true", help="Skip creating git commit and tag"
    )
    ...
    if success and not args.no_commit:
        bump_type = (
            VersionBumpType(args.bump_type)
            if args.bump_type and args.bump_type != "auto"
            else vm.detect_version_bump_type()
        )
