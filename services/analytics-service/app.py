...
class AnalyticsManager:
    ...
    def __init__(self):
        self.blacklist_service_url = os.getenv("BLACKLIST_SERVICE_URL", "http://blacklist-service:8001")
    ...
    async def _get_blacklist_data(self) -> List[Dict]:
        ...
        async with httpx.AsyncClient() as client:
            try:
                # 통계 데이터 조회
                stats_response = await client.get(f"{self.blacklist_service_url}/api/v1/statistics")
                stats_response.raise_for_status()
                stats_data = stats_response.json()
                
                # 활성 IP 데이터 조회 (샘플링)
                ips_response = await client.get(f"{self.blacklist_service_url}/api/v1/active-ips?limit=1000")
    ...
    async def generate_trend_analysis(self, period: str = "7d", granularity: str = "daily") -> List[TrendData]:
        """트렌드 분석 생성"""
    ...
    async def generate_comprehensive_report(self, period: str = "30d") -> AnalyticsReport:
        """종합 분석 리포트 생성"""
        # 병렬로 각종 분석 수행
