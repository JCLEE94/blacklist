...
# 신뢰할 수 있는 호스트 설정
app.add_middleware(TrustedHostMiddleware, allowed_hosts=["*"])  # 프로덕션에서는 구체적인 호스트 지정
...
class ServiceDiscovery:
    ...
    async def health_check(self):
        ...
        try:
            ...
            logger.info(
                f"Health check completed: {len(new_healthy_services)}/{len(SERVICES)} services healthy"
            )
