...
class BlacklistManager:
    ...
    async def add_ip_entry(self, entry: IPEntryCreate) -> IPEntryResponse:
        ...
        try:
            # 중복 확인
            existing = self.db.query(BlacklistIP).filter(
                BlacklistIP.ip == entry.ip,
                BlacklistIP.source == entry.source
            ).first()
            ...
            if existing:
                ...
                return IPEntryResponse.from_orm(existing)
            ...
            return IPEntryResponse.from_orm(db_entry)
    ...
    async def search_ip(self, ip: str) -> Optional[IPEntryResponse]:
        ...
        entry = self.db.query(BlacklistIP).filter(
            BlacklistIP.ip == ip,
            BlacklistIP.is_active == True
        ).first()
        
        if entry:
            return IPEntryResponse.from_orm(entry)
        return None
    ...
    async def get_active_ips(self, limit: int = 10000, offset: int = 0) -> List[str]:
        """활성 IP 목록 조회"""
        entries = self.db.query(BlacklistIP.ip).filter(
            BlacklistIP.is_active == True
    ...
    async def get_fortigate_format(self) -> List[FortiGateEntry]:
        ...
        entries = self.db.query(BlacklistIP).filter(
            BlacklistIP.is_active == True
        ).all()
        
        fortigate_entries = []
        for i, entry in enumerate(entries, 1):
            fortigate_entries.append(FortiGateEntry(
    ...
    async def cleanup_expired(self) -> int:
        """만료된 IP 정리"""
        expired_count = self.db.query(BlacklistIP).filter(
    ...
    async def get_statistics(self) -> Dict[str, Any]:
        ...
        active_ips = self.db.query(BlacklistIP).filter(BlacklistIP.is_active == True).count()
