...
class CollectionManager:
    ...
    async def collect_data(self, source: str, start_date: str = None, end_date: str = None) -> CollectionResult:
        """지정된 소스에서 데이터 수집"""
    ...
    async def _send_to_blacklist_service(self, entries: List[IPEntry]):
        ...
        blacklist_service_url = os.getenv("BLACKLIST_SERVICE_URL", "http://blacklist-service:8001")
...
class RegtechCollector:
    ...
    async def collect(self, start_date: str = None, end_date: str = None) -> List[IPEntry]:
        """REGTECH에서 데이터 수집"""
...
async def enable_source(source: str):
    ...
    return {
        "status": "success",
        "message": f"Source {source} enabled"
    }
...
async def disable_source(source: str):
    ...
    return {
        "status": "success",
        "message": f"Source {source} disabled"
    }
