...
def login():
    ...
    try:
        ...
        if username == os.getenv('ADMIN_USERNAME', 'admin'):
            roles = ['admin', 'user']
        elif username in [os.getenv('REGTECH_USERNAME'), os.getenv('SECUDIUM_USERNAME')]:
        ...
        refresh_token = security_manager.generate_jwt_token(
            user_id=username,
            roles=['refresh'],  # 갱신 전용 토큰
            expires_hours=24 * 7  # 7일
        )
...
@input_validation({
    'refresh_token': {'required': True, 'type': str, 'min_length': 10}
})
def refresh_token():
    """JWT 토큰 갱신"""
    try:
