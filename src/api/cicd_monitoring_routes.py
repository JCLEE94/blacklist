...
def verify_api_endpoints():
    ...
    total = len(results)
    avg_response_time = sum(r['response_time_ms'] for r in results if r['status'] == 'success') / max(successful, 1)
...
def get_pipeline_status():
    ...
    try:
        ...
        return jsonify({
            ...
            'overall_status': determine_overall_status(github_status, registry_status, argocd_status),
            'checked_at': datetime.now().isoformat()
        })
    except Exception as e:
...
def verify_blacklist_jclee_me():
    ...
    try:
        ...
        try:
            response = requests.get('https://blacklist.jclee.me', timeout=5, verify=True)
            verification_steps.append({
        ...
        try:
            response = requests.get('https://blacklist.jclee.me/api/collection/status', timeout=5)
        ...
        try:
            response = requests.get('https://blacklist.jclee.me/api/fortigate', timeout=5)
            verification_steps.append({
        ...
        success_count = sum(1 for step in verification_steps if step['status'] == 'success')
        failed_count = sum(1 for step in verification_steps if step['status'] == 'failed')
        warning_count = sum(1 for step in verification_steps if step['status'] == 'warning')
        ...
        return jsonify({
            'error': 'Failed to verify production deployment',
            'details': str(e)
        }), 500
    ...
    for step in steps:
        if step['status'] == 'failed':
            if step['step'] == 'DNS Resolution':
                ...
                recommendations.append("Verify SSL certificate installation and renewal")
            ...
            if step['step'] == 'Redis Cache':
                recommendations.append("Consider checking Redis connection for optimal performance")
...
def trigger_deployment():
    ...
    try:
        ...
        logger.info(f"Deployment triggered: env={environment}, version={version}, strategy={strategy}")
...
def get_deployment_logs(deployment_id):
    ...
    try:
        ...
        logs = [
            {'timestamp': datetime.now().isoformat(), 'level': 'INFO', 'message': 'Deployment pipeline initiated'},
            {'timestamp': datetime.now().isoformat(), 'level': 'SUCCESS', 'message': 'Git commit pushed to main branch'},
            {'timestamp': datetime.now().isoformat(), 'level': 'INFO', 'message': 'GitHub Actions workflow triggered'},
            {'timestamp': datetime.now().isoformat(), 'level': 'INFO', 'message': 'Docker build started'},
            {'timestamp': datetime.now().isoformat(), 'level': 'SUCCESS', 'message': 'Docker image built successfully'},
            {'timestamp': datetime.now().isoformat(), 'level': 'INFO', 'message': 'Running security scans...'},
            {'timestamp': datetime.now().isoformat(), 'level': 'SUCCESS', 'message': 'Security scans passed'},
            {'timestamp': datetime.now().isoformat(), 'level': 'INFO', 'message': 'Pushing to registry...'},
            {'timestamp': datetime.now().isoformat(), 'level': 'SUCCESS', 'message': 'Image pushed to registry'},
            {'timestamp': datetime.now().isoformat(), 'level': 'WARNING', 'message': 'ArgoCD sync pending approval'}
        ]
...
def generate_deployment_report():
    ...
    try:
        ...
        report = {
            ...
            'api_verification': endpoints.json if hasattr(endpoints, 'json') else endpoints,
...
def determine_overall_status(*statuses):
    ...
    if all(s.get('status') in ['success', 'synced', 'available', 'healthy'] for s in statuses):
        return 'healthy'
