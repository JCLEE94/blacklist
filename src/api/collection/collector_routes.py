...
def list_collectors():
    ...
    try:
        ...
        collectors = (
            factory.list_collectors() if hasattr(factory, "list_collectors") else []
        )

        return jsonify(
            {"success": True, "collectors": collectors, "total": len(collectors)}
        )

    except Exception as e:
        logger.error(f"Failed to list collectors: {e}")
        return jsonify({"success": False, "error": "Unable to list collectors"}), 500
...
def enable_collector(collector_name: str):
    ...
    if not input_validation.validate_collector_name(collector_name):
        return jsonify({"success": False, "error": "Invalid collector name"}), 400
    ...
    try:
        ...
        if hasattr(manager, "enable_collector"):
            ...
            return (
                jsonify(
                    {"success": False, "error": "Enable functionality not available"}
                ),
                501,
            )
        ...
        return (
            jsonify(
                {"success": False, "error": f"Unable to enable collector: {str(e)}"}
            ),
            500,
        )
...
def disable_collector(collector_name: str):
    ...
    try:
        ...
        if hasattr(manager, "disable_collector"):
            ...
            return (
                jsonify(
                    {"success": False, "error": "Disable functionality not available"}
                ),
                501,
            )
        ...
        return (
            jsonify(
                {"success": False, "error": f"Unable to disable collector: {str(e)}"}
            ),
            500,
        )
...
def trigger_collector(collector_name: str):
    ...
    try:
        ...
        if hasattr(manager, "trigger_collector"):
            ...
            return (
                jsonify(
                    {"success": False, "error": "Trigger functionality not available"}
                ),
                501,
            )
        ...
        return (
            jsonify(
                {"success": False, "error": f"Unable to trigger collector: {str(e)}"}
            ),
            500,
        )
...
def cancel_collector(collector_name: str):
    ...
    try:
        ...
        if hasattr(manager, "cancel_collector"):
            ...
            return (
                jsonify(
                    {"success": False, "error": "Cancel functionality not available"}
                ),
                501,
            )
        ...
        return (
            jsonify(
                {"success": False, "error": f"Unable to cancel collector: {str(e)}"}
            ),
            500,
        )
    ...
    with app.test_client() as client:
        ...
        try:
            ...
            if response.status_code not in [200, 503]:
                all_validation_failures.append(
                    f"List collectors: Unexpected status code {response.status_code}"
                )
            else:
                print("  - List collectors endpoint responds correctly")
        except Exception as e:
            all_validation_failures.append(
                f"List collectors: Exception occurred - {str(e)}"
            )
        ...
        try:
            ...
            if response.status_code not in [200, 401, 503]:  # 401 for missing auth
                all_validation_failures.append(
                    f"Enable collector: Unexpected status code {response.status_code}"
                )
            else:
                print("  - Enable collector endpoint responds correctly")
        except Exception as e:
            all_validation_failures.append(
                f"Enable collector: Exception occurred - {str(e)}"
            )
