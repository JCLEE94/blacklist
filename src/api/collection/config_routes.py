...
def get_collection_config():
    ...
    try:
        ...
        config = manager.get_config() if hasattr(manager, 'get_config') else {
...
def update_collection_config():
    ...
    try:
        ...
        if hasattr(manager, 'update_config'):
            ...
            return jsonify({
                "success": False,
                "error": "Configuration update not supported"
            }), 501
        ...
        return jsonify({
            "success": False,
            "error": f"Unable to update configuration: {str(e)}"
        }), 500
    ...
    with app.test_client() as client:
        ...
        try:
            ...
            if response.status_code not in [200, 503]:
                all_validation_failures.append(f"Get config: Unexpected status code {response.status_code}")
            else:
        ...
        total_tests += 1
        try:
            response = client.put('/api/collection/config', 
                                json={'auto_collection_enabled': True})
            if response.status_code not in [200, 401, 503]:  # 401 for missing auth
                all_validation_failures.append(f"Update config: Unexpected status code {response.status_code}")
            else:
                print("  - Update config endpoint responds correctly")
        except Exception as e:
            all_validation_failures.append(f"Update config: Exception occurred - {str(e)}")
        ...
        try:
            ...
            if response.status_code not in [200, 401, 503]:  # 401 for missing auth
                all_validation_failures.append(f"Trigger all: Unexpected status code {response.status_code}")
            else:
                print("  - Trigger all endpoint responds correctly")
        except Exception as e:
            all_validation_failures.append(f"Trigger all: Exception occurred - {str(e)}")
