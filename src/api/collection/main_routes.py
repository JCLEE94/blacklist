    ...
    with app.test_client() as client:
        ...
        try:
            ...
            if response.status_code != 200:
                all_validation_failures.append(f"Info endpoint: Expected 200, got {response.status_code}")
            else:
                print("  - Info endpoint responds correctly")
        except Exception as e:
            all_validation_failures.append(f"Info endpoint: Exception occurred - {str(e)}")
        ...
        try:
            ...
            if response.status_code not in [200, 503]:
                all_validation_failures.append(f"Modular status: Unexpected status code {response.status_code}")
            else:
                print("  - Modular status endpoint accessible")
        except Exception as e:
            all_validation_failures.append(f"Modular status: Exception occurred - {str(e)}")
        ...
        try:
            ...
            if response.status_code not in [200, 503]:
                all_validation_failures.append(f"Modular collectors: Unexpected status code {response.status_code}")
            else:
                print("  - Modular collectors endpoint accessible")
        except Exception as e:
            all_validation_failures.append(f"Modular collectors: Exception occurred - {str(e)}")
