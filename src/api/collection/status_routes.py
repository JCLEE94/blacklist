...
def get_collection_status():
    ...
    try:
        ...
        return jsonify({
            "success": False,
            "error": "Unable to retrieve collection status"
        }), 500
...
def get_daily_collection_stats():
    ...
    try:
        ...
        stats = manager.get_daily_stats(start_date, end_date) if hasattr(manager, 'get_daily_stats') else []
        ...
        return jsonify({
            "success": False,
            "error": "Unable to retrieve daily statistics"
        }), 500
...
def get_collection_history():
    ...
    try:
        ...
        history = manager.get_history(limit, offset) if hasattr(manager, 'get_history') else []
        ...
        return jsonify({
            "success": False,
            "error": "Unable to retrieve collection history"
        }), 500
    ...
    with app.test_client() as client:
        ...
        try:
            ...
            if response.status_code not in [200, 503]:
                all_validation_failures.append(f"Status route: Unexpected status code {response.status_code}")
            else:
                print("  - Status endpoint responds correctly")
        except Exception as e:
            all_validation_failures.append(f"Status route: Exception occurred - {str(e)}")
        ...
        try:
            ...
            if response.status_code not in [200, 503]:
                all_validation_failures.append(f"Daily stats route: Unexpected status code {response.status_code}")
            else:
                print("  - Daily stats endpoint responds correctly")
        except Exception as e:
            all_validation_failures.append(f"Daily stats route: Exception occurred - {str(e)}")
        ...
        try:
            ...
            if response.status_code not in [200, 503]:
                all_validation_failures.append(f"History route: Unexpected status code {response.status_code}")
            else:
                print("  - History endpoint responds correctly")
        except Exception as e:
            all_validation_failures.append(f"History route: Exception occurred - {str(e)}")
