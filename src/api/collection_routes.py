...
from ..utils.security import require_auth, input_validation, rate_limit
...
def enable_collector(collector_name: str):
    ...
    try:
        ...
        if success:
            ...
            return jsonify({
                'success': True,
                'message': f'{collector_name} 수집기가 활성화되었습니다'
            })
        else:
            return jsonify({
                'success': False,
                'error': f'{collector_name} 수집기를 찾을 수 없습니다'
            }), 404
...
def disable_collector(collector_name: str):
    ...
    try:
        ...
        if success:
            ...
            return jsonify({
                'success': True,
                'message': f'{collector_name} 수집기가 비활성화되었습니다'
            })
        else:
            return jsonify({
                'success': False,
...
def cancel_collection(collector_name: str):
    ...
    try:
        ...
        if not collector.is_running:
            return jsonify({
                'success': False,
                'error': f'{collector_name} 수집기가 실행 중이 아닙니다'
            }), 400
        ...
        return jsonify({
            'success': True,
            'message': f'{collector_name} 수집 취소가 요청되었습니다'
        })
...
def get_collection_config():
    ...
    try:
        ...
        config_info = {
            ...
            'concurrent_collections': manager.global_config.get('concurrent_collections', 3),
...
def update_collection_config():
    ...
    try:
        ...
        return jsonify({
            ...
            'config': {
                'concurrent_collections': manager.global_config.get('concurrent_collections'),
