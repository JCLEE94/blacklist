...
deployment_health_bp = Blueprint("deployment_health", __name__, url_prefix="/api/deployment")
...
def detailed_deployment_health():
    ...
    try:
        ...
        degraded_services = [status for status in [db_status, redis_status, collection_status] 
        ...
        response_data = {
            ...
            "summary": {
                "total_components": 3,
                "healthy": len([s for s in [db_status, redis_status, collection_status] if s == "healthy"]),
                "degraded": len([s for s in [db_status, redis_status, collection_status] if s in ["degraded", "disabled"]]),
        ...
        return create_error_response(
            ...
            f"Deployment health check failed: {str(e)}",
            500
        )
...
if __name__ == "__main__":
    ...
    try:
        ...
        if db_status in ["healthy", "unhealthy", "error"] and isinstance(db_details, dict):
            ...
            all_validation_failures.append(f"Database health check returned invalid format: {db_status}, {type(db_details)}")
    except Exception as e:
    ...
    try:
        ...
        if redis_status in ["healthy", "degraded", "error"] and isinstance(redis_details, dict):
            ...
            all_validation_failures.append(f"Redis health check returned invalid format: {redis_status}, {type(redis_details)}")
    except Exception as e:
