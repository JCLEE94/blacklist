...
autonomous_monitoring_bp = Blueprint(
    ...
    __name__, 
    url_prefix='/api/monitoring/autonomous'
)
        ...
        return jsonify({
            ...
            "data": {
                "git_changes": {
                    ...
                    "threshold": monitor.thresholds[SystemMetrics.GIT_CHANGES].warning,
                    "status": "warning" if latest_snapshot.git_changes_count > 100 else "normal"
                },
                "test_coverage": {
                    "percentage": latest_snapshot.test_coverage,
                    "target": 95.0,
                    "status": "critical" if latest_snapshot.test_coverage < 30 else "normal"
                },
                "api_performance": {
                    "response_time_ms": latest_snapshot.api_response_time,
                    "baseline": 65.0,
                    "status": "warning" if latest_snapshot.api_response_time > 100 else "normal"
                },
                "system_resources": {
                    "memory_usage": latest_snapshot.memory_usage,
                    "cpu_usage": latest_snapshot.cpu_usage,
                    "status": "warning" if max(latest_snapshot.memory_usage, latest_snapshot.cpu_usage) > 70 else "normal"
                },
                "file_violations": {
                    "count": latest_snapshot.file_violations_count,
                    "max_allowed": 5,
                    "status": "warning" if latest_snapshot.file_violations_count > 5 else "normal"
                },
                "deployment": {
                    "health": latest_snapshot.deployment_health,
                    "status": "healthy" if latest_snapshot.deployment_health == "healthy" else "degraded"
                }
            },
...
def get_metrics_history():
    ...
    try:
        ...
        filtered_history = [
            snapshot for snapshot in monitor.metrics_history
        ...
        return jsonify({
            ...
            "data": {
                ...
                "oldest_record": filtered_history[0].timestamp.isoformat() if filtered_history else None,
                "newest_record": filtered_history[-1].timestamp.isoformat() if filtered_history else None
            },
        ...
        return jsonify({
            ...
            "data": {
                ...
                "statistics": {
                    ...
                    "auto_fix_rate": (auto_fixed / total_alerts * 100) if total_alerts > 0 else 0,
...
def get_monitoring_dashboard():
    ...
    try:
        ...
        # 트렌드 계산 (최근 5개 데이터포인트)
        recent_metrics = monitor.metrics_history[-5:] if len(monitor.metrics_history) >= 5 else monitor.metrics_history
        
        trends = {}
        if len(recent_metrics) >= 2:
            trends = {
                "git_changes": monitor._calculate_trend([m.git_changes_count for m in recent_metrics]),
                "test_coverage": monitor._calculate_trend([m.test_coverage for m in recent_metrics]),
                "api_performance": monitor._calculate_trend([m.api_response_time for m in recent_metrics]),
                "memory_usage": monitor._calculate_trend([m.memory_usage for m in recent_metrics])
            }
        ...
        return jsonify({
            ...
            "data": {
                ...
                "alert_summary": {
                    ...
                    "emergency": len([a for a in recent_alerts if a.level.value == "emergency"]),
                    "critical": len([a for a in recent_alerts if a.level.value == "critical"]),
                    "warning": len([a for a in recent_alerts if a.level.value == "warning"])
                },
                "automation_status": {
                    ...
                    "next_actions": monitor._suggest_next_actions(latest_snapshot, recent_alerts)
                },
                "system_health": {
...
def enable_auto_healing():
    ...
    try:
        ...
        return jsonify({
            "error": "Failed to enable auto-healing",
            "details": str(e)
        }), 500
...
def disable_auto_healing():
    ...
    try:
        ...
        return jsonify({
            "error": "Failed to disable auto-healing",
            "details": str(e)
        }), 500
...
def monitoring_health_check():
    ...
    try:
        ...
        health_status = {
            ...
            "auto_healing_enabled": status["auto_healing_enabled"],
            "recent_alerts": status["recent_alerts_count"],
            "last_update": status["latest_snapshot"]["timestamp"] if status["latest_snapshot"] else None,
            "timestamp": datetime.now().isoformat()
        }
