...
try:
    ...
    from .monitoring.error_routes import (
        get_error_summary,
        clear_old_errors
    )
except ImportError:
...
if __name__ == "__main__":
    ...
    try:
        ...
        if not callable(register_monitoring_routes):
            all_validation_failures.append("Basic test: register_monitoring_routes not callable")
    ...
    try:
        ...
        if len(mock_app.registered_blueprints) < 4:
            all_validation_failures.append(f"Registration test: Expected at least 4 blueprints, got {len(mock_app.registered_blueprints)}")
    except Exception as e:
    ...
    if all_validation_failures:
        ...
        print("File structure optimization: 491-line monitoring_routes.py successfully split into modular structure")
        sys.exit(0)