...
class AutonomousMonitor:
    ...
    async def _collect_system_metrics(self) -> SystemSnapshot:
        ...
        return SystemSnapshot(
            ...
            alerts_count=len([a for a in self.alerts_history if 
                            a.timestamp > timestamp - timedelta(minutes=5)])
        )
    ...
    def _determine_alert_level(self, 
                             metric_type: SystemMetrics, 
                             value: float) -> Optional[AlertLevel]:
        """메트릭 값에 따른 알림 레벨 결정"""
    ...
    def _get_threshold_for_level(self, 
                                metric_type: SystemMetrics, 
                                level: AlertLevel) -> float:
        """알림 레벨에 해당하는 임계값 반환"""
        threshold = self.thresholds.get(metric_type)
        if not threshold:
            return 0.0
    ...
    async def _auto_fix_api_performance(self) -> bool:
        ...
        try:
            # 캐시 정리
            subprocess.run(["find", ".", "-name", "__pycache__", "-exec", "rm", "-rf", "{}", "+"], timeout=10)
    ...
    async def _generate_korean_report(self, 
    ...
    def _calculate_automation_progress(self, snapshot: SystemSnapshot) -> float:
        ...
        performance_progress = max(0, min(100, (200 - snapshot.api_response_time) / 150 * 100))
