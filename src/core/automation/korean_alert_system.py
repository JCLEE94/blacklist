...
class KoreanAlertSystem:
    ...
    def send_progress_update(self,
                           ...
                           progress: float,
                           details: Optional[str] = None,
                           estimated_completion: Optional[datetime] = None) -> str:
        """진행 상황 업데이트 알림"""
    ...
    def send_automation_step_alert(self,
                                 step_number: int,
                                 step_name: str,
                                 status: str,
                                 details: Optional[str] = None,
    ...
    def send_system_status_alert(self,
                               metric_name: str,
                               current_value: float,
                               threshold: float,
                               unit: str = "",
    ...
    def send_git_operation_alert(self,
                               operation: str,
                               files_count: int,
                               status: str,
    ...
    def send_test_coverage_alert(self,
                               current_coverage: float,
                               target_coverage: float,
                               change_delta: Optional[float] = None) -> str:
        """테스트 커버리지 알림"""
        ...
        return self.send_alert(
            ...
            details=details,
            progress_percentage=(current_coverage / target_coverage * 100) if target_coverage > 0 else None
        )
    
    def send_deployment_alert(self,
                            environment: str,
                            version: str,
                            status: str,
                            details: Optional[str] = None) -> str:
    ...
    def get_active_alerts(self, 
                         ...
                         priority: Optional[AlertPriority] = None) -> List[KoreanAlert]:
        """활성 알림 조회"""
    ...
    def get_alert_statistics(self) -> Dict[str, Any]:
        ...
        for priority in AlertPriority:
            priority_stats[priority.value] = len([a for a in self.alert_history if a.priority == priority])
        
        # 카테고리별 통계  
        category_stats = {}
        for category in AlertCategory:
            category_stats[category.value] = len([a for a in self.alert_history if a.category == category])
        
        # 시간대별 통계 (최근 24시간)
        now = datetime.now()
        recent_alerts = [a for a in self.alert_history if now - a.timestamp <= timedelta(hours=24)]
...
def send_info(title: str, message: str, **kwargs) -> str:
    """정보 알림 전송"""
    return get_korean_alert_system().send_alert(title, message, AlertPriority.INFO, **kwargs)

def send_success(title: str, message: str, **kwargs) -> str:
    """성공 알림 전송"""
    return get_korean_alert_system().send_alert(title, message, AlertPriority.SUCCESS, **kwargs)

def send_warning(title: str, message: str, **kwargs) -> str:
    """경고 알림 전송"""
    return get_korean_alert_system().send_alert(title, message, AlertPriority.WARNING, **kwargs)

def send_critical(title: str, message: str, **kwargs) -> str:
    """위험 알림 전송"""
    return get_korean_alert_system().send_alert(title, message, AlertPriority.CRITICAL, **kwargs)

def send_emergency(title: str, message: str, **kwargs) -> str:
    """긴급 알림 전송"""
    return get_korean_alert_system().send_alert(title, message, AlertPriority.EMERGENCY, **kwargs)

def send_step_update(step: int, name: str, status: str, **kwargs) -> str:
    """자동화 단계 업데이트"""
    return get_korean_alert_system().send_automation_step_alert(step, name, status, **kwargs)
