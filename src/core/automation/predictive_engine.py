...
try:
    ...
    from src.core.automation.korean_alert_system import get_korean_alert_system, AlertPriority, AlertCategory
except ImportError:
    def get_logger(name):
...
class AnomalyDetector:
    ...
    def _threshold_based_detection(self, metric_name: str, value: float) -> AnomalyScore:
        """임계값 기반 이상 감지"""
...
class AutomationRiskPredictor:
    ...
    def predict_automation_success(self, 
                                 current_metrics: Dict[str, float],
                                 automation_context: Dict[str, Any]) -> PredictionResult:
        """자동화 성공 확률 예측"""
    ...
    def predict_merge_conflict_risk(self, 
                                  git_changes: int,
                                  file_types: List[str],
                                  change_complexity: Dict[str, int]) -> PredictionResult:
        """머지 충돌 위험 예측"""
        ...
        risky_files = [f for f in file_types if any(risk in f for risk in high_risk_files)]
        ...
        recommendations = [
            ...
            "고위험 파일은 단독 작업 권장",
            "코드 리뷰 강화"
        ] if risk_level != RiskLevel.VERY_LOW else ["현재 상태 유지"]
    ...
    def _count_recent_failures(self) -> int:
        ...
        return len([h for h in self.success_history if 
                   h.get("success", True) == False and 
                   h.get("timestamp", datetime.min) > cutoff_time])
...
class PredictiveEngine:
                ...
                if anomaly.is_anomaly:
                    ...
                    if self.alert_system:
                        self.alert_system.send_system_status_alert(
                            metric_name, value, anomaly.threshold, 
    ...
    def _assess_trend_risk(self, metric_name: str, trend: str, slope: float) -> RiskLevel:
        """트렌드 위험도 평가"""
        # 메트릭별 위험 방향 정의
        bad_trends = {
    ...
    def _generate_system_recommendations(self, 
                                       ...
                                       automation_prediction: PredictionResult) -> List[str]:
        """시스템 전체 추천사항 생성"""
        recommendations = []
    ...
    def get_prediction_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "recent_anomalies": len([a for a in self.anomaly_detector.baseline_windows.values()]),
