    ...
    def get_stats_for_period(self, start_date: str, end_date: str) -> Dict[str, Any]:
        ...
        try:
            with sqlite3.connect(self.db_path, timeout=10) as conn:
                ...
                # Total IPs in period
                cursor.execute(f"SELECT COUNT(DISTINCT ip_address) {base_query}", (start_date, end_date))
                ...
                daily_trend = [{"date": row["detection_date"], "count": row["count"]} for row in cursor.fetchall()]
                ...
                threat_types = [{"type": row["threat_type"], "count": row["count"]} for row in cursor.fetchall()]
    ...
    def get_system_health(self) -> Dict[str, Any]:
        ...
        try:
            with sqlite3.connect(self.db_path, timeout=10) as conn:
                ...
                # Basic counts
                cursor.execute("SELECT COUNT(DISTINCT ip_address) FROM ip_detections WHERE is_active = 1")
                active_ips = cursor.fetchone()[0]
                
                cursor.execute("SELECT COUNT(DISTINCT source) FROM ip_detections WHERE is_active = 1")
            ...
            with sqlite3.connect(self.db_path, timeout=10) as conn:
                ...
                return {
                    ...
                    "expiration_coverage_percent": round(
                        (with_expiration / total_active * 100) if total_active > 0 else 0, 1
                    ),
                    "timestamp": datetime.now().isoformat(),
                }
