...
class CollectionDashboard:
    ...
    def record_daily_collection(self, 
                              collection_date: str,
                              source: str,
                              total_collected: int,
                              new_ips: int = 0,
        ...
        try:
            ...
            if existing:
                ...
                cursor.execute("""
                    ...
                    WHERE collection_date = ? AND source = ?
                """, (total_collected, new_ips, updated_ips, status, 
                     collection_time_ms, error_message, collection_date, source))
            else:
                # 새 기록 삽입
                cursor.execute("""
                ...
                """, (collection_date, source, total_collected, new_ips,
                     updated_ips, status, collection_time_ms, error_message))
        ...
        try:
            ...
            for row in collection_data:
                ...
                if date in calendar_data:
                    ...
                    elif status == 'partial' and calendar_data[date]['status'] != 'success':
                        calendar_data[date]['status'] = 'partial'
                    elif status == 'failed' and calendar_data[date]['status'] == 'missing':
            ...
            successful_days = sum(1 for d in calendar_data.values() if d['status'] == 'success')
            failed_days = sum(1 for d in calendar_data.values() if d['status'] == 'failed')
        ...
        try:
            ...
            return {
                'summary': {
                    ...
                    'missing_days_count': len(missing_days),
                    'success_rate': calendar_data.get('statistics', {}).get('success_rate', 0)
                },
                'calendar': calendar_data,
                'trends': trends,
