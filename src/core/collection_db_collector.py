...
class DatabaseCollectionSystem:
    ...
    def collect_from_source(
        ...
        try:
            if source_name == "regtech":
                result = self._collect_regtech(source_config, credentials, start_date, end_date, result)
            else:
    ...
    def _collect_regtech(
        ...
        try:
            ...
            # 2. 로그인 실행
            login_action_url = f"{base_url}{endpoints.get('login_action', '/login/loginAction')}"
            login_data = {
            ...
            # 3. 데이터 검색
            search_url = f"{base_url}{endpoints.get('search', '/mipam/miPamBoard013.do')}"
            search_data = {
                "pageNum": "1",
                "searchFlag": "1",
