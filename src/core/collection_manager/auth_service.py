...
class AuthService:
        ...
        try:
            with sqlite3.connect(self.db_path, timeout=5) as conn:
                ...
                if failed_attempts >= self.max_auth_attempts:
                    logger.warning(f"Auth limit exceeded for {source}: {failed_attempts}/{self.max_auth_attempts} attempts")
                    return False
                ...
                if source:
                    ...
                    return {
                        ...
                        "summary": {
                            ...
                            "sources_over_limit": sum(1 for s in stats_by_source.values() if not s["within_limit"]),
                        }
                    }
        ...
        try:
            with sqlite3.connect(self.db_path, timeout=5) as conn:
                ...
                if source:
                    ...
                    logger.info(f"Auth attempts reset for {source}: {cleared} records cleared")
                    return {"success": True, "source": source, "records_cleared": cleared}
                else:
                    # 모든 실패 리셋
                    cursor.execute("DELETE FROM auth_attempts WHERE success = 0")
                    cleared = cursor.rowcount
                    
                    logger.info(f"All failed auth attempts reset: {cleared} records cleared")
                    return {"success": True, "source": "all", "records_cleared": cleared}
    ...
    def is_source_blocked(self, source: str) -> Tuple[bool, str]:
        ...
        if not self.check_auth_attempt_limit(source):
            ...
            return True, f"최근 1시간 내 {failed_count}회 인증 실패로 일시 차단 (한도: {self.max_auth_attempts}회)"
    ...
    def cleanup_old_auth_records(self, days: int = 7) -> Dict:
        ...
        try:
            with sqlite3.connect(self.db_path, timeout=5) as conn:
                ...
                logger.info(f"Cleaned up {old_records} auth records older than {days} days")
                return {"success": True, "records_cleaned": old_records, "cutoff_days": days}
