...
class ProtectionService:
    ...
    def __init__(self, db_path: str, config_path: str):
        ...
        # 환경변수 설정
        self.restart_protection_enabled = os.getenv("RESTART_PROTECTION", "true").lower() in (
            "true", "1", "yes", "on"
        )
        self.force_disable_collection = os.getenv("FORCE_DISABLE_COLLECTION", "true").lower() in (
            "true", "1", "yes", "on"
        )
        
        logger.info(f"Protection Service initialized - restart_protection: {self.restart_protection_enabled}, force_disable: {self.force_disable_collection}")
    ...
    def is_collection_safe_to_enable(self) -> Tuple[bool, str]:
        ...
        if recent_failures >= 10:  # 기본 리밋
            return False, f"최근 인증 실패 {recent_failures}회 초과 - 임시 차단"
    ...
    def detect_rapid_restart(self) -> bool:
        ...
        try:
            ...
            recent_restarts = [
                ...
                if (current_time - datetime.fromisoformat(timestamp)).total_seconds() < 600
            ]
            
            if len(recent_restarts) >= 3:  # 10분 내 3회 이상 재시작
                logger.warning(f"Rapid restart detected: {len(recent_restarts)} restarts in 10 minutes")
    ...
    def _update_restart_data(self, count: int):
        ...
        try:
            ...
            restart_data["restart_timestamps"] = [
                ts for ts in restart_data["restart_timestamps"]
    ...
    def create_protection_bypass(self, reason: str, duration_minutes: int = 60) -> Dict:
        ...
        try:
            ...
            logger.warning(f"Protection bypass created: {reason} (expires in {duration_minutes} minutes)")
