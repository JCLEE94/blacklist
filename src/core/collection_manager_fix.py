...
class CollectionManager:
    ...
    def __init__(self, db_path: str = "instance/blacklist.db", 
        ...
        # 일일 자동 수집 설정
        self.daily_collection_enabled = self._get_setting('daily_collection_enabled', False)
    ...
    def _save_collection_config(self):
        ...
        try:
            ...
            # DB에도 저장
            if self.settings_manager:
                self.settings_manager.set_setting('collection_enabled', self.collection_enabled, 'boolean', 'collection')
                self.settings_manager.set_setting('regtech_enabled', self.sources['regtech']['enabled'], 'boolean', 'collection')
                self.settings_manager.set_setting('secudium_enabled', self.sources['secudium']['enabled'], 'boolean', 'collection')
    ...
    def enable_collection(self, sources: Optional[Dict[str, bool]] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            # 소스 상태 업데이트
            self.sources['regtech']['enabled'] = self.config['sources'].get('regtech', False)
