...
class CollectionService:
    ...
    def trigger_regtech(self, params=None):
        ...
        if not self.status['enabled'] or not self.status['sources']['regtech']['enabled']:
            return {
                'success': False,
                'error': 'REGTECH collection is disabled'
            }
        ...
        try:
            ...
            if result.returncode == 0:
                    ...
                    for line in lines:
                        if 'collected' in line.lower():
                            ...
                            if match:
                                ...
                                self.status['sources']['regtech']['total_collected'] += count
                except:
                    pass
                ...
                self.status['sources']['regtech']['last_error'] = result.stderr or 'Unknown error'
    ...
    def trigger_secudium(self, params=None):
        ...
        if not self.status['enabled'] or not self.status['sources']['secudium']['enabled']:
            return {
                'success': False,
                'error': 'SECUDIUM collection is disabled'
            }
        ...
        try:
            ...
            if params:
                ...
                if collection_type == 'blackip' and params.get('start_date') and params.get('end_date'):
                    cmd.extend(['--start-date', params['start_date']])
                    cmd.extend(['--end-date', params['end_date']])
            ...
            if result.returncode == 0:
                ...
                try:
                    if 'SUCCESS' in result.stdout:
                        ...
                        if match:
                            ...
                            self.status['sources']['secudium']['total_collected'] += count
                except:
                    pass
                ...
                self.status['sources']['secudium']['last_error'] = result.stderr or 'Unknown error'
