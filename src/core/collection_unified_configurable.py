...
class ConfigurableCollectionSystem:
    ...
    def __init__(self, config_path: str = "config/collection_config.json"):
        ...
        self.credentials_file = Path(storage_config.get("credentials_file", "instance/credentials.enc"))
        self.collection_history_file = Path(storage_config.get("history_file", "instance/collection_history.json"))
        self.cipher_key_file = Path(storage_config.get("cipher_key_file", "instance/.cipher_key"))
        ...
        logger.info(f"설정 기반 수집 시스템 초기화 완료: {len(self._get_enabled_sources())}개 소스 활성화")
    ...
    def _get_or_create_cipher_key(self) -> bytes:
        ...
        if self.cipher_key_file.exists():
            ...
            # 설정에서 권한 가져오기
            permissions = self.config.get("security", {}).get("file_permissions", "0o600")
            self.cipher_key_file.chmod(int(permissions, 8))
            return key
    
    def save_credentials(self, username: str, password: str, source: str = "regtech") -> bool:
        """자격증명 안전하게 저장"""
        try:
            # 기존 자격증명 로드 (여러 소스 지원)
    ...
    def collect_from_source(
        ...
        if not start_date:
            default_range = self.config.get("collection", {}).get("default_date_range", 7)
            start_date = (datetime.now() - timedelta(days=default_range)).strftime("%Y-%m-%d")
        ...
        try:
            if source_name == "regtech":
                result = self._collect_regtech(source_config, start_date, end_date, result)
            else:
    ...
    def _collect_regtech(
        ...
        try:
            ...
            login_data = {
                ...
                "userPw": creds["password"]
            }
            
            login_action_resp = session.post(login_action_url, data=login_data, timeout=timeout)
            ...
            # 로그인 성공 확인
            if "로그인" in login_action_resp.text or "login" in login_action_resp.text.lower():
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        stats = {
            ...
            "successful_collections": sum(1 for h in self.collection_history if h.get("success")),
            "failed_collections": sum(1 for h in self.collection_history if not h.get("success")),
            "total_ips_collected": sum(h.get("count", 0) for h in self.collection_history if h.get("success")),
            "sources": {},
            "recent_collections": self.collection_history[-10:] if self.collection_history else [],
    ...
    def get_collection_calendar(self, year: int, month: int) -> Dict[str, Any]:
        ...
        for record in self.collection_history:
            if record.get("success"):
                ...
                if collected_date in calendar_data:
                    ...
                    if source and source not in calendar_data[collected_date]["sources"]:
        ...
        return {
            ...
            "summary": {
                ...
                "collected_days": sum(1 for d in calendar_data.values() if d["collected"]),
