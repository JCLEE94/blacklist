...
class CollectionMonitor:
    ...
    def generate_status_report(
        ...
        return {
            ...
            "recommendations": self._generate_recommendations(collectors, collection_history),
        }
    ...
    def _get_collector_status(self, collector: BaseCollector) -> Dict[str, Any]:
        ...
        return {
            ...
            "last_result": (
                collector.current_result.to_dict() 
                if collector.current_result else None
            ),
        }

    def _get_running_collectors(self, collectors: Dict[str, BaseCollector]) -> List[str]:
        """실행 중인 수집기 목록
        ...
        """
        return [
            name for name, collector in collectors.items()
            if collector.is_running
        ]

    def _get_healthy_collectors(self, collectors: Dict[str, BaseCollector]) -> List[str]:
        """건강한 수집기 목록
    ...
    def _assess_system_health(
        ...
        # 최근 성공률 계산
        recent_results = collection_history[-20:] if len(collection_history) >= 20 else collection_history
        if recent_results:
            success_count = sum(
                1 for r in recent_results 
                if r.status == CollectionStatus.COMPLETED
            )
    ...
    def _get_resource_usage(self, collectors: Dict[str, BaseCollector]) -> Dict[str, Any]:
        """리소스 사용량 조회
        ...
        return {
            ...
            "utilization_rate": (running_count / len(collectors) * 100) if collectors else 0,
        }
    ...
    def _generate_recommendations(
        ...
        if collection_history:
            recent_failures = [
                r for r in collection_history[-10:]
        ...
        if collection_history:
            slow_collections = [
                r for r in collection_history[-10:]
                if r.duration and r.duration > self._performance_thresholds["max_duration"]
            ]
            if slow_collections:
    ...
    def add_alert(self, level: str, message: str, collector_name: Optional[str] = None):
        ...
        self.logger.log(
            ...
            f"Alert: {message}" + (f" (collector: {collector_name})" if collector_name else "")
        )
    ...
    def check_collector_health(self, collector: BaseCollector) -> bool:
        ...
        if consecutive_failures >= self._performance_thresholds["max_consecutive_failures"]:
            self.add_alert(
                "warning",
