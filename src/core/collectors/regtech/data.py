...
class DataCollector:
                ...
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        if "excel" in content_type or "spreadsheet" in content_type:
                            ...
                            processor = DataProcessor()
                            ips = processor.parse_excel_response(
                                response.content, endpoint
                            )
                            collected_ips.extend(ips)
                            logger.info(
                                f"Collected {len(ips)} IPs from Excel at {endpoint}"
                            )
                ...
                try:
                    ...
                    if response.status_code == 200:
                        try:
                            ...
                            collected_ips.extend(ips)
                            logger.info(
                                f"Collected {len(ips)} IPs from API at {endpoint}"
                            )
    ...
    def collect_from_web(self, target_urls: List[str] = None) -> List[Dict[str, Any]]:
    ...
    def search_threat_data(self, query: str, limit: int = 100) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for endpoint in search_endpoints:
                try:
                    params = {"q": query, "limit": limit, "type": "ip"}
                    ...
                    if response.status_code == 200:
                        try:
                            ...
                            logger.info(
                                f"Search '{query}' from {endpoint}: {len(data)} results"
                            )
                        except ValueError:
                            # Try parsing as HTML
                            from .processing import DataProcessor
        ...
        for method in methods:
            if not hasattr(collector, method):
                all_validation_failures.append(
                    f"Collection methods: Missing method '{method}'"
                )
