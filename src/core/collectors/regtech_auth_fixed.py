...
class FixedRegtechAuth:
    ...
    def _verify_login_success(self, response: requests.Response) -> bool:
        ...
        for indicator in failure_indicators:
            if indicator in response_text:
                logger.warning(f"Login failure indicator: Found '{indicator}' in response")
                has_failure = True
                break
...
if __name__ == "__main__":
    ...
    try:
        ...
        session = auth.login()
        if not session:
            all_validation_failures.append("Authentication failed - no session returned")
    except Exception as e:
    ...
    try:
        if session:
            ...
            all_validation_failures.append("Cannot test authenticated access - no session")
    except Exception as e:
