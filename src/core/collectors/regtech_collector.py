...
class RegtechCollector(BaseCollector):
    ...
    async def _collect_data(self) -> List[Any]:
        ...
        while session_retry_count < self.session_retry_limit:
            try:
                ...
                collected_ips = await self._robust_collect_ips(session, start_date, end_date)
    ...
    def _verify_login_success(self, response: requests.Response) -> bool:
        ...
        try:
            ...
            if any(indicator in response_text[:1000] for indicator in failure_indicators):
                return False
            
            # 로그인 성공 지표
            success_indicators = ["dashboard", "main", "home", "welcome"]
            if any(indicator in response_text[:1000] for indicator in success_indicators):
                return True
    ...
    async def _robust_collect_ips(
        ...
        while page < max_pages and consecutive_errors < self.max_page_errors:
            try:
                ...
                # 페이지 데이터 수집
                page_ips = await self._collect_single_page(session, page, start_date, end_date)
                ...
                self.logger.info(f"페이지 {page + 1}: {len(page_ips)}개 수집 (총 {len(all_ips)}개)")
                page += 1
    ...
    def _extract_ips_from_soup(self, soup: BeautifulSoup, page: int) -> List[Dict[str, Any]]:
                ...
                if caption and "요주의 IP" in caption.text:
                    ...
                    for row_idx, row in enumerate(rows):
                        try:
                            ...
                            self.logger.warning(f"행 처리 중 오류 (페이지 {page + 1}, 행 {row_idx + 1}): {e}")
                            continue
    ...
    def _is_valid_ip(self, ip: str) -> bool:
        ...
        try:
            ...
            # 사설 IP 및 특수 IP 제외
            excluded_prefixes = [
                "192.168.", "10.", "127.", "172.16.", "172.17.", "172.18.", 
                "172.19.", "172.20.", "172.21.", "172.22.", "172.23.", 
                "172.24.", "172.25.", "172.26.", "172.27.", "172.28.", 
                "172.29.", "172.30.", "172.31.", "169.254.", "224.", "0."
            ]
