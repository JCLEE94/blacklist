...
class FixedRegtechCollector:
    ...
    def _collect_from_board(
        ...
        try:
            ...
            response = self.session.get(
                board_url, params=params, timeout=self.timeout, verify=False
            )
    ...
    def _collect_from_excel(
        ...
        try:
            ...
            response = self.session.get(
                excel_url, params=params, timeout=self.timeout, verify=False
            )
                    ...
                    if response.status_code == 200:
                        ...
                        if "json" in content_type:
                            return self._parse_json_response(
                                response.json(), "REGTECH_API"
                            )
                        else:
                            return self._parse_board_page(
                                response.text, "REGTECH_API_HTML"
                            )
                except Exception:
                    continue
    ...
    def _parse_json_response(
        self, json_data: Dict, source: str
    ) -> List[Dict[str, Any]]:
...
if __name__ == "__main__":
    ...
    try:
        ...
        if not collector.username or not collector.password:
            all_validation_failures.append(
                "Collector initialization failed - missing credentials"
            )
    except Exception as e:
    ...
    try:
        ...
        if not result.get("success"):
            all_validation_failures.append(
                f"Data collection failed: {result.get('error', 'Unknown error')}"
            )
        else:
