                            ...
                            for item in data['list']:
                                if 'ip' in item or 'ipAddress' in item:
                                    ...
                                    collected_ips.append({
                                        "ip": ip,
                                        "source": "REGTECH",
                                        "description": item.get('description', 'Malicious IP'),
                                        "detection_date": datetime.now().strftime('%Y-%m-%d'),
                                        "confidence": "high"
                                    })
                        
                        if collected_ips:
                            logger.info(f"✅ Collected {len(collected_ips)} IPs from API")
                            return {"success": True, "data": collected_ips, "count": len(collected_ips)}
                    except:
                        pass
...
async def collect_from_web_with_excel(self, start_date: str = None, end_date: str = None) -> Dict[str, Any]:
    """Excel 다운로드 방식으로 수집"""
