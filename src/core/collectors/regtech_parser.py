...
class RegtechParser:
    ...
    def parse_excel_file(
        self, file_content: bytes, filename: str = "regtech_data.xlsx"
    ) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            logger.info(
                f"Excel file loaded: {len(df)} rows, columns: {list(df.columns)}"
            )
            ...
            try:
                ...
                for col_name, cell_value in row.items():
                    ...
                    for ip in ip_matches:
                        if self.ip_utils.is_valid_ip(
                            ip
                        ) and not self.ip_utils.is_private_ip(ip):
                            # 날짜 정보 추출 시도
