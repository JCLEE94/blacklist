...
class ConfigLoader:
    ...
    def get_env(
        ...
        if choices and converted_value not in choices:
            raise ValueError(f"'{key}' must be one of {choices}, got '{converted_value}'")
        
        return converted_value
    
    @staticmethod
    def get_int(key: str, default: int = 0, min_val: Optional[int] = None, 
...
class ConfigValidator:
    ...
    def check_env(self, key: str, required: bool = True, 
    ...
    def check_file(self, path: str, required: bool = True,
                   message: Optional[str] = None) -> 'ConfigValidator':
        """파일 존재 확인"""
    ...
    def check_directory(self, path: str, create: bool = True,
                       message: Optional[str] = None) -> 'ConfigValidator':
        """디렉토리 확인 (선택적 생성)"""
