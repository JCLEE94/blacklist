...
def handle_api_errors(func: Callable) -> Callable:
    ...
    def wrapper(*args, **kwargs) -> Union[Response, Any]:
        try:
            ...
            return jsonify({"error": e.message, "error_code": e.error_code}), 401
        except AuthorizationError as e:
            logger.warning(f"Authorization error in {func.__name__}: {e.message}")
            return jsonify({"error": e.message, "error_code": e.error_code}), 403
            ...
            return (
                jsonify(
                    {"error": "Internal storage error", "error_code": "STORAGE_ERROR"}
                ),
                500,
            )
        except ServiceUnavailableError as e:
            logger.error(f"Service unavailable in {func.__name__}: {e.message}")
            return jsonify({"error": e.message, "error_code": e.error_code}), 503
...
def validate_and_convert(
    ...
    try:
        ...
        raise ValidationError(
            message=error_message, field=field, value=data, validation_errors=[str(e)]
        )
