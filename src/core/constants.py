...
def is_valid_ttl(ttl: int) -> bool:
    """TTL 값 유효성 검증"""
    return CONFIG_VALIDATION_RULES["cache_ttl_range"]["min"] <= ttl <= CONFIG_VALIDATION_RULES["cache_ttl_range"]["max"]

def is_valid_port(port: int) -> bool:
    """포트 번호 유효성 검증"""
    return CONFIG_VALIDATION_RULES["port_range"]["min"] <= port <= CONFIG_VALIDATION_RULES["port_range"]["max"]