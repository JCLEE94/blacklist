...
class DataCleaningPipeline:
    ...
    def process_collector_data(self, raw_data: List[Dict[str, Any]], 
    ...
    def _validate_ip_entry(self, entry: Dict[str, Any], source: str) -> Optional[Dict[str, Any]]:
        """
        IP 엔트리 기본 검증
    ...
    def _is_valid_public_ip(self, ip_str: str) -> bool:
        ...
        try:
            ...
            special_ips = {
                ...
                '127.0.0.1', '::1',
                '224.0.0.0', '239.255.255.255'  # 멀티캐스트 범위
            }
    ...
    def _clean_and_normalize(self, entry: Dict[str, Any], source: str) -> Dict[str, Any]:
        """
        데이터 정제 및 정규화
    ...
    def _calculate_risk_score(self, entry: Dict[str, Any]) -> int:
        ...
        # 최근 탐지일수록 높은 점수
        try:
            detection_date = datetime.strptime(entry.get('detection_date', ''), '%Y-%m-%d')
    ...
    def _bulk_save_to_db(self, cleaned_data: List[Dict[str, Any]], 
        ...
        try:
            ...
            result = self.blacklist_manager.bulk_import_ips(
                ips_data=cleaned_data,
                source=f'{source}_CLEANED'
            )
            ...
            if result.get('success', False):
                ...
                logger.info(f"{source} 벌크 저장 완료: {imported}개 저장, {skipped}개 스킵, {errors}개 오류")
                return result
            else:
...
# 편의 함수들
def process_regtech_data(regtech_data: List[Dict[str, Any]], 
                        blacklist_manager=None) -> Dict[str, Any]:
...
def process_secudium_data(secudium_data: List[Dict[str, Any]], 
                         blacklist_manager=None) -> Dict[str, Any]:
    """SECUDIUM 데이터 처리 편의 함수"""
    pipeline = DataCleaningPipeline(blacklist_manager)
    return pipeline.process_collector_data(secudium_data, 'SECUDIUM')

def process_mixed_data(mixed_data: List[Dict[str, Any]], 
                      blacklist_manager=None) -> Dict[str, Any]:
    """혼합 소스 데이터 처리 편의 함수"""
    pipeline = DataCleaningPipeline(blacklist_manager)
