...
class FixedDataStorage:
    ...
    def store_ips(
        self, ip_data: List[Dict[str, Any]], source: str = "REGTECH"
    ) -> Dict[str, Any]:
        """
        ...
        """
        if not ip_data:
            return {"success": True, "imported_count": 0, "message": "No data to store"}
        ...
        try:
            with psycopg2.connect(self.database_url) as conn:
                ...
                logger.info(
                    f"Storage complete: {imported_count} imported, {failed_count} failed"
                )
                return result
    ...
    def _calculate_expiry_date(
        self, detection_date_str, days_to_expire: int = 90
    ) -> str:
...
if __name__ == "__main__":
    ...
    try:
        ...
        if not storage.database_url:
            all_validation_failures.append(
                "Storage initialization failed - no database URL"
            )
    except Exception as e:
    ...
    try:
        ...
        if not result.get("success"):
            all_validation_failures.append(
                f"IP storage test failed: {result.get('error', 'Unknown error')}"
            )
        elif result.get("imported_count") != 2:
            all_validation_failures.append(
                f"Expected 2 IPs imported, got {result.get('imported_count', 0)}"
            )
        else:
    ...
    try:
        with psycopg2.connect(storage.database_url) as conn:
            cursor = conn.cursor()
            cursor.execute(
                "DELETE FROM blacklist_entries WHERE source = 'TEST_REGTECH'"
            )
            conn.commit()
