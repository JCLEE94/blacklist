...
class DatabaseManager:
    ...
    def backup_database(self, backup_path: str = None) -> str:
        """데이터베이스 백업"""
        if not backup_path:
            backup_dir = os.path.join(os.path.dirname(self.database_url.replace('sqlite:///', '')), 'backups')
...
class MigrationManager:
    ...
    def add_migration(self, version: str, up_func: callable, down_func: callable = None):
        """마이그레이션 추가"""
