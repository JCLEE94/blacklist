...
class CollectionSettingsDB:
    ...
    def _init_tables(self):
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            # 인덱스 생성
            conn.execute("CREATE INDEX IF NOT EXISTS idx_collection_history_source ON collection_history(source_name)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_collection_history_date ON collection_history(collected_at)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_collection_credentials_source ON collection_credentials(source_name)")
        ...
        try:
            ...
            with sqlite3.connect(self.db_path) as conn:
                conn.execute("""
                    ...
                    VALUES (?, ?, ?, ?, ?, ?)
                """, (name, display_name, enabled, base_url, config_encrypted, datetime.now()))
                conn.commit()
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                if row:
                    ...
                    if config_encrypted:
                        config_decrypted = self.cipher.decrypt(config_encrypted.encode()).decode()
    ...
    def save_credentials(self, source_name: str, username: str, password: str) -> bool:
        ...
        try:
            ...
            with sqlite3.connect(self.db_path) as conn:
                # 기존 자격증명 삭제
                conn.execute("DELETE FROM collection_credentials WHERE source_name = ?", (source_name,))
                ...
                conn.execute("""
                    ...
                    VALUES (?, ?, ?, ?, ?)
                """, (source_name, username, password_encrypted, datetime.now(), datetime.now()))
                conn.commit()
    ...
    def get_credentials(self, source_name: str) -> Optional[Dict[str, str]]:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                if row:
                    # 패스워드 복호화
                    password = self.cipher.decrypt(row['password_encrypted'].encode()).decode()
                    return {
        ...
        try:
            ...
            with sqlite3.connect(self.db_path) as conn:
                conn.execute("""
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                """, (source_name, start_date, end_date, success, collected_count,
                      error_message, datetime.now(), metadata_json))
                conn.commit()
