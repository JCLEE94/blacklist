...
class EnterpriseBackupManager:
    ...
    async def create_backup(self, backup_type: BackupType, 
                           components: List[str] = None) -> BackupMetadata:
        """백업 생성"""
        ...
        metadata = BackupMetadata(
            ...
            korean_description=self._generate_backup_description(backup_type, components),
        ...
        try:
            ...
            if await self._verify_backup(metadata, compressed_file):
                ...
                logger.info(f"✅ {backup_type.value} 완료: {backup_id} ({self._format_size(metadata.size_bytes)})")
            else:
    ...
    async def _backup_component(self, component: str, backup_dir: Path) -> Tuple[int, str]:
        """개별 컴포넌트 백업"""
    ...
    async def _verify_backup(self, metadata: BackupMetadata, 
        ...
        try:
            ...
            if calculated_checksum != metadata.checksum:
                logger.error(f"❌ 체크섬 불일치: {calculated_checksum} != {metadata.checksum}")
                return False
    ...
    async def restore_from_backup(self, backup_id: str, 
        ...
        try:
            ...
            if recovery_success:
                ...
                await self._send_recovery_notification(recovery_id, True, recovery_level)
            else:
                logger.error(f"❌ {recovery_level.value} 실패: {recovery_id}")
                await self._send_recovery_notification(recovery_id, False, recovery_level)
    ...
    async def _restore_component(self, component: str, backup_dir: Path) -> bool:
        ...
        try:
            # 기존 파일/디렉토리 백업 (안전 조치)
            if dest_path.exists():
                backup_old_path = dest_path.with_suffix(f'.backup_{datetime.now().strftime("%Y%m%d_%H%M%S")}')
    ...
    def _generate_backup_description(self, backup_type: BackupType, 
                                   components: List[str]) -> str:
        """백업 설명 생성"""
        korean_components = [
            self.backup_components[comp]["korean_name"] 
            for comp in components if comp in self.backup_components
        ]
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            for backup in expired_backups:
                ...
                try:
                    ...
                    # 데이터베이스에서 제거
                    cursor.execute("DELETE FROM backup_metadata WHERE backup_id = ?", 
                                 (backup_id,))
    ...
    async def _send_recovery_notification(self, recovery_id: str, success: bool, 
                                        recovery_level: RecoveryLevel):
        """복구 완료 알림 (한국어)"""
