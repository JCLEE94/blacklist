...
class FeatureFlagManager:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                """, (
                    ...
                    flag.status.name, flag.rollout_percentage,
                    json.dumps(flag.target_segments), flag.created_at,
                    flag.updated_at, flag.korean_name,
    ...
    def is_feature_enabled(self, flag_id: str, user_id: str = "anonymous", 
                          segment: str = "default") -> bool:
    ...
    def update_feature_flag(self, flag_id: str, updates: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            with sqlite3.connect(self.db_path) as conn:
                ...
                """, (
                    flag.status.name, flag.rollout_percentage,
                    json.dumps(flag.target_segments), flag.updated_at,
                    flag.korean_name, json.dumps(flag.dependencies),
                    flag_id
                ))
                conn.commit()
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                """, (
                    ...
                    experiment.status.name, experiment.feature_flag_id,
                    json.dumps([asdict(v) for v in experiment.variants]),
                    json.dumps(experiment.success_metrics), experiment.start_date,
                    experiment.end_date, experiment.sample_size,
                    experiment.confidence_level, experiment.minimum_detectable_effect,
                    experiment.korean_name, experiment.korean_description
                ))
                conn.commit()
    ...
    def assign_user_to_experiment(self, user_id: str, experiment_id: str, 
                                 segment: str = "default") -> Optional[str]:
        """사용자를 실험 변형에 할당"""
            ...
            try:
                with sqlite3.connect(self.db_path) as conn:
                    ...
                    cursor.execute("""
                        ...
                        VALUES (?, ?, ?, ?, ?, ?)
                    """, (
                        str(uuid.uuid4()), user_id, experiment_id, 
                        assigned_variant, datetime.now(), segment
                    ))
                    conn.commit()
    ...
    def get_ab_experiment(self, experiment_id: str) -> Optional[ABTestExperiment]:
        ...
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.cursor()
            cursor.execute("SELECT * FROM ab_experiments WHERE experiment_id = ?", (experiment_id,))
            row = cursor.fetchone()
    ...
    def record_metric(self, user_id: str, experiment_id: str, metric_name: str, 
                     value: float, session_id: str = None) -> bool:
        """실험 메트릭 기록"""
        try:
            with sqlite3.connect(self.db_path) as conn:
                cursor = conn.cursor()
                ...
                cursor.execute("""
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                """, (
                    str(uuid.uuid4()), experiment_id, variant_id, user_id,
                    metric_name, value, datetime.now(), session_id
                ))
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            for metric_name in experiment.success_metrics:
                ...
                for variant in experiment.variants:
                    ...
                    # 신뢰구간 계산 (간단한 추정)
                    margin_error = 1.96 * (std_dev / (sample_size ** 0.5)) if std_dev > 0 else 0
                    ...
                    if (control_data and len(control_data) > 1 and 
                        variant_id != control_variant_id):
                        control_mean = statistics.mean(control_data)
                        if control_mean != 0:
                            lift_percentage = ((mean_val - control_mean) / control_mean) * 100
                            # 간단한 유의성 검정
                            if abs(lift_percentage) >= experiment.minimum_detectable_effect:
                                is_significant = True
                                p_value = 0.03  # 가정값
                    
                    # 한국어 요약 생성
                    korean_summary = self._generate_result_summary(
                        variant, metric_name, mean_val, lift_percentage, 
    ...
    def _generate_result_summary(self, variant: ExperimentVariant, metric_name: str, 
                               mean_value: float, lift_percentage: float,
                               is_significant: bool, sample_size: int) -> str:
        """실험 결과 한국어 요약 생성"""
        ...
        return (f"{variant.korean_name}: {metric_korean} 평균 {mean_value:.2f}{unit}, "
    ...
    def get_experiment_dashboard_data(self, experiment_id: str) -> Dict[str, Any]:
        ...
        return {
            "experiment_info": {
                ...
                "end_date": experiment.end_date.strftime("%Y-%m-%d %H:%M") if experiment.end_date else None,
    ...
    def _generate_experiment_summary(self, experiment: ABTestExperiment, 
        ...
        if significant_count == 0:
            return (f"{experiment.korean_name} 실험에서 {total_results}개 메트릭을 분석한 결과, "
                   "통계적으로 유의한 변화를 발견하지 못했습니다.")
        
        best_results = sorted(significant_results, key=lambda x: abs(x.lift_percentage), reverse=True)[:3]
        
        summary_parts = []
        for result in best_results:
            variant_name = next(v.korean_name for v in experiment.variants if v.variant_id == result.variant_id)
...
# 편의 함수들
def is_feature_enabled(flag_id: str, user_id: str = "anonymous", segment: str = "default") -> bool:
    """Feature Flag 활성화 여부 확인 (전역 함수)"""
    return get_feature_flag_manager().is_feature_enabled(flag_id, user_id, segment)


def get_user_experiment_variant(user_id: str, experiment_id: str, segment: str = "default") -> Optional[str]:
    """사용자의 실험 변형 조회 (전역 함수)"""
    return get_feature_flag_manager().assign_user_to_experiment(user_id, experiment_id, segment)
...
if __name__ == "__main__":
    async def main():
        ...
        print(f"New UI Dashboard (admin): {is_feature_enabled('new_ui_dashboard', 'admin_user', 'admin')}")
        print(f"Advanced Analytics (default): {is_feature_enabled('advanced_analytics', 'user_123', 'default')}")
