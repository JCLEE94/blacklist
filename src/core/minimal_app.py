...
def create_minimal_app(config: Dict[str, Any] = None) -> Flask:
    ...
    # 기본 설정 적용
    app.config.update({
        'SECRET_KEY': 'minimal-app-secret-key',
        'TESTING': False,
        'DEBUG': False
    })
...
if __name__ == "__main__":
    ...
    try:
        ...
        if not app.config.get('TESTING') or not app.config.get('DEBUG'):
            all_validation_failures.append("Config application: Configuration not applied")
    except Exception as e:
    ...
    try:
        ...
        if status['mode'] != 'minimal' or 'timestamp' not in status:
            all_validation_failures.append("MinimalBlacklistApp: Invalid status response")
    except Exception as e:
    ...
    try:
        ...
        with app.test_client() as client:
            ...
            if response.status_code != 200:
                all_validation_failures.append("Route registration: Health endpoint not working")
    except Exception as e:
