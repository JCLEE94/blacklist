...
class MonthData:
    ...
    def __post_init__(self):
        ...
        if self.detection_date and not self.expiry_date:
            try:
                from datetime import datetime, timedelta
                detection = datetime.fromisoformat(self.detection_date.replace('Z', '+00:00'))
...
class SystemHealth:
    ...
    def add_service_status(self, service_name: str, status: str, 
    ...
    def calculate_overall_status(self) -> HealthStatus:
        """전체 상태 계산"""
        if self.issues:
            if any('critical' in issue.lower() or 'error' in issue.lower() for issue in self.issues):
