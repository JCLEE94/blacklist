...
class SLAMetric:
    ...
    def is_breached(self) -> bool:
        ...
        if self.name == MonitoringMetric.UPTIME.value:
            ...
            MonitoringMetric.RESPONSE_TIME.value,
...
class ProductionMonitor:
    ...
    async def _measure_api_response_time(self) -> Optional[float]:
        ...
        try:
            ...
            response = requests.get("http://localhost:32542/health", timeout=10)
            end_time = time.time()
    ...
    def _generate_korean_alert(self, metric: SLAMetric) -> str:
        ...
        return messages.get(
            metric.name, f"⚠️ {metric.name} 이상 감지: {metric.value}{metric.unit}"
        )
            ...
            with sqlite3.connect(self.db_path) as conn:
                ...
                analysis = {
                    ...
                    "critical_breaches": sum(
                        row[1] for row in results if row[3] == "HIGH"
                    ),
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                return {
                    "overall_status": (
                        "정상"
                        if all(m["status"] == "정상" for m in latest_metrics.values())
                        else "경고"
                    ),
