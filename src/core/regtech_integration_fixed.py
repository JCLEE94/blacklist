...
class RegtechIntegrationSystem:
    ...
    def run_collection(self, start_date: str = None, end_date: str = None, 
        ...
        try:
            ...
            if clear_existing:
                ...
                logger.info(f"Cleared {clear_result.get('cleared_count', 0)} existing entries")
            ...
            execution_time = (collection_end_time - collection_start_time).total_seconds()
            ...
            logger.info(f"Final result: {stored_count} new IPs, {final_ip_count} total IPs in database")
    ...
    def get_active_ips(self) -> List[str]:
        ...
        try:
            ...
            return [ip_data["ip"].split('/')[0] for ip_data in stored_ips]  # Remove /32 subnet notation
        except Exception as e:
    ...
    def get_system_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "database_connected": total_ips >= 0,  # If we can get count, DB is connected
        ...
        try:
            ...
            try:
                ...
                with psycopg2.connect(self.storage.database_url) as conn:
                    cursor = conn.cursor()
                    cursor.execute("DELETE FROM blacklist_entries WHERE source = 'TEST_E2E'")
                    conn.commit()
            except Exception as e:
...
if __name__ == "__main__":
    ...
    try:
        ...
        if not integration.collector or not integration.storage:
            all_validation_failures.append("System initialization failed - missing components")
    except Exception as e:
    ...
    try:
        ...
        if not e2e_result.get("success"):
            all_validation_failures.append(f"End-to-end test failed: {e2e_result.get('error')}")
        else:
    ...
    try:
        ...
        if os.getenv('REGTECH_USERNAME') and os.getenv('REGTECH_PASSWORD'):
            ...
            if not collection_result.get("success"):
                print(f"Collection test returned no data (expected): {collection_result.get('error', 'No error')}")
            else:
                print(f"Collection test completed: {collection_result.get('stored_count', 0)} IPs")
        else:
