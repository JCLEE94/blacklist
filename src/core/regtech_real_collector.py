...
class RealRegtechCollector:
    ...
    def collect_real_data(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"Login successful, cookie: {self.session.cookies.get('regtech-front')[:20]}...")
            ...
            logger.info(f"Content-Type: {excel_resp.headers.get('Content-Type', 'Unknown')}")
            logger.info(f"Content-Length: {excel_resp.headers.get('Content-Length', 'Unknown')}")
            ...
            if 'excel' not in content_type.lower() and 'spreadsheet' not in content_type.lower():
                ...
                if len(excel_resp.content) < 1000:
                    logger.warning(f"Response too small ({len(excel_resp.content)} bytes), might be error page")
            ...
            try:
                    ...
                    for idx, row in df.iterrows():
                        ...
                        if ip and '.' in ip:
                            ...
                            ip_data = {
                                "ip": ip,
                                "source": "REGTECH",
                                "description": row.get('설명', row.get('Description', 'Malicious IP from REGTECH')),
                ...
                try:
                    ...
                    if '<html' in text.lower():
                        logger.warning("Response is HTML, not Excel. May need different approach.")
            ...
            if collected_ips:
                logger.info(f"Successfully collected {len(collected_ips)} real IPs from REGTECH")
                return {
                    "success": True,
                    "data": collected_ips,
        ...
        if result.get("success"):
            ...
            if result.get('data'):
                ...
                for i, ip_data in enumerate(result['data'][:5], 1):
                    print(f"   {i}. {ip_data['ip']} - {ip_data.get('description', 'N/A')}")
        else:
