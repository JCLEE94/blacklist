...
def get_missing_days():
    ...
    try:
        ...
        return jsonify(
            {
                "success": True,
                "data": {"missing_days": missing_days, "count": len(missing_days)},
            }
        )
    except Exception as e:
...
def collect_specific_date():
    ...
    try:
        ...
        if not target_date:
            return jsonify({"success": False, "error": "날짜가 필요합니다"}), 400
        ...
        if success:
            ...
            return jsonify({"success": False, "error": "수집 시작에 실패했습니다"}), 500
