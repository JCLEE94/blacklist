...
unified_collection_bp = Blueprint("unified_collection", __name__, url_prefix="/api/collection")
...
def get_collection_status():
    ...
    try:
        ...
        status = {
            "enabled": collection_mgr.is_collection_enabled() if collection_mgr else False,
            "sources": {
                "regtech": {
        ...
        for src in sources_to_trigger:
            ...
            try:
                ...
                if not collection_mgr:
                    results["failed"].append(src)
                    results["messages"].append(f"{src}: Collection manager not available")
                    continue
                ...
                if success:
                    ...
                    results["messages"].append(f"{src}: Collection triggered successfully")
                else:
                    results["failed"].append(src)
