...
class CollectionScheduler:
        ...
        try:
            ...
            if source in self.collection_callbacks:
                ...
                if result and result.get("success"):
                    ...
                    logger.info(
                        f"Scheduled collection successful for {source}: {result.get('stored_count', 0)} IPs"
                    )
                else:
                    schedule["failure_count"] += 1
                    logger.error(
                        f"Scheduled collection failed for {source}: {result.get('error', 'Unknown error')}"
                    )
            else:
    ...
    def get_schedule_status(self) -> Dict:
        """스케줄 상태 조회"""
        status = {"is_running": self.is_running, "schedules": {}}
    ...
    def update_schedule(
        self, source: str, enabled: bool = None, interval_hours: int = None
    ):
        """스케줄 설정 업데이트"""
        ...
        logger.info(
            f"Schedule updated for {source}: enabled={schedule['enabled']}, interval={schedule['interval_hours']}h"
        )
        return True
    ...
    def trigger_immediate_collection(self, source: str) -> bool:
        ...
        collection_thread = threading.Thread(
            target=self._execute_collection, args=(source,), daemon=True
        )
