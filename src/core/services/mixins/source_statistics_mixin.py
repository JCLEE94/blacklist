...
class SourceStatisticsMixin:
    ...
    def get_source_summary(self) -> Dict[str, Any]:
        ...
        try:
            ...
            total_ips_all_sources = sum(stats["total_ips"] for stats in source_stats.values())
            total_active_all_sources = sum(stats["active_ips"] for stats in source_stats.values())
            
            # 가장 활성 소스 찾기
            most_active_source = max(source_stats.items(), 
            ...
            for source, stats in source_stats.items():
                if total_active_all_sources > 0:
                    contribution = (stats["active_ips"] / total_active_all_sources) * 100
            ...
            return {
                ...
                } if most_active_source[0] else None,
                "source_contributions": source_activity,
                "source_count_by_activity": {
                    "high_activity": len([s for s, stats in source_stats.items() 
                                        if stats["active_percentage"] > 80]),
                    "medium_activity": len([s for s, stats in source_stats.items() 
                                          if 20 <= stats["active_percentage"] <= 80]),
                    "low_activity": len([s for s, stats in source_stats.items() 
        ...
        try:
            ...
            comparison = {
                ...
                "comparison": {
                    ...
                    "activity_rate_diff": stats_a["active_percentage"] - stats_b["active_percentage"],
                    "larger_source": source_a if stats_a["total_ips"] > stats_b["total_ips"] else source_b,
                    "more_active_source": source_a if stats_a["active_ips"] > stats_b["active_ips"] else source_b
                }
            }
...
if __name__ == "__main__":
    ...
    try:
        ...
        mixin_methods = [method for method in dir(SourceStatisticsMixin) 
