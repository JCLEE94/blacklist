...
class ProductionSLAEnforcer:
        ...
        while True:
            try:
                ...
                if value is not None:
                    ...
                    # 위반 여부 확인
                    is_breach = severity not in [SLASeverity.NORMAL, SLASeverity.WARNING]
                    ...
                    measurement = SLAMeasurement(
                        ...
                        is_breach=is_breach,
                        breach_duration=0,
                        korean_status=self._generate_status_message(metric_type, value, severity)
                    )
    ...
    def _evaluate_severity(self, metric_type: SLAMetricType, value: float) -> SLASeverity:
        """심각도 평가"""
        target = self.sla_targets[metric_type]
        
        # 가동시간과 가용성은 높을수록 좋음
        if metric_type in [SLAMetricType.UPTIME, SLAMetricType.AVAILABILITY, SLAMetricType.THROUGHPUT]:
    ...
    def _generate_status_message(self, metric_type: SLAMetricType, 
                                value: float, severity: SLASeverity) -> str:
        """상태 메시지 생성"""
        target = self.sla_targets[metric_type]
    ...
    async def _handle_sla_recovery(self, metric_type: SLAMetricType):
        ...
        for breach_id, breach in self.active_breaches.items():
            if (breach.metric_type == metric_type and 
                not breach.resolved):
                ...
                logger.info(f"✅ SLA 복구: {breach.korean_description} ({breach.duration_seconds}초)")
    ...
    def _determine_enforcement_actions(self, breach: SLABreach) -> List[SLAEnforcementAction]:
        """강제 집행 액션 결정"""
        ...
        if breach.metric_type == SLAMetricType.RESPONSE_TIME:
            ...
            actions.extend([SLAEnforcementAction.CIRCUIT_BREAK, SLAEnforcementAction.ROLLBACK])
        elif breach.metric_type == SLAMetricType.UPTIME:
            actions.append(SLAEnforcementAction.FAILOVER)
        
        return list(set(actions))  # 중복 제거
    
    async def _execute_action(self, action: SLAEnforcementAction, breach: SLABreach) -> bool:
        """개별 강제 집행 액션 실행"""
        try:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                """, (
                    ...
                    json.dumps([action.name for action in breach.enforcement_actions]),
                    breach.resolved,
    ...
    async def _log_enforcement_action(self, breach_id: str, action: SLAEnforcementAction, 
                                    success: bool):
        """강제집행 액션 로그"""
        try:
            with sqlite3.connect(self.db_path) as conn:
    ...
    async def _generate_daily_sla_report(self) -> Dict[str, Any]:
        ...
        for metric_type_name, avg_value, total_measurements, breach_count in daily_stats:
            metric_type = SLAMetricType[metric_type_name]
            target = self.sla_targets[metric_type]
            
            # SLA 준수율 계산
            if total_measurements > 0:
                compliance_rate = ((total_measurements - breach_count) / total_measurements) * 100
            else:
        ...
        if compliance_scores:
            report_data["overall_sla_compliance"] = round(statistics.mean(compliance_scores), 1)
            ...
            try:
                ...
                for breach in list(self.active_breaches.values()):
                    if not breach.resolved:
                        ...
                        # 5분 이상 지속되는 심각한 위반
                        if duration > 300 and breach.severity in [SLASeverity.CRITICAL, SLASeverity.EMERGENCY]:
                            logger.critical(f"🚨 장기간 SLA 위반: {breach.korean_description} ({duration/60:.1f}분)")
                            
                            # 추가 복구 조치
                            if SLAEnforcementAction.FAILOVER not in breach.enforcement_actions:
                                await self._execute_action(SLAEnforcementAction.FAILOVER, breach)
