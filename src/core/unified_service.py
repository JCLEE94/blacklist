...
class UnifiedBlacklistService:
                ...
                if self.collection_manager and hasattr(
                    ...
                    if self.collection_manager.daily_collection_enabled:
                        ...
                        if not last_collection or not last_collection.startswith(
                            ...
                            # REGTECH 수집 (3개월 단위)
                            result = await self._collect_regtech_data_with_date(
                                start_date, end_date
                            )

                            if result.get("success"):
                                self.logger.info(
                                    f"✅ 3개월 자동 수집 완료: {result.get('total_collected', 0)}개 IP"
                                )

                                # 마지막 수집 시간 업데이트
                                self.collection_manager.last_daily_collection = (
                                    datetime.now().isoformat()
                                )
                                self.collection_manager.config[
                                    "last_daily_collection"
                                ] = self.collection_manager.last_daily_collection
    ...
    def clear_all_database_data(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.blacklist_manager:
                ...
                return {"success": True, "message": "All database data cleared"}
            else:
    ...
    def set_daily_collection_config(
        self, enabled: bool, strategy: str, collection_days: int
    ) -> Dict[str, Any]:
        """일일 수집 설정"""
        try:
        ...
        try:
            ...
            if not self.blacklist_manager:
                ...
                try:
                    ...
                    self.logger.error(f"Failed to reinitialize blacklist_manager: {e}")
                    return {
                        "status": "unhealthy",
                        "total_ips": 0,
                        "active_ips": 0,
    ...
    def _load_regtech_ips_from_files(self) -> List[Dict[str, Any]]:
        ...
        try:
            # 가장 최근 파일 찾기
            regtech_files = glob.glob("data/regtech/regtech_simple_*.json")
            if not regtech_files:
                self.logger.warning("No REGTECH data files found")
                return []

            # 가장 최근 파일 선택
            latest_file = max(regtech_files, key=lambda x: os.path.getmtime(x))
            self.logger.info(f"Loading REGTECH data from: {latest_file}")
            ...
            # IP 데이터 추출
            ips = data.get("data", [])
            self.logger.info(f"Loaded {len(ips)} IPs from REGTECH file")

            return ips

        except Exception as e:
            self.logger.error(f"Error loading REGTECH files: {e}")
            return []

    def add_collection_log(
        self, source: str, action: str, details: Dict[str, Any] = None
    ):
        """수집 로그 추가 - 메모리와 데이터베이스에 저장"""
        current_time = datetime.now()

        # 상세한 메시지 생성
        detailed_message = self._create_detailed_log_message(
            source, action, details, current_time
        )
    ...
    def _create_detailed_log_message(
        self, source: str, action: str, details: Dict[str, Any], timestamp: datetime
    ) -> str:
    ...
    def get_collection_logs(self, limit: int = 100) -> list:
        ...
        try:
            ...
            sorted_logs = sorted(
                unique_logs.values(), key=lambda x: x["timestamp"], reverse=True
            )
    ...
    def get_blacklist_paginated(
        self, page: int = 1, per_page: int = 100, source_filter: str = None
    ) -> Dict[str, Any]:
        """페이징된 블랙리스트 반환"""
        try:
        ...
        for i in range(days):
            ...
            if self.blacklist_manager:
                try:
                    ...
                    self.logger.warning(
                        f"Failed to get daily stats for {date_str}: {e}"
                    )
                ...
                if log.get("timestamp", "").startswith(date_str):
                    if "collection_completed" in log.get("action", ""):
                        ...
                        if details.get("ip_count", 0) > 0:
                            daily_stat["total_ips"] = max(
                                daily_stat["total_ips"], details["ip_count"]
                            )
        ...
        try:
            ...
            if include_metadata:
                ...
                for row in paginated_results:
                    ...
                    self.logger.info(
                        f"[DEBUG] Enhanced API: Processing IP {ip} with source {source}"
                    )

                    # 위협 수준 결정 (소스별로 다르게 설정)
                    threat_level = (
                        "high" if source in ["REGTECH", "SECUDIUM"] else "medium"
                    )
    ...
    def get_monthly_stats(self, start_date: str, end_date: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.blacklist_manager and hasattr(
                self.blacklist_manager, "get_stats_for_period"
            ):
    ...
    def cleanup_old_data(self, cutoff_date: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.blacklist_manager and hasattr(
                self.blacklist_manager, "cleanup_old_data"
            ):
    ...
    def clear_all_data(self) -> Dict[str, Any]:
        ...
        try:
            ...
            with sqlite3.connect(db_path) as conn:
                ...
                for table in tables_to_clear:
                    try:
                        ...
                        self.logger.info(
                            f"Cleared {table_counts.get(table, 0)} records from {table}"
                        )
            ...
            try:
                ...
                self.logger.info(
                    f"✅ 데이터베이스 완전 클리어 완료: {total_deleted}개 레코드 삭제, ID 시퀀스 리셋"
                )
    ...
    def set_daily_collection_config(
        self, enabled: bool, strategy: str = None, collection_days: int = 3
    ) -> Dict[str, Any]:
        """일일 수집 설정 저장"""
        try:
            # 설정을 파일이나 데이터베이스에 저장
            ...
            # 수집 로그 추가
            action = (
                "daily_collection_enabled" if enabled else "daily_collection_disabled"
            )
            self.add_collection_log(
                "SYSTEM",
                action,
    ...
    def get_collection_intervals(self) -> dict:
        ...
        try:
            ...
            config_path = os.path.join(
                "/app" if os.path.exists("/app") else ".",
                "instance/collection_intervals.json",
            )
            ...
            return {"regtech_days": 90, "secudium_days": 3, "error": str(e)}

    def update_collection_intervals(
        self, regtech_days: int, secudium_days: int
    ) -> dict:
        """수집 간격 설정 업데이트"""
        try:
            config = {
            ...
            self.logger.info(
                f"Collection intervals updated: REGTECH={regtech_days}days, SECUDIUM={secudium_days}days"
            )
    ...
    def enable_collection(self) -> dict:
        ...
        try:
            ...
            return {"success": False, "error": str(e), "message": "수집 활성화 실패"}
    ...
    def disable_collection(self) -> dict:
        ...
        try:
            ...
            return {"success": False, "error": str(e), "message": "수집 비활성화 실패"}
    ...
    def trigger_regtech_collection(
        ...
        try:
            ...
            if not regtech_collector:
                ...
                if progress_tracker:
                    progress_tracker.fail_collection(
                        "regtech", "REGTECH collector not available"
                    )
                return {
                    "success": False,
            ...
            # 실제 수집 실행 (동기적으로 처리)
            try:
                result = regtech_collector.collect_from_web(
                    start_date=start_date, end_date=end_date
                )

                # collect_from_web이 딕셔너리를 반환하는 경우 처리
                if isinstance(result, dict):
                    if not result.get("success", False):
                        error_msg = result.get("error", "Unknown error")
                        self.logger.error(f"REGTECH collection failed: {error_msg}")
                        if progress_tracker:
                            progress_tracker.fail_collection("regtech", error_msg)
                        return {
                            "success": False,
                            "error": error_msg,
                            "message": f"REGTECH 수집 실패: {error_msg}",
                        }
                    # 파일에서 IP 리스트 로드
                    ips = self._load_regtech_ips_from_files()
                else:
                    # 이전 버전과의 호환성 - 리스트를 반환하는 경우
                    ips = result

                self.logger.info(
                    f"REGTECH collection completed: {len(ips) if ips else 0} IPs collected"
                )
                ...
                if ips and self.blacklist_manager:
                    try:
                        ...
                        for ip_entry in ips:
                            ...
                            if hasattr(ip_entry, "ip_address"):
                                ...
                                ip_addr = ip_entry.get("ip_address") or ip_entry.get(
                                    "ip"
                                )
                            ...
                            ips_data.append(
                                {
                                    ...
                                    "threat_level": getattr(
                                        ip_entry, "threat_level", None
                                    ),
                                    "as_name": getattr(ip_entry, "as_name", None),
                                    "city": getattr(ip_entry, "city", None),
                                    "reg_date": getattr(ip_entry, "reg_date", None),
                                    "attack_type": getattr(
                                        ip_entry, "reason", None
                                    ),  # reason을 attack_type으로도 저장
                                    "detection_date": getattr(
                                        ip_entry, "reg_date", None
                                    ),  # reg_date를 detection_date로도 저장
                                }
                            )

                        self.logger.info(
                            f"REGTECH: Calling bulk_import_ips with {len(ips_data)} IPs"
                        )
                        self.logger.info(
                            f"REGTECH: Sample IP data: {ips_data[:3] if ips_data else 'None'}"
                        )
                        ...
                        self.logger.info(
                            f"REGTECH: blacklist_manager type: {type(self.blacklist_manager)}"
                        )
                        result = self.blacklist_manager.bulk_import_ips(
                            ips_data, source="REGTECH"
                        )
                        ...
                        if result.get("success"):
                            self.logger.info(
                                f"REGTECH: {result['imported_count']}개 IP가 데이터베이스에 저장됨"
                            )
                            ...
                            if progress_tracker:
                                try:
                                    self.logger.info(
                                        f"DEBUG: About to call complete_collection with tracker: {type(progress_tracker)}"
                                    )
                                    progress_tracker.complete_collection(
                                        "regtech",
                                        f"REGTECH 수집 완료: {result['imported_count']}개 IP 저장됨",
                                    )
                                    self.logger.info(
                                        f"DEBUG: complete_collection completed successfully"
                                    )
                                except Exception as complete_error:
                                    self.logger.error(
                                        f"DEBUG: Error in complete_collection: {complete_error}"
                                    )
                                    import traceback

                                    complete_tb = traceback.format_exc()
                                    self.logger.error(
                                        f"DEBUG: complete_collection traceback: {complete_tb}"
                                    )
                                    raise
                            ...
                            try:
                                ...
                                self.logger.info(
                                    f"REGTECH: 저장 후 DB 상태: {source_counts}"
                                )
                            ...
                            try:
                                ...
                                self.logger.info(
                                    f"DEBUG: Return value created successfully: {return_value}"
                                )
                                return return_value
                            except Exception as return_error:
                                self.logger.error(
                                    f"DEBUG: Error creating return value: {return_error}"
                                )
                                import traceback

                                return_tb = traceback.format_exc()
                                self.logger.error(
                                    f"DEBUG: Return creation traceback: {return_tb}"
                                )
                                raise
                        else:
                            self.logger.error(
                                f"REGTECH: 데이터베이스 저장 실패 - {result.get('error')}"
                            )
                            if progress_tracker:
                                progress_tracker.fail_collection(
                    ...
                    if progress_tracker:
                        progress_tracker.complete_collection(
                            "regtech",
                            f"REGTECH 수집 완료: {len(ips) if ips else 0}개 IP (저장 안 함)",
                        )
                    return {
                        "success": True,
    ...
    def trigger_secudium_collection(self) -> dict:
        ...
        try:
            ...
            if not secudium_collector:
                return {"success": False, "error": "SECUDIUM collector not available"}

            # 수집 시작 로그
            self.add_collection_log(
                "secudium",
                "collection_start",
                {"triggered_by": "manual", "start_time": datetime.now().isoformat()},
            )
            ...
            try:
                ...
                if entries:  # 리스트가 비어있지 않은 경우
                    ...
                    if self.blacklist_manager:
                        try:
                            ...
                            self.logger.info(
                                f"SECUDIUM: Calling bulk_import_ips with {len(ips_data)} IPs"
                            )
                            result = self.blacklist_manager.bulk_import_ips(
                                ips_data, source="SECUDIUM"
                            )
                            if result.get("success"):
                                self.logger.info(
                                    f"SECUDIUM: {result['imported_count']}개 IP가 데이터베이스에 저장됨"
                                )
                            else:
                                self.logger.error(
                                    f"SECUDIUM: 데이터베이스 저장 실패 - {result.get('error')}"
                                )
                        except Exception as e:
                            self.logger.error(
                                f"SECUDIUM: Error saving to database - {e}"
                            )
                            import traceback

                            self.logger.error(
                                f"SECUDIUM: Traceback - {traceback.format_exc()}"
                            )
                    ...
                    self.add_collection_log(
                        "secudium",
                        "collection_failed",
                        {"error": "No IPs collected", "triggered_by": "manual"},
                    )
                ...
                self.add_collection_log(
                    "secudium",
                    "collection_failed",
                    {"error": str(e), "triggered_by": "manual"},
                )
                raise e
    ...
    try:
        ...
        try:
            ...
            if isinstance(health, dict):
                print("  ✅ 시스템 헬스 체크 성공")
                print(f"    💻 총 IP: {health.get('total_ips', 0)}")
