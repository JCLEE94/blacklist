...
def get_trend_analysis():
    ...
    try:
        ...
        if len(daily_data) > 1:
            ...
            previous_avg = sum(d['unique_ips'] for d in daily_data[-14:-7]) / 7 if len(daily_data) >= 14 else recent_avg
        ...
        result = {
            "trend_analysis": {
                ...
                "recent_7day_avg": round(recent_avg, 1) if 'recent_avg' in locals() else 0,
                "previous_7day_avg": round(previous_avg, 1) if 'previous_avg' in locals() else 0,
            },
    ...
    try:
        ...
        result = {
            "source_analysis": {
                ...
                "active_sources": len([s for s in source_details if s["status"] == "active"]),
                "total_ips": stats.get("total_ips", 0),
            },
            "sources": sorted(source_details, key=lambda x: x["ip_count"], reverse=True),
            "period": {
...
def get_geo_analysis():
    ...
    try:
        ...
        # 대륙별 그룹화 (간단한 버전)
        continent_mapping = {
            "KR": "Asia", "CN": "Asia", "JP": "Asia", "IN": "Asia",
