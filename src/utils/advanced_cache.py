...
class EnhancedSmartCache:
    ...
    def __init__(self, redis_url: str = None, redis_client=None, default_ttl=300, 
                 max_memory_entries=10000, enable_compression=True, 
                    ...
                    try:
                        ...
                        if tags:
                            for tag in tags:
                                ...
                                self.redis_client.expire(f"tag:{tag}", ttl + 60)  # Tag TTL slightly longer
        ...
        with self._lock:
            ...
            if self.redis_latency:
                redis_metrics = {
                    ...
                    'p95_latency': sorted(self.redis_latency)[int(len(self.redis_latency) * 0.95)] if len(self.redis_latency) > 20 else max(self.redis_latency) if self.redis_latency else 0
                }
            ...
            return {
                ...
                **self.cache_stats,
                ...
                # Performance metrics
                'avg_operation_time': round(avg_operation_time * 1000, 2),  # Convert to ms
        ...
        try:
            ...
            if isinstance(value, (dict, list, str, int, float, bool, type(None))):
                if HAS_ORJSON:
                    ...
                    json_data = json.dumps(value, ensure_ascii=False, separators=(',', ':')).encode('utf-8')
                ...
                # Compress if enabled and data is large enough
                if self.enable_compression and len(pickle_data) > self.compression_threshold:
                    self.cache_stats['compressions'] += 1
    ...
    def _update_memory_cache(self, key: str, value: Any, ttl: int, tags: List[str] = None):
    ...
    def tag_set(self, key: str, value: Any, tags: List[str], ttl: Optional[int] = None) -> bool:
    ...
    async def async_set(self, key: str, value: Any, ttl: Optional[int] = None, tags: List[str] = None) -> bool:
    ...
    def get_performance_metrics(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            if self.operation_times:
                ...
                metrics.update({
                    'avg_operation_time_ms': (sum(sorted_times) / len(sorted_times)) * 1000,
                    'p95_operation_time_ms': sorted_times[int(len(sorted_times) * 0.95)] * 1000,
                    'p99_operation_time_ms': sorted_times[int(len(sorted_times) * 0.99)] * 1000
                })
            ...
            if self.max_memory_entries > 0:
                metrics['memory_efficiency'] = len(self.memory_cache) / self.max_memory_entries
...
def get_smart_cache(redis_url: str = None, redis_client=None, default_ttl=300, 
                   max_memory_entries=10000, enable_compression=True):
...
def enhanced_cached(ttl: int = 300, key_prefix: str = "", tags: List[str] = None, 
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            args_hash = hashlib.md5(str(args[1:] if args and hasattr(args[0], 'cache') else args).encode()).hexdigest()
