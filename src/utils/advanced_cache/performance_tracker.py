...
class PerformanceTracker:
    ...
    def get_average_operation_time(self, operation: str = None) -> float:
        ...
        with self.lock:
            if operation:
                operation_times = [
                    op['time'] for op in self.operation_times 
    ...
    def get_error_rate(self, operation: str = None) -> float:
        ...
        with self.lock:
            if operation:
                ...
                return (total_errors / self.total_operations * 100) if self.total_operations > 0 else 0.0
    ...
    def get_performance_summary(self) -> Dict[str, Any]:
        ...
        with self.lock:
            ...
            return {
                ...
                "operations_per_second": self.total_operations / uptime_seconds if uptime_seconds > 0 else 0,
    ...
    def get_trend_analysis(self, window_minutes: int = 30) -> Dict[str, Any]:
        ...
        with self.lock:
            ...
            # Recent hit rates
            recent_hits = [
                hr for hr in self.hit_rates 
                if hr['timestamp'] >= cutoff_time
            ]
            ...
            if len(self.memory_usage_history) >= 10:
                recent_memory = [
                    mem for mem in self.memory_usage_history 
            ...
            return {
                ...
                "recent_average_response_time_ms": (
                    sum(op['time'] for op in recent_ops if op['success']) / len(recent_ops) * 1000
                    if recent_ops else 0
                ),
                "recent_error_count": len([op for op in recent_ops if not op['success']]),
