...
class AsyncProcessor:
    ...
    def __init__(self, config: ProcessingConfig = None):
        ...
        ) if self.config.enable_thread_pool else None
        
        # 프로세스 풀 (CPU 집약적 작업용)
        self.process_pool = ProcessPoolExecutor(
            max_workers=self.config.max_process_workers
        ) if self.config.enable_process_pool else None
        ...
        logger.info(f"AsyncProcessor initialized with {self.config.max_workers} thread workers, "
    ...
    async def submit_async_task(self, 
        ...
        try:
            if use_process_pool and self.process_pool:
                ...
                return self._create_task_result(task_id, True, result, started_at=started_at)
            ...
            return self._create_task_result(task_id, False, error=str(e), started_at=started_at)
    ...
    def _create_task_result(self, task_id: str, success: bool, result=None, error=None, started_at=None):
        """작업 결과 객체 생성"""
        now = datetime.now()
        return TaskResult(
            task_id=task_id,
    ...
    async def batch_process(self, 
                           ...
                           items: List[Any], 
                           batch_size: int = None,
                           use_process_pool: bool = False) -> List[TaskResult]:
        """배치 처리 (병렬 실행)"""
    ...
    def get_performance_stats(self) -> Dict[str, Any]:
        ...
        if self.task_stats["total_completed"] > 0:
            successful_tasks = self.task_stats["total_completed"] - self.task_stats["total_failed"]
    ...
    def cleanup_completed_tasks(self, keep_recent: int = 100):
        ...
        logger.info(f"Cleaned up old completed tasks, keeping {len(self.completed_tasks)} recent tasks")
...
if __name__ == "__main__":
    ...
    async def test_async_processor():
        ...
        try:
            ...
            batch_results = await processor.batch_process(batch_task, test_items, batch_size=3)
