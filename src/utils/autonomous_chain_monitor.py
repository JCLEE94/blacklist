...
class AutonomousChainMonitor:
    ...
    def __init__(self, db_path: str = "instance/chain_monitor.db"):
        ...
        self.logger.info("자율적 체인 모니터링 시스템 초기화 완료", 
    ...
    def register_chain(self, chain_id: str, chain_name: str, task_id: str, 
                      dependencies: List[str] = None, priority: ChainPriority = ChainPriority.NORMAL) -> ChainExecutionContext:
        """체인 등록"""
        ...
        self.logger.info(f"체인 등록: {chain_name}", 
                        chain_id=chain_id, task_id=task_id, 
                        korean_message=korean_msg)
        
        return context
    
    def start_chain_execution(self, chain_id: str, execution_func: Callable, *args, **kwargs):
        """체인 실행 시작"""
        ...
        # 비동기 실행
        future = self.executor.submit(self._execute_chain_with_monitoring, 
                                    context, execution_func, *args, **kwargs)
        
        return future
    
    def _execute_chain_with_monitoring(self, context: ChainExecutionContext, 
                                     execution_func: Callable, *args, **kwargs):
        """모니터링과 함께 체인 실행"""
        ...
        try:
            ...
            # 자동 복구 시도
            if self.auto_recovery_enabled and context.metrics.retry_count < context.metrics.max_retries:
    ...
    def _is_chain_completed_successfully(self, chain_id: str) -> bool:
        ...
        for completed_chain in self.completed_chains:
            if completed_chain.chain_id == chain_id and completed_chain.status == ChainStatus.SUCCESS:
                return True
        return False
    ...
    def _complete_chain(self, context: ChainExecutionContext):
        ...
        self.system_metrics["average_success_rate"] = (successful / total) * 100 if total > 0 else 0
    ...
    def _check_active_chains(self):
        ...
        for chain_id, context in list(self.active_chains.items()):
            # 타임아웃 체크 (30분)
            if context.metrics.start_time and (current_time - context.metrics.start_time).total_seconds() > 1800:
    ...
    def _attempt_chain_recovery(self):
        """실패한 체인 복구 시도"""
        for chain_id, context in list(self.active_chains.items()):
            if context.status == ChainStatus.FAILED and context.metrics.retry_count < context.metrics.max_retries:
                if chain_id in self.failure_callbacks:
                    try:
                        recovery_func = self.failure_callbacks[chain_id]
                        recovery_func(context)
                    except Exception as e:
                        self.logger.error(f"체인 복구 실패: {e}", chain_id=chain_id, exception=e)
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                """, (
                    ...
                    context.metrics.start_time.isoformat() if context.metrics.start_time else None,
                    context.metrics.end_time.isoformat() if context.metrics.end_time else None,
                    context.metrics.duration_seconds,
                    context.metrics.success_rate,
                    context.metrics.retry_count,
                    context.error_message,
                    context.korean_status_message,
                    json.dumps(asdict(context.metrics), default=str, ensure_ascii=False)
                ))
    ...
    def get_korean_progress_report(self) -> str:
        ...
        if self.completed_chains:
            ...
            for context in recent_completed:
                ...
                duration = f" ({context.metrics.duration_seconds:.1f}초)" if context.metrics.duration_seconds else ""
                report.append(f"  {status_emoji} {context.chain_name}{duration}")
                report.append(f"     {context.korean_status_message}")
                report.append("")
        
        report.append(f"🕐 마지막 업데이트: {self.system_metrics['last_update'].strftime('%Y-%m-%d %H:%M:%S')}")
...
# 편의 함수들
def register_chain(chain_name: str, task_id: str, dependencies: List[str] = None) -> str:
    """체인 등록 편의 함수"""
    import uuid
