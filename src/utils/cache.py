...
class CacheManager:
    ...
    def __init__(self, redis_url: Optional[str] = None, default_ttl: int = 300, max_memory_items: int = 10000):
    ...
    def warm_cache(self, warm_functions: List[Callable] = None) -> Dict[str, Any]:
        ...
        logger.info(f"Cache warming completed: {results['warmed_keys']} keys in {results['duration']:.2f}s")
    ...
    def get_cache_stats(self) -> Dict[str, Any]:
        ...
        stats = {
            ...
            'max_memory_items': self.max_memory_items if self.cache_type == "memory" else "unlimited"
        }
    ...
    def optimize_cache(self) -> Dict[str, Any]:
        ...
        if self.cache_type == "memory":
            ...
            hit_rate = self.stats['hits'] / max(self.stats['total_requests'], 1)
            if hit_rate < 0.7:
                optimization_results['recommendations'].append("캐시 히트율이 낮음 - TTL 증가 또는 캐시 워밍 고려")
            
            if self.stats['evictions'] > self.stats['sets'] * 0.1:
                optimization_results['recommendations'].append("캐시 제거 빈도가 높음 - max_memory_items 증가 고려")
            ...
            try:
                ...
                if max_memory > 0 and memory_usage / max_memory > 0.8:
                    optimization_results['recommendations'].append("Redis 메모리 사용률이 높음 - 메모리 확장 고려")
                
                # 키 만료 정책 확인
                eviction_policy = info.get('maxmemory_policy', 'noeviction')
                if eviction_policy == 'noeviction':
                    optimization_results['recommendations'].append("Redis eviction 정책을 'allkeys-lru'로 변경 권장")
...
def cached(cache_manager: CacheManager, ttl: int = 300, key_prefix: str = ""):
    ...
    def decorator(func: Callable) -> Callable:
        ...
        wrapper.cache_clear = lambda: cache_manager.delete(f"{key_prefix}:{func.__name__}:*")
        return wrapper
...
def get_cache() -> CacheManager:
    ...
    if _cache_instance is None:
        ...
        try:
            ...
            logger.warning(f"Failed to create cache with Redis, using in-memory cache: {e}")
