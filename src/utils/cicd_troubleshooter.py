...
class CICDTroubleshooter:
    ...
    def __init__(self, gateway_url="http://192.168.50.120:5678", api_key="super-secret-ai-key"):
        ...
        self.error_patterns = {
            ...
            "registry_auth": {
                "patterns": ["unauthorized: authentication required", "docker login", "registry auth"],
                "fix_method": self.fix_registry_auth,
                "severity": "high"
            },
            "ssh_connection": {
                "patterns": ["ssh: connect to host", "Connection refused", "ssh timeout"],
            ...
            "network": {
                "patterns": ["network unreachable", "DNS resolution", "network timeout"],
                "fix_method": self.fix_network_issues,
                "severity": "medium"
            }
        }
    
    def monitor_and_fix_pipeline(self, project_id: str, pipeline_id: str) -> Dict[str, Any]:
        """파이프라인 모니터링 및 자동 수정"""
    ...
    def get_failed_jobs(self, project_id: str, pipeline_id: str) -> List[Dict[str, Any]]:
        """실패한 작업 목록 조회"""
        try:
            response = self.session.get(
    ...
    def set_cicd_variable(self, project_id: str, key: str, value: str) -> bool:
        ...
        try:
            ...
            response = self.session.post(
                f"{self.base_url}/api/v1/projects/{project_id}/variables",
                json=data
            )
            return response.status_code in [200, 201]
        except:
