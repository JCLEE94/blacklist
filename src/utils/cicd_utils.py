...
class CICDUtils:
    ...
    def create_issue(self, project_id: str, title: str, description: str) -> bool:
        ...
        try:
            ...
            response = self.session.post(
                f"{self.base_url}/api/v1/issues", json=data
            )
            if response.status_code in [200, 201]:
    ...
    def send_notification(self, project_id: str, message: str, channel: str = "general") -> bool:
        """알림 발송"""
        try:
            data = {
                "project_id": project_id,
...
YAML 문법 검증"""
    ...
    def analyze_log_patterns(self, log_content: str) -> Dict[str, Any]:
        ...
        patterns = {
            ...
            "permission_issues": len(re.findall(r'(?i)permission denied|access denied', log_content)),
            "network_issues": len(re.findall(r'(?i)network|dns|connection', log_content)),
        }
