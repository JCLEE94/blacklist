...
class PerformanceProfiler:
        ...
        try:
            ...
            with self._lock:
                ...
                if len(self.function_timings[operation_name]) > 100:
                    self.function_timings[operation_name] = self.function_timings[operation_name][-100:]
    ...
    def get_performance_summary(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            summary = {
                ...
                'memory': {
                    'avg_usage': sum(m.memory_after for m in recent_metrics) / len(recent_metrics),
                    'max_usage': max(m.memory_after for m in recent_metrics),
                    'avg_growth': sum(m.memory_after - m.memory_before for m in recent_metrics) / len(recent_metrics)
                },
                'function_timings': {
                    name: {
                        'count': len(timings),
    ...
    def _generate_recommendations(self) -> List[str]:
        ...
        if self.cache_stats['hit_rate'] < 0.7:
            recommendations.append(f"캐시 히트율이 낮습니다 ({self.cache_stats['hit_rate']:.2%}). TTL 조정 또는 캐시 워밍 고려")
        ...
        # 메모리 사용량 분석
        if self.metrics:
            avg_memory_growth = sum(m.memory_after - m.memory_before for m in self.metrics) / len(self.metrics)
    ...
    async def measure_async(self, operation_name: str, coro, details: Dict = None):
        ...
        try:
            ...
            loop.run_in_executor(
                self.executor,
                self._record_metric,
                operation_name, duration, details
            )
    
    def _record_metric(self, operation_name: str, duration: float, details: Dict = None):
        """메트릭 기록 (스레드 풀에서 실행)"""
        process = psutil.Process()
...
class ResponseOptimizer:
    ...
    @staticmethod
    def fast_json_response(data: Any, status_code: int = 200, headers: Dict = None) -> tuple:
        """빠른 JSON 응답 생성"""
        if HAS_ORJSON:
    ...
    @staticmethod
    def optimize_cache_headers(max_age: int = 300, etag: str = None, 
                             last_modified: datetime = None) -> Dict[str, str]:
        """캐시 헤더 최적화"""
        headers = {
...
def benchmark_api_endpoints(base_url: str = 'http://localhost:2541', iterations: int = 100) -> Dict[str, Any]:
    """API 엔드포인트 벤치마크"""
    import requests
    import statistics
