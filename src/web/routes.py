    ...
    try:
        ...
        try:
            ...
            if db_path.exists():
                ...
                cursor.execute(
                    "SELECT COUNT(*) FROM blacklist_ip WHERE UPPER(source) = 'REGTECH'"
                )
                regtech_count = cursor.fetchone()[0]

                cursor.execute(
                    "SELECT COUNT(*) FROM blacklist_ip WHERE UPPER(source) = 'SECUDIUM'"
                )
                ...
                cursor.execute(
                    "SELECT DISTINCT source, COUNT(*) FROM blacklist_ip GROUP BY source HAVING COUNT(*) > 0"
                )
        ...
        context = {
            ...
            "uptime_hours": round(
                (time.time() - health.get("start_time", time.time())) / 3600, 1
            ),
...
def data_management():
    ...
    return render_template(
        "data_management.html", monthly_data=monthly_data, build_time=get_build_time()
    )
                    ...
                    for daily_file in daily_files:
                        ...
                        try:
                            ...
                            if daily_details_file.exists():
                                with open(
                                    daily_details_file, "r", encoding="utf-8"
                                ) as f:
                            ...
                            month_info["daily_files"].append(
                                {
                                    ...
                                    "ip_count": daily_details.get(
                                        "total_ips", daily_ip_count
                                    ),
                                    "total_detections": daily_details.get(
                                        "total_detections", daily_ip_count
                                    ),
                    ...
                    if total_ips > 0:
                        ...
                        for day in range(1, last_day + 1):
                            if day == last_day:
                                ...
                                daily_count = min(
                                    daily_count, remaining - (last_day - day)
                                )
...
def get_daily_ips(month, date):
    ...
    try:
        ...
        result = {"date": date, "month": month, "day": int(date.split("-")[2])}
...
def api_secudium_test():
    ...
    try:
        ...
        username = settings.secudium_username or os.environ.get(
            "SECUDIUM_USERNAME", "nextrade"
        )
        password = settings.secudium_password or os.environ.get(
            "SECUDIUM_PASSWORD", "default_password"
        )
        ...
        if success:
            return jsonify(
                {"success": True, "message": "SECUDIUM connection successful"}
            )
        else:
...
def api_secudium_logs():
    ...
    try:
        ...
        if os.path.exists(log_file):
            ...
            # 마지막 20개 로그만 반환
            return jsonify({"logs": [log.strip() for log in logs[-20:]]})
    ...
    try:
        ...
        if os.path.exists(pid_file):
            ...
            try:
                ...
                os.remove(pid_file)
                return jsonify({"success": True, "message": "Process already stopped"})

        return jsonify({"success": True, "message": "No active collection"})
...
def api_daily_stats():
    ...
    try:
        ...
        for item in daily_data:
            item["percentage"] = (
                round((item["detection_count"] / total_count) * 100, 1)
                if total_count > 0
                else 0
            )
...
def realtime_feed():
    ...
    try:
        ...
        return jsonify(
            {"success": True, "event": event, "timestamp": datetime.now().isoformat()}
        )
