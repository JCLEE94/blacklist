...
class RealtimeDashboard {
    ...
    startRealtimeUpdates() {
        ...
        // 시스템 모니터링 업데이트 (10초마다)
        this.updateIntervals.set('monitoring', setInterval(() => {
            this.updateSystemMonitoring();
        }, this.options.updateInterval * 2));
        
        // 수집 상태 업데이트 (15초마다)
        this.updateIntervals.set('collection', setInterval(() => {
    ...
    updateProgressBar(type, percentage) {
        const progressBar = document.querySelector(`.progress-bar.bg-${this.getProgressBarClass(type)}`);
        const percentageElement = progressBar?.parentElement?.previousElementSibling?.lastElementChild;
    ...
    async updateCollectionStatus() {
        try {
            const response = await fetch(`${this.options.apiBase}/realtime/collection-status`);
            if (!response.ok) throw new Error(`HTTP ${response.status}`);
    ...
    updateCollectionInfo(collections) {
        ...
        collections.forEach(collection => {
            console.log(`${collection.name}: ${collection.status} (${collection.progress}%)`);
        });
    }
    ...
    async updateMonthlyChart() {
        ...
        try {
            const response = await fetch(`${this.options.apiBase}/stats/detection-trends?days=90`);
     ...
     */
    createActivityFeed() {
        const systemCard = document.querySelector('.col-lg-6:last-child .modern-card .card-body');
