...
class StatisticsDashboard {
    constructor(options = {}) {
        this.options = {
            ...
            updateInterval: options.updateInterval || 30000, // 30초
            ...options
        };
        ...
        this.charts.detectionTrend = new Chart(ctx, {
            ...
            data: {
                ...
                datasets: [{
                    ...
                    backgroundColor: this.hexToRgba(this.options.chartColors.primary, 0.1),
                    borderWidth: 2,
                    fill: true,
                    tension: 0.4
                }, {
                    ...
                    backgroundColor: this.hexToRgba(this.options.chartColors.danger, 0.1),
                    borderWidth: 2,
                    fill: true,
                    tension: 0.4
                }]
        ...
        this.charts.hourlyActivity = new Chart(ctx, {
            ...
            data: {
                ...
                datasets: [{
                    ...
                    borderColor: this.options.chartColors.primary,
                    backgroundColor: this.hexToRgba(this.options.chartColors.primary, 0.2),
                    borderWidth: 2,
    ...
    async updateSourceDistribution() {
        try {
            const response = await fetch(`/api/v2/sources/distribution?${this.buildQueryString()}`);
            const data = await response.json();
    ...
    async updateIPTypeDistribution() {
        try {
            const response = await fetch(`/api/v2/analytics/ip-types?${this.buildQueryString()}`);
            const data = await response.json();
    ...
    async updateGeographicDistribution() {
        try {
            const response = await fetch(`/api/v2/analytics/geographic?${this.buildQueryString()}`);
            const data = await response.json();
    ...
    async updateHourlyActivity() {
        try {
            const response = await fetch(`/api/v2/analytics/hourly-activity?${this.buildQueryString()}`);
            const data = await response.json();
    ...
    async updateThreatLevels() {
        try {
            const response = await fetch(`/api/v2/analytics/threat-levels?${this.buildQueryString()}`);
            const data = await response.json();
    ...
    async exportStatistics() {
        try {
            const response = await fetch(`/api/v2/analytics/export?${this.buildQueryString()}`);
...
document.addEventListener('DOMContentLoaded', function() {
    // 통계 페이지에서만 초기화
    if (window.location.pathname === '/statistics' || document.getElementById('statisticsContainer')) {
