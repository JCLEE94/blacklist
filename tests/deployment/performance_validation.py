...
class PerformanceValidator:
    ...
    def test_endpoint_performance(self, endpoint: str, description: str, iterations: int = 5) -> Dict[str, Any]:
    ...
    def concurrent_load_test(self, endpoint: str, concurrent_users: int, duration_seconds: int) -> Dict[str, Any]:
        """Perform concurrent load test"""
        print(f"   Running concurrent load test - {concurrent_users} users for {duration_seconds}s...")
    ...
    def monitor_resource_usage(self, duration_seconds: int = 60) -> Dict[str, Any]:
        ...
        while time.time() - start_time < duration_seconds:
            try:
                sample = {
                    ...
                    "network": psutil.net_io_counters()._asdict() if hasattr(psutil, 'net_io_counters') else {}
                }
        ...
        memory_ok = resource_stats["memory"]["max"] <= self.thresholds["max_memory_percent"]
    ...
    def validate_deployment_performance(self, concurrent_users: int = 5, test_duration: int = 30) -> Dict[str, Any]:
        ...
        load_stats = self.concurrent_load_test("/health", concurrent_users, test_duration)
    ...
    def calculate_performance_score(self) -> float:
        ...
        if "success_rate" in self.results["load_test"]:
            ...
            throughput_score = min(load_throughput / self.thresholds["min_throughput_rps"], 2) * 12.5  # 0-25 points
        ...
        if "cpu" in self.results["resource_usage"]:
            ...
            cpu_score = max(0, 15 - (cpu_usage / self.thresholds["max_cpu_percent"]) * 15)  # 0-15 points
            memory_score = max(0, 10 - (memory_usage / self.thresholds["max_memory_percent"]) * 10)  # 0-10 points
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Performance validation for blacklist deployment")
    parser.add_argument("--base-url", default="http://localhost:32542", help="Base URL for testing")
...
if __name__ == "__main__":
    ...
    try:
        ...
        if hasattr(validator, 'base_url') and hasattr(validator, 'thresholds'):
            ...
            all_validation_failures.append("PerformanceValidator missing required attributes")
    except Exception as e:
        all_validation_failures.append(f"PerformanceValidator instantiation failed: {e}")
    ...
    try:
        ...
        if isinstance(result, dict) and "response_time_ms" in result and "success" in result:
            print("✅ Response time measurement function working")
        else:
            all_validation_failures.append(f"Response time measurement returned invalid format: {type(result)}")
    except Exception as e:
        all_validation_failures.append(f"Response time measurement function failed: {e}")
    ...
    try:
        ...
        if isinstance(score, (int, float)) and 0 <= score <= 100:
            ...
            all_validation_failures.append(f"Performance score calculation returned invalid value: {score}")
    except Exception as e:
    ...
    if all_validation_failures:
        ...
        print("Performance validation module is validated and ready for deployment testing")
