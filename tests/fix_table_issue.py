...
def create_ip_detections_table():
    ...
    # Create indexes
    cursor.execute(
        "CREATE INDEX IF NOT EXISTS idx_ip_detections_ip ON ip_detections(ip_address)"
    )
    cursor.execute(
        "CREATE INDEX IF NOT EXISTS idx_ip_detections_source ON ip_detections(source)"
    )
    cursor.execute(
        "CREATE INDEX IF NOT EXISTS idx_ip_detections_active ON ip_detections(is_active)"
    )
    cursor.execute(
        "CREATE INDEX IF NOT EXISTS idx_ip_detections_date ON ip_detections(detection_date)"
    )
            ...
            if data.get("success"):
                ...
                if entries:
                    ...
                    for entry in entries[:5]:
                        print(
                            f"  {entry.get('ip_address')} | {entry.get('source')} | {entry.get('detection_type')}"
                        )
