...
def generate_report(results):
    ...
    for result in results:
        module_name = result['module'].replace('test_', '').replace('_integration.py', '')
        status = f"{Colors.GREEN}PASS{Colors.RESET}" if result['failed'] == 0 else f"{Colors.RED}FAIL{Colors.RESET}"
        
        print(f"{module_name:<40} {status:<19} "
...
def main():
    ...
    for i, module in enumerate(test_modules, 1):
        ...
        # 간단한 결과 표시
        if result['failed'] == 0:
            print(f"{Colors.GREEN}✅ PASSED{Colors.RESET} ({result['passed']} tests in {result['elapsed_time']:.2f}s)")
        else:
            print(f"{Colors.RED}❌ FAILED{Colors.RESET} ({result['failed']} failures in {result['elapsed_time']:.2f}s)")
