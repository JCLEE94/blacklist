...
""")
    ...
    def test_security_scanning_detects_hardcoded_secrets(self, setup_test_env):
        ...
        # grep 기반 간단한 보안 스캔
        result = subprocess.run(
            ["grep", "-E", r"(password|secret|key|token)\s*=\s*['\"][^'\"]*['\"]", str(test_file)],
            capture_output=True,
            text=True
        )
...
class TestDeploymentStage:
    ...
    def test_image_update_detection(self):
        ...
        assert "registry.jclee.me/blacklist:latest" in annotations["argocd-image-updater.argoproj.io/image-list"]
