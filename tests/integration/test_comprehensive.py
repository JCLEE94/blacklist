...
class IntegrationTester:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if data.get('success', False):
                    ...
                    self.log_test("수집 시스템 활성화", True, f"활성화 성공 ({cleared_items}개 항목 클리어)")
                else:
                    self.log_test("수집 시스템 활성화", False, data.get('message', 'Unknown error'))
            else:
    ...
    def test_regtech_collection(self):
        ...
        try:
            ...
            if response.status_code in [200, 400]:  # 400도 정상 응답으로 처리 (실패 메시지 포함)
                data = response.json()
        ...
        for endpoint, method, description in endpoints:
            try:
                ...
                if response.status_code == 200:
                    try:
                        data = response.json()
                        self.log_test(f"API 엔드포인트: {description}", True, f"응답 정상 ({len(str(data))} bytes)")
                    except:
                        # JSON이 아닌 응답 (예: 텍스트)
                        self.log_test(f"API 엔드포인트: {description}", True, f"응답 정상 ({len(response.text)} bytes)")
                else:
                    self.log_test(f"API 엔드포인트: {description}", False, f"HTTP {response.status_code}")
