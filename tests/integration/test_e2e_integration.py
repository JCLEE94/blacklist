        ...
        try:
            ...
            with app.test_client() as client:
                ...
                if blacklist_manager:
                    # Mock IPs 추가
                    mock_ips = [
                        {'ip': '192.168.1.100', 'source': 'REGTECH', 'threat_type': 'C&C'},
                        {'ip': '10.0.0.50', 'source': 'REGTECH', 'threat_type': 'Malware'},
                        {'ip': '172.16.0.10', 'source': 'PUBLIC', 'threat_type': 'Phishing'}
                    ]
                ...
                assert len(fortigate_data['blacklist']) >= len(mock_ips) if blacklist_manager else True
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        try:
            ...
            with app.test_client() as client:
                ...
                print(f"✅ Sustained load - Avg: {avg_sustained:.2f}ms, Max: {max_sustained:.2f}ms")
                
                # 성능 기준
                assert avg_sustained < 50, f"Average response time too high: {avg_sustained:.2f}ms"
                assert max_sustained < 200, f"Max response time too high: {max_sustained:.2f}ms"
...
def test_multi_source_integration():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        try:
            # Mock 수집기 설정
            with patch('src.core.regtech_simple_collector.RegtechSimpleCollector') as mock_regtech:
                # Mock 수집 데이터
                mock_regtech_instance = Mock()
                mock_regtech_instance.collect_ips.return_value = [
                    {'ip': '192.168.1.1', 'source': 'REGTECH', 'threat_type': 'C&C'},
                    {'ip': '192.168.1.2', 'source': 'REGTECH', 'threat_type': 'Malware'}
                ]
                ...
                with app.test_client() as client:
                    ...
                    print(f"✅ Collection enabled: {status.get('collection_enabled', False)}")
