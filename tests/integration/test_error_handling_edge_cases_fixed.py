...
class TestErrorHandlingIntegration:
    ...
    def test_database_connection_failure(
        self, mock_container_system, mock_database_connection
    ):
        """데이터베이스 연결 실패 처리 테스트"""
        # 데이터베이스 연결 실패 시뮬레이션
        mock_service = mock_container_system["unified_service"]
        mock_service.get_system_health.side_effect = Exception(
    ...
    def test_external_api_timeout(self, mock_external_services):
        ...
        mock_external_services["regtech_post"].side_effect = TimeoutError(
            "Request timed out"
        )
