...
class TestAPIPerformance:
    ...
    def test_health_endpoint_performance(self, performance_tester):
        ...
        result = performance_tester.load_test_endpoint('/health', concurrent_requests=5, total_requests=50)
        
        # Performance assertions
        assert result['success_rate'] >= 99.0, f"Health endpoint success rate too low: {result['success_rate']}%"
        assert result['avg_response_time'] < 0.1, f"Health endpoint too slow: {result['avg_response_time']}s"
        assert result['p95_response_time'] < 0.2, f"Health endpoint P95 too slow: {result['p95_response_time']}s"
    ...
    def test_blacklist_api_performance(self, performance_tester):
        ...
        result = performance_tester.load_test_endpoint('/api/blacklist/active', concurrent_requests=10, total_requests=100)
        
        # Performance assertions
        assert result['success_rate'] >= 95.0, f"Blacklist API success rate too low: {result['success_rate']}%"
        assert result['avg_response_time'] < 0.5, f"Blacklist API too slow: {result['avg_response_time']}s"
        assert result['p99_response_time'] < 2.0, f"Blacklist API P99 too slow: {result['p99_response_time']}s"
    ...
    def test_collection_status_performance(self, performance_tester):
        ...
        result = performance_tester.load_test_endpoint('/api/collection/status', concurrent_requests=5, total_requests=50)
        
        # Performance assertions
        assert result['success_rate'] >= 90.0, f"Collection status success rate too low: {result['success_rate']}%"
        assert result['avg_response_time'] < 1.0, f"Collection status too slow: {result['avg_response_time']}s"
...
class TestDatabasePerformance:
    ...
    def test_bulk_ip_operations(self, performance_tester, enhanced_mock_container):
        ...
        assert result['duration'] < 10.0, f"Bulk operation too slow: {result['duration']:.1f}s"
...
class TestCachePerformance:
    ...
    def test_cache_operations_performance(self, performance_tester, enhanced_mock_container):
        ...
        # Performance assertions
        assert set_result['success'], f"Cache set operations failed: {set_result['error']}"
        assert get_result['success'], f"Cache get operations failed: {get_result['error']}"
        
        set_ops_per_second = len(test_keys) / set_result['duration']
        get_ops_per_second = len(test_keys) / get_result['duration']
        
        print(f"\n🗄️  Cache Performance:")
        print(f"   Set Operations: {len(test_keys)} in {set_result['duration']:.1f}s ({set_ops_per_second:.0f} ops/sec)")
        print(f"   Get Operations: {len(test_keys)} in {get_result['duration']:.1f}s ({get_ops_per_second:.0f} ops/sec)")
...
def test_comprehensive_performance_benchmark(performance_tester):
    ...
    assert overall_avg_response < 1.0, f"Overall response time too slow: {overall_avg_response:.3f}s"
