...
def test_secudium_disabled(base_url):
    ...
    try:
        response = requests.post(f"{base_url}/api/collection/secudium/trigger", timeout=10)
...
def test_cookie_configuration():
    ...
    try:
        ...
        print_test("Cookie Configuration", False, "Cannot import RegtechCollector (run from project root)")
        return False
    except Exception as e:
