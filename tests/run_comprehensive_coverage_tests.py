...
class ComprehensiveTestRunner:
    ...
    def __init__(self, live_system=False, coverage_only=False):
        ...
        self.base_url = "https://blacklist.jclee.me" if live_system else "http://localhost:32542"
        self.test_results = {}
    ...
    def run_pytest_coverage(self):
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                print(f"❌ Pytest coverage analysis failed (exit code: {result.returncode})")
                return False
    ...
    def generate_report(self):
        ...
        errors = sum(1 for r in self.test_results.values() if r['status'] in ['ERROR', 'TIMEOUT', 'NOT_FOUND'])
        ...
        if coverage_file.exists():
            try:
                ...
                if total_coverage >= 95:
                    ...
                    print("🎯 GOOD: Achieved 80%+ coverage, approaching enterprise target")
        ...
        if coverage_file.exists():
            try:
                ...
                low_coverage_files = [
                    filename for filename, file_data in files.items()
                    if file_data.get('summary', {}).get('percent_covered', 100) < 80
                ]
                
                if low_coverage_files:
                    print(f"  • Improve coverage for {len(low_coverage_files)} low-coverage files")
                    for filename in low_coverage_files[:5]:  # Show first 5
                        coverage_pct = files[filename].get('summary', {}).get('percent_covered', 0)
        ...
        overall_success = (
            passed == total and  # All test suites passed
            total > 0           # At least some tests ran
        )
