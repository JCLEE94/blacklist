...
class TestHealthEndpoints(unittest.TestCase):
    ...
    def test_health_check_response_format(self):
        ...
        expected_health_format = {"status": str, "timestamp": str, "service": str}
        ...
        try:
            ...
            self.assertIsInstance(
                parsed_timestamp, datetime, "Timestamp should be valid datetime"
            )
        except ValueError:
    ...
    def test_detailed_health_check(self):
        ...
        self.assertIn(
            "components", detailed_health, "Detailed health should have components"
        )
        self.assertIsInstance(
            detailed_health["components"], dict, "Components should be dict"
        )
        ...
        for component in required_components:
            self.assertIn(
                component,
                detailed_health["components"],
                f"Missing component: {component}",
            )

            component_data = detailed_health["components"][component]
            self.assertIn(
                "status", component_data, f"Component {component} missing status"
            )

        # Validate database component
        db_component = detailed_health["components"]["database"]
        self.assertIn(
            "response_time", db_component, "Database should have response_time"
        )
        self.assertIsInstance(
            db_component["response_time"],
            (int, float),
...
class TestBlacklistEndpoints(unittest.TestCase):
    ...
    def test_active_blacklist_format(self):
        ...
        self.assertEqual(
            text_response, expected_text, "Text format should match expected"
        )
        ...
        for ip in active_ips:
            ...
            for part in ip_parts:
                self.assertTrue(
                    0 <= int(part) <= 255, f"IP octet {part} should be 0-255"
                )
    ...
    def test_fortigate_external_connector_format(self):
        ...
        active_entries = [
            entry for entry in self.test_blacklist_data if entry["is_active"]
        ]
    ...
    def test_enhanced_blacklist_v2_format(self):
        ...
        v2_response = {
            ...
            "meta": {
                ...
                "active_count": len(
                    [e for e in self.test_blacklist_data if e["is_active"]]
                ),
...
class TestCollectionEndpoints(unittest.TestCase):
    ...
    def test_collection_status_response(self):
        ...
        for source in expected_sources:
            ...
            for field in source_required:
                self.assertIn(
                    field, source_data, f"Source {source} missing field: {field}"
                )

        # Validate statistics
        stats = collection_status["statistics"]
        self.assertIsInstance(
            stats["total_records"], int, "Total records should be int"
        )
        self.assertIsInstance(
            stats["new_records_today"], int, "New records should be int"
        )
    ...
    def test_collection_trigger_request(self):
        ...
        trigger_requests = [
            {
                "source": "REGTECH",
                "force": False,
                "date_range": {"start_date": "2024-08-20", "end_date": "2024-08-22"},
            },
            {"source": "SECUDIUM", "force": True, "immediate": True},
        ]
        ...
        for request_data in trigger_requests:
            ...
            self.assertIn(
                "force", request_data, "Trigger request should have force flag"
            )
            ...
            # Validate force flag
            self.assertIsInstance(
                request_data["force"], bool, "Force should be boolean"
            )
            ...
            if "date_range" in request_data:
                ...
                self.assertIn(
                    "start_date", date_range, "Date range should have start_date"
                )
...
class TestAuthenticationEndpoints(unittest.TestCase):
    ...
    def test_jwt_login_request_format(self):
        ...
        for field in required_fields:
            ...
            self.assertTrue(
                len(login_request[field]) > 0, f"{field} should not be empty"
            )

        # Mock successful login response
        login_response = {
            "success": True,
            "access_token": "eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJzdWIiOiIxMjM0NTY3ODkwIiwibmFtZSI6IkpvaG4gRG9lIn0.Gfx6VO9tcxwk6xqx9yYzSfebfeakZp5JYIgP_edcw_A",
            "refresh_token": "eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJzdWIiOiIxMjM0NTY3ODkwIiwibmFtZSI6IkpvaG4gRG9lIn0.Gfx6VO9tcxwk6xqx9yYzSfebfeakZp5JYIgP_edcw_A",
            "expires_in": 3600,
            "user": {
                "username": "admin",
                "role": "admin",
        ...
        self.assertEqual(
            len(token_parts), 3, "JWT should have 3 parts separated by dots"
        )

        # Validate expires_in
        self.assertIsInstance(
            login_response["expires_in"], int, "expires_in should be int"
        )
        self.assertTrue(
            login_response["expires_in"] > 0, "expires_in should be positive"
        )
