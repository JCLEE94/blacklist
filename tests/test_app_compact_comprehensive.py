...
class TestCompactFlaskApp:
    ...
    def test_create_app_system_stability_success(
        self, mock_init_stability, mock_get_container
    ):
    ...
    @patch("src.core.app_compact.get_container")
    @patch("src.utils.system_stability.initialize_system_stability")
    def test_create_app_system_stability_failure(
        self, mock_init_stability, mock_get_container
    ):
    ...
    def test_setup_advanced_features_success(self, mock_get_container):
        ...
        with (
            patch("src.utils.advanced_cache.get_smart_cache") as mock_cache,
            patch("src.utils.security.get_security_manager") as mock_security,
            patch(
                "src.utils.unified_decorators.initialize_decorators"
            ) as mock_decorators,
        ):
    ...
    def test_main_development_mode(self, mock_exists, mock_create_app):
        ...
        with patch.dict(os.environ, {"FLASK_ENV": "development"}):
            with patch.object(mock_app, "run") as mock_run:
                ...
                # The actual code uses 2542 as default port, not 8541
                mock_run.assert_called_once_with(host="0.0.0.0", port=2542, debug=True)
