...
try:
    ...
    pytest.skip(f"Unable to import refactored test modules: {e}", allow_module_level=True)
...
if __name__ == "__main__":
    ...
    try:
        required_classes = [
            TestBase, TestAuthenticationRoutes, TestAPIKeyRoutes,
            TestSecurityFeatures, TestPerformanceAndReliability
        ]
    ...
    try:
        ...
        test_classes = [
            TestAuthenticationRoutes, TestAPIKeyRoutes, 
            TestSecurityFeatures, TestPerformanceAndReliability
        ]
        
        for test_cls in test_classes:
            if not test_cls.__name__.startswith('Test'):
                all_validation_failures.append(f"Invalid test class name: {test_cls.__name__}")
                break
        else:
            # Test passed
            pass
    ...
    if all_validation_failures:
        ...
        print("Refactored authentication comprehensive module is validated and ready for use")
        sys.exit(0)
    ...
    def test_login_failure_invalid_credentials(self):
        ...
        assert response.status_code in [
            401,
            400,
        ]  # Accept both unauthorized and bad request
        data = response.json()

        # Check for failure indicator
        failure_indicator = (
            data.get("success", True) is False or data.get("status") == "error"
        )
    ...
    def test_login_validation_errors(self):
        ...
        # Missing username
        response = requests.post(
            f"{self.BASE_URL}/api/auth/login", json={"password": "test"}, timeout=10
        )
        ...
        # Missing password
        response = requests.post(
            f"{self.BASE_URL}/api/auth/login", json={"username": "test"}, timeout=10
        )
    ...
    def test_token_refresh_success(self):
        ...
        if login_response.status_code == 429:
            ...
            pytest.skip(
                f"Login failed with status {login_response.status_code} - test skipped"
            )
    ...
    def test_token_verification_valid(self):
        ...
        # Verify token
        response = requests.post(
            f"{self.BASE_URL}/api/auth/verify", json={"token": token}, timeout=10
        )
    ...
    def test_change_password_not_implemented(self):
        ...
        response = requests.post(
            ...
            json={"current_password": "old_pass", "new_password": "new_password123"},
            timeout=10,
        )
...
class TestAPIKeyRoutes:
    ...
    def test_list_api_keys(self):
        ...
        response = requests.get(
            f"{self.BASE_URL}/api/keys/list", headers=self.auth_headers, timeout=10
        )
    ...
    def test_validate_api_key_endpoint(self):
        ...
        response = requests.post(
            f"{self.BASE_URL}/api/keys/validate", json={"api_key": test_key}, timeout=10
        )
    ...
    def test_api_key_stats(self):
        ...
        response = requests.get(
            f"{self.BASE_URL}/api/keys/stats", headers=self.auth_headers, timeout=10
        )
    ...
    def test_unauthorized_access(self):
        ...
        response = requests.post(
            f"{self.BASE_URL}/api/keys/create", json={"name": "test"}, timeout=10
        )
        assert response.status_code == 401
...
class TestSecurityFeatures:
    ...
    def test_expired_token_handling(self):
        ...
        expired_token = (
            "eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJleHAiOjE2NDcyOTQyMDB9.invalid"
        )
    ...
    def test_injection_attack_prevention(self):
        ...
        for malicious_input in malicious_inputs:
            response = requests.post(
                f"{self.BASE_URL}/api/auth/login",
                json={"username": malicious_input, "password": malicious_input},
                timeout=10,
            )
    ...
    def test_authorization_header_variations(self):
        ...
        for header in invalid_headers:
            response = requests.get(
                f"{self.BASE_URL}/api/auth/profile", headers=header, timeout=10
            )
            assert response.status_code == 401
...
class TestPerformanceAndReliability:
    ...
    def test_concurrent_login_requests(self):
        ...
        # Start 3 concurrent requests with delay to avoid rate limiting
        threads = []
        for i in range(3):  # Reduced from 5 to 3 to avoid rate limiting
            thread = threading.Thread(target=login_worker)
            threads.append(thread)
            thread.start()
            if i < 2:  # Add delay between requests
