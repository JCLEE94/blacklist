...
from test_utils import TestBase, validate_response_structure, is_valid_jwt_format
    ...
    def test_profile_retrieval_authenticated(self):
        ...
        if response.status_code == 200:
            ...
            if data:  # Only check if data is not empty
                assert has_expected_field, f"No expected profile fields found in {list(data.keys())}"
...
if __name__ == "__main__":
    ...
    try:
        ...
        if hasattr(test_auth, 'BASE_URL') and hasattr(test_auth, 'test_tokens'):
            ...
            all_validation_failures.append("TestAuthenticationRoutes missing required attributes")
    except Exception as e:
        all_validation_failures.append(f"TestAuthenticationRoutes instantiation failed: {e}")
