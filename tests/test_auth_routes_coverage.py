...
class TestLoginEndpoint:
    ...
    def test_login_success_admin(self):
        ...
        with patch.dict(
            os.environ,
            {"ADMIN_USERNAME": "testadmin", "ADMIN_PASSWORD": "testpassword"},
        ):
            # Mock security manager
            mock_security = MagicMock()
            mock_security.generate_jwt_token.side_effect = [
                "access_token_123",
                "refresh_token_456",
            ]
    ...
    def test_login_success_collector(self):
        ...
        with patch.dict(
            os.environ,
            {"REGTECH_USERNAME": "regtech_user", "REGTECH_PASSWORD": "regtech_pass"},
        ):
            mock_security = MagicMock()
            mock_security.generate_jwt_token.side_effect = [
                "access_token_123",
    ...
    def test_login_invalid_credentials(self):
        ...
        with patch.dict(
            os.environ,
            {"ADMIN_USERNAME": "admin", "ADMIN_PASSWORD": "correct_password"},
        ):
            response = self.client.post(
                "/api/auth/login",
...
class TestProfileEndpoint:
    ...
    def test_get_profile_success(self):
        ...
        with self.app.app_context():
            ...
            response = self.client.get(
                "/api/auth/profile", headers={"Authorization": "Bearer valid_token_123"}
            )
    ...
    def test_get_profile_invalid_auth_header(self):
        ...
        response = self.client.get(
            "/api/auth/profile", headers={"Authorization": "InvalidFormat token123"}
        )
    ...
    def test_get_profile_invalid_token(self):
        ...
        with self.app.app_context():
            ...
            response = self.client.get(
                "/api/auth/profile", headers={"Authorization": "Bearer invalid_token"}
            )
...
if __name__ == "__main__":
    ...
    try:
        if auth_bp is None:
            ...
            all_validation_failures.append(
                f"Blueprint creation: Expected name 'auth', got '{auth_bp.name}'"
            )
        elif auth_bp.url_prefix != "/api/auth":
            all_validation_failures.append(
                f"Blueprint creation: Expected url_prefix '/api/auth', got '{auth_bp.url_prefix}'"
            )
    ...
    try:
        ...
        for route in expected_routes:
            if route not in route_rules:
                all_validation_failures.append(
                    f"Route registration: Missing expected route: {route}"
                )
    except Exception as e:
    ...
    try:
        ...
        if (
            not hasattr(auth_bp, "error_handler_spec")
            or auth_bp.error_handler_spec is None
        ):
            all_validation_failures.append(
                "Error handlers: error_handler_spec not found"
            )
        else:
            ...
            for error_code in expected_errors:
                if error_code not in error_spec:
                    all_validation_failures.append(
                        f"Error handlers: Missing error handler for {error_code}"
                    )
    except Exception as e:
    ...
    try:
        ...
        for func in functions:
            if not callable(func):
                all_validation_failures.append(
                    f"Route functions: {func.__name__} is not callable"
                )
    ...
    try:
        ...
        if not isinstance(admin_username, str):
            all_validation_failures.append(
                "Environment variables: ADMIN_USERNAME not string type"
            )
        if not isinstance(admin_password, str):
            all_validation_failures.append(
                "Environment variables: ADMIN_PASSWORD not string type"
            )
    except Exception as e:
