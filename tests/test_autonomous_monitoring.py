...
class TestAutonomousMonitor:
    ...
    def test_threshold_checking(self, monitor):
        ...
        # Git 변경사항 알림 확인
        git_alert = next(
            (a for a in alerts if a.metric_type == SystemMetrics.GIT_CHANGES), None
        )
        assert git_alert is not None
        assert git_alert.level in [
            AlertLevel.WARNING,
            AlertLevel.CRITICAL,
        ...
        coverage_alert = next(
            (a for a in alerts if a.metric_type == SystemMetrics.TEST_COVERAGE), None
        )
    ...
    def test_alert_message_generation(self, monitor):
        ...
        message = monitor._generate_alert_message(
            SystemMetrics.API_PERFORMANCE, 150.0, AlertLevel.WARNING
        )
...
class TestBackupManager:
    ...
    def test_git_backup_creation(self, backup_manager):
        ...
        with patch("subprocess.run") as mock_run:
            ...
            mock_run.side_effect = [
                ...
                Mock(
                    returncode=0, stdout="diff --git a/file1.py\n+new line\n"
                ),  # git diff
    ...
    def test_backup_cleanup(self, backup_manager):
        ...
        automation_backups = [
            r
            for r in backup_manager.backup_records
...
class TestPredictiveEngine:
    ...
    def test_anomaly_detection(self, predictive_engine):
        ...
        # 이상값 테스트
        anomaly_score = predictive_engine.anomaly_detector.detect_anomaly(
            "test_metric", 80
        )
    ...
    def test_trend_analysis(self, predictive_engine):
        ...
        trend, slope = predictive_engine.time_series_analyzer.detect_trend(
            increasing_values
        )
...
class TestIntegratedMonitoringSystem:
    ...
    async def test_end_to_end_monitoring_cycle(self, integrated_system):
        ...
        assert health_analysis["overall_health"] in [
            "excellent",
            "good",
            "fair",
            "poor",
...
def test_monitoring_api_endpoints_exist():
    ...
    try:
        from src.api.monitoring.autonomous_monitoring_routes import (
