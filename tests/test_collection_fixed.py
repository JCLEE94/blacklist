...
def test_regtech_direct():
    ...
    try:
        ...
        if result.get('success'):
            print(f"✅ REGTECH collection successful: {result.get('total_collected', 0)} IPs")
...
def test_collector_factory():
    ...
    try:
        ...
        if regtech:
            ...
            if result.get('success'):
                print(f"✅ Factory REGTECH collection: {result.get('total_collected', 0)} IPs")
            else:
...
def check_database_data():
    ...
    try:
        ...
        # Show sample data
        cursor.execute("SELECT ip_address, source, category, added_date FROM blacklist_ips LIMIT 5")
