...
class MockCollectionResponse:
    ...
    def __init__(
        self, status_code: int = 200, content: str = "", json_data: Dict = None
    ):
...
def mock_requests_session():
    ...
    login_response = MockCollectionResponse(
        status_code=200, json_data={"success": True, "token": "mock_token_12345"}
    )

    # 데이터 다운로드 응답 모의
    download_response = MockCollectionResponse(
        status_code=200, content="Mock Excel File Content"
    )
