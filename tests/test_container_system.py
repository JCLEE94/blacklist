    ...
    try:
        ...
        for service_name in services_to_test:
            try:
                ...
                print("  ✅ {service_name}: {'성공' if service else '실패'}")
            except Exception as e:
                resolved_services[service_name] = False
                print("  ❌ {service_name}: {str(e)[:50]}...")
        ...
        if successful_count >= total_count * 0.7:  # 70% 이상 성공
            print("✅ 컨테이너 서비스 등록/해결 테스트 통과 ({successful_count}/{total_count})")
            return True
        else:
            print("❌ 컨테이너 서비스 등록/해결 테스트 실패 ({successful_count}/{total_count})")
            return False

    except Exception as e:
        print("❌ 컨테이너 테스트 중 오류: {str(e)}")
        return False
...
def test_container_singleton_behavior():
    ...
    try:
        ...
        try:
            ...
            print("  ⚠️ 서비스 싱글톤 테스트 불가: {str(e)[:30]}...")
            singleton_ok = False
...
def test_container_dependency_injection():
    ...
    try:
        ...
        print("  📁 등록된 서비스 수: {len(service_list)}")
            ...
            if container.has_service(service):
                try:
                    ...
                    if instance is not None:
                        ...
                        print("  ⚠️ {service}: None 반환")
                except Exception as e:
                    missing_services.append(service)
                    print("  ❌ {service}: {str(e)[:30]}...")
            else:
                missing_services.append(service)
                print("  ❌ {service}: 등록되지 않음")

        success_rate = len(available_services) / len(required_services)

        if success_rate >= 0.7:  # 70% 이상
            print(
                "✅ 의존성 주입 테스트 통과 ({len(available_services)}/{len(required_services)})"
            )
            return True
        else:
            print(
                "❌ 의존성 주입 테스트 실패 ({len(available_services)}/{len(required_services)})"
            )
            return False

    except Exception as e:
        print("❌ 의존성 주입 테스트 중 오류: {str(e)}")
        return False
...
def test_container_error_handling():
    ...
    try:
        ...
        try:
            ...
            print("  ⚠️ 예상과 다른 예외 발생: {type(e).__name__}")

        # 컨테이너 건강 상태 확인
        try:
            health_status = container.get_health_status()
            print("  ✅ 건강 상태 추출 성공: {health_status.get('status', 'unknown')}")
        except Exception as e:
            print("  ⚠️ 건강 상태 추출 실패: {str(e)[:30]}...")

        print("✅ 컨테이너 에러 처리 테스트 완료")
        return True

    except Exception as e:
        print("❌ 에러 처리 테스트 중 오류: {str(e)}")
        return False
