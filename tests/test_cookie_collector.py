        ...
        for ip_data in collected_data:
            try:
                ...
                ''', (
                    ...
                    ip_data.get('detection_date', datetime.now().strftime('%Y-%m-%d')),
                    1
                ))
                saved_count += 1
            except Exception as e:
...
async def test_api_integration():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"   REGTECH 사용 가능: {data.get('sources', {}).get('regtech', {}).get('available')}")
        
        # 수집 트리거 (실제 쿠키가 있을 때만 성공)
        response = requests.post('http://localhost:32542/api/collection/regtech/trigger')
