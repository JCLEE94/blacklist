...
class TestConstants(unittest.TestCase):
    ...
    def test_http_status_codes(self):
        ...
        required_codes = ["OK", "BAD_REQUEST", "UNAUTHORIZED", "NOT_FOUND", "INTERNAL_SERVER_ERROR"]
...
class TestIPPatternValidation(unittest.TestCase):
    ...
    def test_ipv4_pattern(self):
        ...
        invalid_ipv4 = ["256.1.1.1", "192.168.1", "192.168.1.1.1", "not.an.ip"]
...
class TestSecurityConfiguration(unittest.TestCase):
    ...
    def test_rate_limiting_configuration(self):
        ...
        self.assertGreater(DEFAULT_RATE_LIMITS["admin"], DEFAULT_RATE_LIMITS["authenticated"])
        self.assertGreater(DEFAULT_RATE_LIMITS["authenticated"], DEFAULT_RATE_LIMITS["public"])
...
class TestSystemLimitsAndMonitoring(unittest.TestCase):
    ...
    def test_health_check_intervals(self):
        ...
        self.assertLess(HEALTH_CHECK_INTERVALS["critical"], HEALTH_CHECK_INTERVALS["normal"])
        self.assertLess(HEALTH_CHECK_INTERVALS["normal"], HEALTH_CHECK_INTERVALS["background"])
...
if __name__ == "__main__":
    ...
    try:
        ...
        if not API_VERSION or not isinstance(DEFAULT_CACHE_TTL, int) or not IP_PATTERNS:
            all_validation_failures.append("Constants import/validation: Basic constants failed validation")
    except Exception as e:
        all_validation_failures.append(f"Constants import/validation: Exception during import - {e}")
    ...
    try:
        ...
        if test_key != expected_key:
            all_validation_failures.append(f"Utility functions: Cache key generation failed. Expected {expected_key}, got {test_key}")
    except Exception as e:
        all_validation_failures.append(f"Utility functions: Exception during cache key test - {e}")
    
    # Test 3: Validation function tests
    total_tests += 1
    try:
        if not is_valid_ttl(300) or is_valid_ttl(-1) or not is_valid_port(8080) or is_valid_port(80):
            all_validation_failures.append("Validation functions: TTL or port validation failed")
    except Exception as e:
        all_validation_failures.append(f"Validation functions: Exception during validation tests - {e}")
    ...
    try:
        ...
        all_validation_failures.append(f"IP patterns: Regex compilation failed for {pattern_name} - {e}")
    
    # Test 5: Configuration structure validation
    total_tests += 1
    try:
        from src.core.constants import HTTP_STATUS_CODES, ERROR_MESSAGES, SUCCESS_MESSAGES
        if not HTTP_STATUS_CODES.get("OK") == 200:
            all_validation_failures.append("Configuration validation: HTTP status codes invalid")
        if not ERROR_MESSAGES.get("invalid_ip"):
            all_validation_failures.append("Configuration validation: Error messages missing")  
        if not SUCCESS_MESSAGES.get("auth_successful"):
            all_validation_failures.append("Configuration validation: Success messages missing")
    except Exception as e:
        all_validation_failures.append(f"Configuration validation: Exception during structure validation - {e}")
    ...
    if all_validation_failures:
        ...
        print("Constants and utilities module is validated and formal tests can now be written")
        sys.exit(0)