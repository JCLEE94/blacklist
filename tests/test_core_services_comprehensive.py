    ...
    def test_service_factory_singleton(self):
        ...
        try:
            ...
            if service1 is not None and service2 is not None:
                self.assertIs(service1, service2, "Service factory should return same instance")
        except ImportError:
...
class TestCacheIntegration(unittest.TestCase):
    ...
    def test_cache_key_generation_patterns(self):
        ...
        test_cases = [
            ...
            ("health", "check", datetime.now().strftime("%H"), f"health:check:{datetime.now().strftime('%H')}"),
        ]
...
class TestConfigurationManagement(unittest.TestCase):
    ...
    def test_configuration_hierarchy(self):
        ...
        config_sources = [
            {"source": "defaults", "priority": 1, "values": {"port": 8080, "debug": False}},
    ...
    def test_configuration_validation_patterns(self):
        ...
        for test in validation_tests:
            ...
            if test["valid"]:
                self.assertTrue(result, f"Config {test['key']}={test['value']} should be valid")
            else:
                self.assertFalse(result, f"Config {test['key']}={test['value']} should be invalid")
...
if __name__ == "__main__":
    ...
    try:
        ...
        if container is None:
            all_validation_failures.append("Container system: get_container() returned None")
    except ImportError:
        # Container might not be available in all environments
        pass
    except Exception as e:
        all_validation_failures.append(f"Container system: Exception during container access - {e}")
    ...
    try:
        ...
        all_validation_failures.append(f"Service factory: Exception during service access - {e}")
    
    # Test 3: Configuration validation
    total_tests += 1
    try:
        from src.core.constants import is_valid_port, is_valid_ttl, get_cache_key
        if not is_valid_port(8080) or is_valid_port(-1):
            all_validation_failures.append("Configuration validation: Port validation failed")
        if not is_valid_ttl(300) or is_valid_ttl(-1):
            all_validation_failures.append("Configuration validation: TTL validation failed")
        key = get_cache_key("test", "arg1", "arg2")
        if key != "test:arg1:arg2":
            all_validation_failures.append(f"Configuration validation: Cache key generation failed. Expected 'test:arg1:arg2', got '{key}'")
    except Exception as e:
        all_validation_failures.append(f"Configuration validation: Exception during validation - {e}")
    ...
    try:
        ...
        if handled_count != len(test_exceptions):
            all_validation_failures.append(f"Error handling: Expected to handle {len(test_exceptions)} exceptions, handled {handled_count}")
    except Exception as e:
        all_validation_failures.append(f"Error handling: Exception during error handling test - {e}")
    ...
    try:
        ...
        if current_step != len(lifecycle_steps):
            all_validation_failures.append(f"Service lifecycle: Expected {len(lifecycle_steps)} steps, completed {current_step}")
    except Exception as e:
        all_validation_failures.append(f"Service lifecycle: Exception during lifecycle test - {e}")
    ...
    if all_validation_failures:
        ...
        print("Core services functionality is validated and formal tests can now be written")
        sys.exit(0)