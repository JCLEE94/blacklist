    ...
    try:
        ...
        if collector._authenticate(session):
            ...
            if result.get("success"):
                print(
                    f"✅ REGTECH collection successful: {result.get('total_collected', 0)} IPs collected"
                )
            else:
...
def test_secudium_collection():
    ...
    try:
        ...
        if collector._authenticate(session):
            ...
            if result.get("success"):
                print(
                    f"✅ SECUDIUM collection successful: {result.get('total_collected', 0)} IPs collected"
                )
            else:
...
def test_collection_trigger():
    ...
    try:
        ...
        response = requests.post(f"{base_url}/regtech/trigger", timeout=30)
        if response.status_code == 200:
    ...
    try:
        ...
        response = requests.post(f"{base_url}/secudium/trigger", timeout=30)
        if response.status_code == 200:
            result = response.json()
            if result.get("success"):
