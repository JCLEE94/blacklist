...
def test_pytest_installation():
    ...
    assert major_version >= 7, f"pytest version {pytest.__version__} is too old, need >= 7.0"
...
def test_pytest_plugins():
    ...
    print(f"✅ pytest plugins available: cov={pytest_cov.__version__}, mock={mock_version}")
    return True
...
def test_test_discovery():
    ...
    assert current_function.startswith('test_'), "Test function naming convention correct"
