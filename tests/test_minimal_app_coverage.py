...
class TestBuildInfoContext:
    ...
    def test_build_info_context_with_file(self):
        ...
        with tempfile.NamedTemporaryFile(
            mode="w", delete=False, suffix=".build_info"
        ) as f:
            f.write("BUILD_TIME='2025-08-21 10:00:00 KST'\n")
            f.write("VERSION=1.1.9\n")
            temp_file = f.name
    ...
    try:
        ...
        if test_app is None:
            all_validation_failures.append(
                "Basic app creation: Expected Flask app, got None"
            )
        elif not hasattr(test_app, "config"):
            all_validation_failures.append(
                "Basic app creation: App missing config attribute"
            )
        elif test_app.config.get("SECRET_KEY") != "dev-secret-key-change-in-production":
            all_validation_failures.append(
                f"Basic app creation: Unexpected SECRET_KEY: {test_app.config.get('SECRET_KEY')}"
            )
    ...
    try:
        ...
        with test_app.app_context():
            ...
            if not context_processors:
                all_validation_failures.append(
                    "Template context: No context processors found"
                )
            else:
                ...
                for processor in context_processors:
                    try:
                        ...
                        all_validation_failures.append(
                            f"Template context: Context processor failed: {e}"
                        )

                if "build_time" not in context:
                    all_validation_failures.append(
                        "Template context: build_time not in context"
                    )
    except Exception as e:
    ...
    try:
        ...
        for key in required_config:
            if key not in test_app.config:
                all_validation_failures.append(
                    f"Configuration: Missing required config key: {key}"
                )

        # Check expected values
        expected_values = {"DEBUG": False, "TESTING": False}
        for key, expected in expected_values.items():
            if test_app.config.get(key) != expected:
                all_validation_failures.append(
                    f"Configuration: {key} expected {expected}, got {test_app.config.get(key)}"
                )
    except Exception as e:
        all_validation_failures.append(
            f"Configuration validation: Exception raised: {e}"
        )
