...
class TestPerformanceMonitoring:
    ...
    def test_api_response_times(self):
        ...
        for endpoint in endpoints:
            ...
            print(
                f"  {endpoint}: {response_time:.3f}s (status: {response.status_code})"
            )
        ...
        for header, expected_values in recommended_headers.items():
            if header in headers:
                ...
                has_expected_value = any(
                    val.lower() in header_value for val in expected_values
                )
                print(
                    f"  {header}: {headers[header]} ({'✓' if has_expected_value else '○'})"
                )
...
class TestAlertingAndNotifications:
    ...
    def test_notification_channels(self):
        ...
        response = requests.get(
            f"{self.BASE_URL}/api/monitoring/notifications", timeout=10
        )
