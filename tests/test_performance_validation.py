...
def test_api_response_time_baseline():
    ...
    try:
        ...
        with patch('src.core.services.unified_service_core.get_container', return_value=mock_container):
            ...
            assert avg_response_time < 1000, f"Response time {avg_response_time:.2f}ms exceeds 1000ms threshold"
    ...
    try:
        ...
        with patch('src.core.services.unified_service_core.get_container', return_value=mock_container):
            ...
            def make_request():
                try:
                    start_time = time.time()
                    result = service.get_active_ips() if hasattr(service, 'get_active_ips') else ["test"]
                    end_time = time.time()
                    response_time = (end_time - start_time) * 1000
            ...
            if response_times:
                ...
                assert len(response_times) >= 40, "Should handle most concurrent requests"
                assert avg_response_time < 5000, "Response time under load should be reasonable"
...
def test_database_query_performance():
    ...
    try:
        ...
        # Insert test data
        test_data = [(f"192.168.1.{i}", "test_source", "2024-01-01") for i in range(1000)]
        cursor.executemany("INSERT INTO test_blacklist (ip_address, source, detection_date) VALUES (?, ?, ?)", test_data)
        conn.commit()
        ...
        for _ in range(100):
            start_time = time.time()
            cursor.execute("SELECT COUNT(*) FROM test_blacklist WHERE source = ?", ("test_source",))
