...
def test_security_module_direct_calls():
    ...
    from src.utils.security import (
        SecurityManager, SecurityHeaders, sanitize_input, 
        generate_csrf_token, validate_csrf_token, setup_security
    )
    ...
    print(f"API key parts: {parts}, part 1 length: {len(parts[1]) if len(parts) > 1 else 'N/A'}")
...
def test_performance_optimizer_direct_calls():
    ...
    from src.utils.performance_optimizer import (
        PerformanceMetrics, QueryOptimizer, SmartCache, 
        MemoryOptimizer, PerformanceMonitor, 
        get_performance_monitor, optimize_database_queries,
...
def test_structured_logging_direct_calls():
    ...
    from src.utils.structured_logging import (
        StructuredLogger, BufferHandler, LogManager,
...
def test_memory_core_optimizer_direct_calls():
    ...
    with patch('src.utils.memory.core_optimizer.psutil') as mock_psutil:
        ...
        with patch('src.utils.memory.core_optimizer.gc.get_count', return_value=[100, 50, 10]):
