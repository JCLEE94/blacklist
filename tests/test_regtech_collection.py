...
class REGTECHCollector:
                        ...
                        for item in items:
                            ...
                            if ip:
                                collected_ips.append({
                                    ...
                                    'source': 'REGTECH',
                                    'threat_level': item.get('threat_level', 'medium') if isinstance(item, dict) else 'medium'
                                })
    ...
    def save_results(self, ips):
        ...
        # JSON 저장
        output_file = f"regtech_collection_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
