...
class REGTECHCookieCollector:
                ...
                if response.status_code == 200:
                    ...
                    if 'excel' in content_type or 'spreadsheet' in content_type:
                        ...
                        try:
                            ...
                            # IP 컬럼 찾기
                            ip_columns = [col for col in df.columns if 'ip' in col.lower() or '아이피' in col or 'IP' in col]
                    ...
                    # JSON 응답 체크
                        ...
                        try:
                            ...
                            if isinstance(data, dict):
                                # 다양한 키 시도
                                for key in ['data', 'items', 'list', 'blacklist', 'ips']:
                                    if key in data and isinstance(data[key], list):
                                        items = data[key]
                                        for item in items[:10]:
                                            if isinstance(item, dict):
                                                ip = item.get('ip') or item.get('ipAddress') or item.get('target_ip')
                                                if ip:
                                                    collected_data.append({
                                                        'ip': ip,
                                                        'source': 'REGTECH',
    ...
    def save_results(self, data):
        ...
        filename = f"regtech_cookie_collection_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
