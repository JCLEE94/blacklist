...
def test_regtech_collector():
    ...
    try:
        ...
        # Collector 초기화
        collector = RegtechCollector(username="nextrade", password="Sprtmxm1@3")

        print("✅ Collector 초기화 성공")
        print(f"   - Username: {collector.username}")
        print(
            f"   - Base URL: {collector.base_url if hasattr(collector, 'base_url') else 'N/A'}"
        )
        ...
        if login_result:
            ...
            if hasattr(collector, "collect_from_web"):
                ...
                result = collector.collect_from_web(
                    start_date=start_date, end_date=end_date, max_pages=1, page_size=100
                )
                ...
                print(
                    f"   사용 가능한 메서드: {[m for m in dir(collector) if not m.startswith('_')]}"
                )
