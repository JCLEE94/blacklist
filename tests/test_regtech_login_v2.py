...
class REGTECHLoginTest:
    ...
    def test_login(self):
        ...
        try:
            ...
            if login_form:
                ...
                for hidden in login_form.find_all('input', {'type': 'hidden'}):
                    ...
                    if name:
                        ...
                        print(f"   Hidden: {name} = {value[:20]}..." if len(value) > 20 else f"   Hidden: {name} = {value}")
            ...
            # 방법 2: meta 태그
            if not csrf_token:
                csrf_meta = soup.find('meta', {'name': 'csrf-token'}) or soup.find('meta', {'name': '_csrf'})
                if csrf_meta:
        ...
        try:
            ...
            if response.text:
                ...
                try:
                    ...
                    if json_data.get('success') or json_data.get('result') == 'success':
                        ...
                        print(f"   ❌ 로그인 실패: {json_data.get('message', 'Unknown error')}")
                        return False
                except:
                    # HTML 응답 분석
                    if '성공' in response.text or 'success' in response.text.lower():
                        ...
                        error_msg = re.search(r'(비밀번호가?.*?[.。]|아이디가?.*?[.。])', response.text)
                        if error_msg:
            ...
            if response.status_code in [301, 302, 303, 307]:
                ...
                if redirect_url and ('main' in redirect_url.lower() or 'index' in redirect_url.lower()):
                    print("   ✅ 로그인 성공 (리다이렉트)")
                    return True
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if ips:
                        ...
                        # 다운로드 링크 찾기
                        if 'download' in response.text.lower() or 'excel' in response.text.lower():
                            print("   📥 다운로드 링크 발견")
