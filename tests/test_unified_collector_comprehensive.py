...
class TestUnifiedCollectorDataCollection:
    ...
    def test_collect_all_success(self):
        ...
        with (
            patch.object(
                collector, "collect_regtech_data", return_value=[{"ip": "1.1.1.1"}]
            ),
            patch.object(
                collector, "collect_secudium_data", return_value=[{"ip": "2.2.2.2"}]
            ),
    ...
    def test_collect_all_partial_failure(self):
        ...
        with (
            ...
            patch.object(
                ...
                side_effect=Exception("Secudium failed"),
            ),
            patch.object(collector, "save_to_database"),
        ):
    ...
    def test_collect_regtech_data_success(self):
        ...
        with (
            patch.object(collector.session, "post") as mock_post,
    ...
    def test_collect_secudium_data_failure(self):
        ...
        with patch.object(
            collector.session, "post", side_effect=Exception("Network error")
        ):
            result = collector.collect_secudium_data()
            assert isinstance(result, list)
            assert len(result) == 0
...
class TestUnifiedCollectorErrorHandling:
    ...
    def test_network_timeout_handling(self):
        ...
        with patch.object(
            collector.session, "post", side_effect=TimeoutError("Request timeout")
        ):
            result = collector.collect_regtech_data()
            assert isinstance(result, list)
            assert len(result) == 0
    ...
    def test_invalid_credentials_handling(self):
        ...
        collector = UnifiedCollector(
            regtech_username="invalid_user", regtech_password="invalid_pass"
        )
    ...
    def test_excel_parsing_error(self):
        ...
        with (
            ...
            with patch(
                "pandas.read_excel", side_effect=Exception("Excel parsing failed")
            ):
                result = collector.collect_regtech_data()
                assert isinstance(result, list)
                assert len(result) == 0
    ...
    def test_missing_credentials(self):
        ...
        collector = UnifiedCollector(regtech_username=None, regtech_password=None)
...
class TestUnifiedCollectorLogging:
    ...
    def test_error_logging(self, mock_logger):
        ...
        with patch.object(
            collector.session, "post", side_effect=Exception("Test error")
        ):
    ...
    def test_info_logging(self, mock_logger):
        ...
        with (
            patch.object(collector, "collect_regtech_data", return_value=[]),
