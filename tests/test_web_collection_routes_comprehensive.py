...
class TestRawDataViewerRoute:
    ...
    def test_raw_data_viewer_template_error(self, mock_url_for, mock_redirect, 
                                          mock_flash, mock_render, mock_logger):
...
class TestApiRawDataRoute:
    ...
    def test_api_raw_data_with_params(self):
        ...
        with app.test_client() as client:
            response = client.get('/api/raw-data?page=2&limit=50&date=2024-01-01&source=REGTECH')
...
class TestRouteParameterHandling:
    ...
    def test_request_args_extraction(self):
        ...
        with app.test_client() as client:
            response = client.get('/test-params?page=5&limit=25&date=2024-01-01&source=TEST')
...
class TestUrlPatterns:
    ...
    def test_route_methods(self):
        ...
        with app.test_client() as client:
            ...
            for path in ['/blacklist-search', '/collection-control', '/raw-data', '/api/raw-data']:
...
class TestIntegration:
    ...
    def test_multiple_requests_to_same_route(self):
        ...
        with patch('src.web.collection_routes.render_template', return_value="template"):
            with app.test_client() as client:
...
class TestFlashMessaging:
    ...
    def test_flash_message_on_error(self, mock_render, mock_flash):
        ...
        with patch('src.web.collection_routes.redirect'), \
             patch('src.web.collection_routes.url_for'):
