...
class BaseUITestSuite:
    ...
    async def measure_performance(self, action_name: str, action_func: Callable, threshold: float) -> Any:
        """성능 측정 데코레이터"""
        ...
        try:
            ...
            self.reporter.add_error(f"Performance test failed for {action_name}: {str(e)}")
            raise
    
    async def check_element_visibility(self, selector: str, timeout: int = 5000, required: bool = True) -> bool:
        """요소 가시성 확인"""
        try:
            ...
            if required:
                self.reporter.add_error(f"Required element not visible: {selector} - {str(e)}")
                raise
            else:
    ...
    async def execute_test_with_timing(self, test_name: str, test_func: Callable) -> None:
        """테스트 실행 및 시간 측정"""
        start_time = time.time()
    ...
    async def test_chart_rendering(self, chart_selector: str) -> None:
        ...
        await self.measure_performance(
            "chart_render",
            lambda: self.page.wait_for_selector(f"{chart_selector} canvas", timeout=3000),
    ...
    async def check_responsive_element(self, selector: str, max_width: int) -> bool:
        ...
        if await element.count() > 0:
            ...
            if element_width > max_width:
                self.reporter.add_warning(f"Element {selector} overflows on mobile: {element_width}px > {max_width}px")
                return False
        return True
