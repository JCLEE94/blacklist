...
class CollectionTestSuite(BaseUITestSuite):
    ...
    async def test_collection_logs(self, selectors):
        """수집 로그 테스트"""
        log_container = self.page.locator(
            selectors.get("logs_container", ".collection-logs")
        )
    ...
    async def test_api_documentation(self):
        ...
        async def api_docs_test():
            ...
            found_indicators = await self.check_multiple_elements(
                page_indicators, "API 문서"
            )
                ...
                try:
                    ...
                    if response not in [200, 401, 403]:  # 인증이 필요할 수 있음
                        self.reporter.add_warning(
                            f"API 엔드포인트 {href} 응답 상태 이상: {response}"
                        )
