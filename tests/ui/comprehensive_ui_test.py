        ...
        for test_func in test_functions:
            try:
                ...
                self.reporter.add_error(
                    f"테스트 {test_func.__name__} 실행 중 오류: {str(e)}"
                )
    ...
    async def run_performance_tests(self):
        ...
        for test_func in performance_tests:
            try:
                ...
                self.reporter.add_error(
                    f"성능 테스트 {test_func.__name__} 실행 중 오류: {str(e)}"
                )
    ...
    async def run_core_tests(self):
        ...
        for test_func in core_tests:
            try:
                ...
                self.reporter.add_error(
                    f"핵심 테스트 {test_func.__name__} 실행 중 오류: {str(e)}"
                )
...
async def test_comprehensive_ui_functionality(ui_reporter):
    ...
    if ui_reporter.has_failures():
        pytest.fail(
            f"UI 테스트 실패: {len(ui_reporter.results['failed_tests'])}개 테스트 실패"
        )
