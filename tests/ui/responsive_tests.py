...
class ResponsiveTestSuite(BaseUITestSuite):
    ...
    async def test_desktop_layout(self):
        ...
        desktop_nav_visible = (
            await desktop_nav.count() > 0 and await desktop_nav.first.is_visible()
        )
    ...
    async def test_chart_responsive(self):
        ...
        if await chart_container.count() > 0:
            ...
            await self.check_responsive_element(
                ".chart-container, #blacklist-chart", viewport_width
            )
        ...
        if table_count > 0:
            for i in range(min(table_count, 3)):  # 최대 3개 테이블만 확인
                ...
                if not is_scrollable:
                    await self.check_responsive_element(
                        f"table:nth-of-type({i+1})", viewport_width
                    )
...
class RealtimeTestSuite(BaseUITestSuite):
    ...
    async def test_auto_refresh_controls(self):
        ...
        auto_refresh_found = await self.check_multiple_elements(
            auto_refresh_controls, "자동 새고침"
        )
