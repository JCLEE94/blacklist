...
class UITestReporter:
    ...
    def get_test_stats(self) -> Dict[str, int]:
        ...
        pass_rate = (len(self.results["passed_tests"]) / total_tests * 100) if total_tests > 0 else 0
    ...
    def get_performance_summary(self) -> Dict[str, str]:
        ...
        for metric_name, data in self.results["performance_metrics"].items():
            ...
            summary.append(f"  {status_icon} {metric_name}: {data['value']:.2f}ms (임계값: {data['threshold']}ms)")
    ...
    def generate_report(self) -> str:
        ...
        if self.results["failed_tests"]:
            ...
            for test in self.results["failed_tests"]:
                report.append(f"  - {test['test_name']}: {test.get('details', {}).get('error', 'Unknown error')}")
