...
from src.core.database.schema_manager import DatabaseSchema, get_database_schema, initialize_database, migrate_database
...
class TestDatabaseSchema:
    ...
    def test_get_connection(self, schema_manager):
        ...
        with mock.patch.object(schema_manager.connection_manager, 'get_connection') as mock_conn:
    ...
    def test_create_all_tables_success(self, schema_manager):
        ...
        with mock.patch.object(schema_manager.connection_manager, 'get_connection') as mock_get_conn:
            mock_conn = mock.Mock()
            mock_get_conn.return_value.__enter__.return_value = mock_conn
            
            with mock.patch.object(schema_manager.table_definitions, 'create_all_tables', return_value=True):
    ...
    def test_create_all_tables_failure(self, schema_manager):
        ...
        with mock.patch.object(schema_manager.connection_manager, 'get_connection') as mock_get_conn:
            ...
            with mock.patch.object(schema_manager.table_definitions, 'create_all_tables', return_value=False):
                result = schema_manager.create_all_tables()
                assert result is False

    def test_create_all_tables_exception(self, schema_manager):
        """Test table creation with exception"""
        with mock.patch.object(schema_manager.connection_manager, 'get_connection', side_effect=Exception("DB Error")):
            result = schema_manager.create_all_tables()
            assert result is False
    ...
    def test_get_current_schema_version(self, schema_manager):
        ...
        with mock.patch.object(schema_manager.migration_service, 'get_current_schema_version', return_value="1.0.0"):
    ...
    def test_migrate_schema(self, schema_manager):
        ...
        with mock.patch.object(schema_manager.migration_service, 'migrate_schema', return_value=True):
            result = schema_manager.migrate_schema("1.0.0")
            assert result is True
            schema_manager.migration_service.migrate_schema.assert_called_once_with("1.0.0")

    def test_cleanup_old_data(self, schema_manager):
        """Test cleanup_old_data method"""
        with mock.patch.object(schema_manager.migration_service, 'cleanup_old_data', return_value=42):
            result = schema_manager.cleanup_old_data(30)
            assert result == 42
            schema_manager.migration_service.cleanup_old_data.assert_called_once_with(30)

    def test_vacuum_database(self, schema_manager):
        """Test vacuum_database method"""
        with mock.patch.object(schema_manager.migration_service, 'vacuum_database', return_value=True):
    ...
    def test_get_table_stats_connection_error(self, schema_manager):
        ...
        with mock.patch.object(schema_manager.connection_manager, 'get_connection', side_effect=Exception("Connection failed")):
            stats = schema_manager.get_table_stats()
            assert stats == {}
...
class TestModuleFunctions:
    ...
    def test_initialize_database(self):
        ...
        with mock.patch.object(DatabaseSchema, 'initialize', return_value=True) as mock_initialize:
    ...
    def test_migrate_database(self):
        ...
        with mock.patch.object(DatabaseSchema, 'migrate', return_value=True) as mock_migrate:
