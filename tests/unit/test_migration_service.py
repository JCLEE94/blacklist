...
class TestMigrationService:
    ...
    def test_get_current_schema_version_exists(self, migration_service):
        ...
        migration_service.connection_manager.get_connection.return_value.__enter__ = Mock(return_value=mock_conn)
        migration_service.connection_manager.get_connection.return_value.__exit__ = Mock(return_value=None)
    ...
    def test_get_current_schema_version_exception(self, migration_service):
        """현재 스키마 버전 조회 - 예외 발생"""
        migration_service.connection_manager.get_connection.side_effect = Exception("DB Error")
    ...
    def test_migrate_schema_from_v1(self, mock_record, mock_migrate_v2, migration_service):
    ...
    @patch.object(MigrationService, '_migrate_to_v2')
    def test_migrate_schema_no_previous_version(self, mock_migrate_v2, migration_service):
        ...
        assert result is True
        # 메타데이터 테이블 생성 확인
        migration_service.table_definitions.create_metadata_table.assert_called_once_with(mock_conn)
        mock_migrate_v2.assert_called_once_with(mock_conn)
    ...
    def test_migrate_schema_exception(self, migration_service):
        ...
        migration_service.connection_manager.get_connection.side_effect = Exception("Migration failed")
    ...
    def test_migrate_to_v2(self, migration_service):
        ...
        # 새 테이블 생성 메서드 호출 확인
        migration_service.table_definitions.create_auth_attempts_table.assert_called_once_with(mock_conn)
        migration_service.table_definitions.create_system_status_table.assert_called_once_with(mock_conn)
        migration_service.table_definitions.create_cache_table.assert_called_once_with(mock_conn)
        migration_service.table_definitions.create_metadata_table.assert_called_once_with(mock_conn)
        migration_service.table_definitions.create_system_logs_table.assert_called_once_with(mock_conn)
    ...
    def test_cleanup_old_data_exception(self, migration_service):
        """오래된 데이터 정리 - 예외 발생"""
        migration_service.connection_manager.get_connection.side_effect = Exception("Cleanup failed")
    ...
    def test_vacuum_database_exception(self, migration_service):
        """데이터베이스 압축 - 예외 발생"""
        migration_service.connection_manager.get_connection.side_effect = Exception("Vacuum failed")
