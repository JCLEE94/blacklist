...
class TestStatisticsServiceMixin:
    ...
    def populated_db(self, temp_db_path):
        ...
        # Insert test data
        test_data = [
            ('192.168.1.1', 'regtech', 1, '2023-01-01 10:00:00', '2023-01-01 10:00:00', '2023-01-01', 'malware', 'US'),
            ('192.168.1.2', 'secudium', 1, '2023-01-02 11:00:00', '2023-01-02 11:00:00', '2023-01-02', 'phishing', 'CN'),
            ('192.168.1.3', 'regtech', 0, '2023-01-03 12:00:00', '2023-01-03 12:00:00', '2023-01-03', 'botnet', 'RU'),
            ('192.168.1.4', 'secudium', 1, '2023-01-04 13:00:00', '2023-01-04 13:00:00', '2023-01-04', 'spam', 'KR'),
        ]
    ...
    async def test_get_statistics_success(self, service):
        ...
        service.blacklist_manager.get_active_ips.return_value = ['192.168.1.1', '192.168.1.2']
    ...
    async def test_get_statistics_exception(self, service):
        ...
        service.blacklist_manager.get_system_health.side_effect = Exception("Health check failed")
    ...
    def test_get_blacklist_summary_success(self, service):
        ...
        service.blacklist_manager.get_active_ips.return_value = ['192.168.1.1', '192.168.1.2', '192.168.1.3']
        
        with mock.patch.object(service, '_get_source_counts_from_db', return_value={'regtech': 2, 'secudium': 1}):
    ...
    def test_get_blacklist_summary_exception(self, service):
        ...
        service.blacklist_manager.get_active_ips.side_effect = Exception("Database error")
    ...
    def test_get_analytics_summary(self, service):
        ...
        with mock.patch.object(service, 'get_blacklist_summary', return_value={'success': True}):
            with mock.patch.object(service, 'get_daily_stats', return_value=[]):
                with mock.patch.object(service, 'get_source_statistics', return_value={}):
    ...
    def test_get_analytics_summary_exception(self, service):
        ...
        with mock.patch.object(service, 'get_blacklist_summary', side_effect=Exception("Summary failed")):
    ...
    def test_get_daily_collection_stats_exception(self, service):
        ...
        with mock.patch.object(service, 'get_collection_logs', side_effect=Exception("Log error")):
            result = service.get_daily_collection_stats()
    ...
    def test_get_blacklist_paginated_with_source_filter(self, service):
        ...
        with mock.patch.object(service, 'get_blacklist_with_metadata') as mock_get:
            ...
            result = service.get_blacklist_paginated(page=1, per_page=25, source_filter='regtech')
            
            mock_get.assert_called_once_with(limit=25, offset=0, source_filter='regtech')

    def test_get_blacklist_paginated_exception(self, service):
        """Test paginated blacklist retrieval with exception"""
        with mock.patch.object(service, 'get_blacklist_with_metadata', side_effect=Exception("Pagination error")):
    ...
    def test_pagination_calculation(self, service):
        ...
        with mock.patch.object(service, 'get_blacklist_with_metadata') as mock_get:
            ...
            for page, per_page, expected_offset in test_cases:
                ...
                mock_get.assert_called_with(limit=per_page, offset=expected_offset, source_filter=None)
